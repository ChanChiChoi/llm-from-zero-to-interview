...
所以面试中不要把多模态简单说成“给 LLM 加一个 vision encoder”。更准确的理解是：
...
多模态 token 数还会快速放大 attention 成本。可以粗略写成：
...
所以 connector、resampler、patch 压缩、帧采样和检索式上下文构造，都是多模态系统里的核心工程设计，不是附属细节。
...
一个多模态上线 gate 至少要同时看任务质量、grounding、安全和成本：
...
def zero_shot_classify(image_vector, templates):
    ...
    for label in labels:
        ...
        class_text_vec = [sum(vec[i] for vec in prompt_vectors) / len(prompt_vectors) for i in range(3)]
...
4. prompt ensemble 不是换分类头，而是让类别文本表示更稳。
...
所以高分辨率、小 patch 会以二次关系放大视觉 encoder 的 attention 成本。
...
它仍然是图文对齐预训练，只是把 CLIP 的 batch softmax 分类目标换成了 pairwise sigmoid 目标。
...
def conv_params(channels=768, kernel=7):
    ...
    return {
        ...
        "convnext_block_main_params": depthwise + pointwise,
    }
...
def siglip_loss():
    ...
    for image, text, logit, label in pairs:
        loss = -math.log(sigmoid(label * logit))
        losses.append(loss)
        rows.append({
            "pair": f"{image}-{text}",
            "label": label,
...
convnext= {'dense_7x7_params': 28901376, 'depthwise_7x7_params': 37632, 'depthwise_reduction_x': 768.0, 'convnext_block_main_params': 4756224}
...
+
\tanh(g_\ell)
\operatorname{CrossAttn}(h_\ell,Z_v)
```

其中 `h_l` 是语言模型第 `l` 层的文本 hidden state，`Z_v` 是 Perceiver Resampler 输出的视觉 latents，`g_l` 是可学习 gate。gate 的作用是让模型逐步学习何时使用视觉信息，而不是一开始强行破坏语言模型行为。
...
其中 `T` 是文本 token 数，`n_v` 是原始视觉 token 数，`K_f` 是 Flamingo visual latent 数，`K_q` 是 Q-Former query 数，`L_c` 是插入 cross-attention 的层数。这个估算不是精确硬件时间，但能说明不同路线把成本放在不同位置。
...
def score_cells(text_tokens=128, heads=32, cross_heads=16):
    ...
    for name, cfg in ARCHS.items():
        total_tokens = text_tokens + cfg["llm_visual_tokens"]
...
{'model': 'flamingo_resampler', 'llm_visual_tokens': 64, 'llm_score_cells': 1179648, 'cross_score_cells': 4718592, 'total_relative_to_llava': 0.372, 'trainable_billion': 2.0, 'llm_modified': True}
...
2. Flamingo 路线压缩 LLM 视觉 token，但用多层 cross-attention 换来 interleaved/few-shot 能力。
...
真实项目会按样本级和数据集级同时做过滤：样本级过滤明显错误样本，数据集级观察任务覆盖、长度分布和风险分布。
...
其中 `A` 表示 assistant answer token 集合。多模态 SFT loss 是：
...
这和文本 SFT 的 assistant-only loss 本质相同，只是条件里多了视觉 token。
...
def audit_samples(samples):
    ...
    return {
        ...
        "unsupported_claim_rate": round(unsupported_total / max(1, claim_total), 3),
        "kept_ids": kept,
...
def dpo_loss(pair, beta=0.2):
    ...
    return {
        ...
        "chosen_prob": round(sigmoid(margin), 3),
        "loss": round(-math.log(sigmoid(margin)), 3),
    }
...
def audit_pairs(pairs):
    ...
    return {
        ...
        "avg_chosen_prob": round(sum(row["chosen_prob"] for row in rows) / len(rows), 3),
...
```text
audit= {'avg_loss': 0.738, 'avg_chosen_prob': 0.478, 'hallucination_negative_rate': 0.667, 'over_refusal_negative_rate': 0.333, 'rows': [{'id': 'blurry_sign', 'delta_model': -0.6, 'delta_ref': -0.2, 'chosen_prob': 0.48, 'loss': 0.734}, {'id': 'red_car_absent', 'delta_model': -0.7, 'delta_ref': -0.1, 'chosen_prob': 0.47, 'loss': 0.755}, {'id': 'visible_cat', 'delta_model': -0.2, 'delta_ref': 0.1, 'chosen_prob': 0.485, 'loss': 0.724}]}
```

这组输出说明：

1. 当前模型在三个样本上还没有充分偏向 chosen，平均 `chosen_prob` 低于 `0.5`。
2. 前两个 rejected 是幻觉 hard negative，训练会惩罚看似具体但没有图像依据的回答。
3. 第三个 rejected 是过度拒答，避免模型学成“凡事都说无法确定”。
4. 真实训练时还要把 log probability 换成模型对完整 answer token 的累积 log probability。
...
答案正确但 citation 错，说明系统不可审计；citation 对但答案错，说明推理或抽取仍有问题。
...
def ocr_metrics(gold, pred):
    cer = edit_distance(list(gold), list(pred)) / max(1, len(gold))
    wer = edit_distance(gold.split(), pred.split()) / max(1, len(gold.split()))
    return {"cer": round(cer, 3), "wer": round(wer, 3)}


def field_f1(gold, pred):
    tp = sum(1 for key, value in pred.items() if gold.get(key) == value)
...
def chart_metrics(gold_points, pred_points):
    ...
    for key, gold_value in gold_points.items():
        ...
        rel_errors.append(abs(pred_value - gold_value) / max(abs(gold_value), 1e-9))
    gold_growth = gold_points["2022"] - gold_points["2021"]
    pred_growth = pred_points["2022"] - pred_points["2021"]
    return {
        "avg_relative_error": round(sum(rel_errors) / len(rel_errors), 3),
...
chart= {'avg_relative_error': 0.026, 'growth_gold': 30, 'growth_pred': 28, 'growth_abs_error': 2, 'within_abs_tol': True}
...
其中 `(x_1,y_1)` 是左上角，`(x_2,y_2)` 是右下角。坐标可以是像素坐标，也可以归一化到 `[0,1]` 或 `[0,1000]`，但训练、推理和评估必须使用同一套坐标系。

一个 grounded answer 可以抽象成：
...
其中 `a` 是文本答案，`E` 是支持答案的视觉证据集合，例如 box、mask、点坐标、OCR 区域或文档 citation。
...
若对象 `a` 在对象 `b` 左侧，可以用一个容差 `delta` 写成：
...
这个式子只是最小示意。真实系统还要考虑坐标是否来自裁剪图、是否被镜像翻转、对象是否重叠，以及前后关系是否需要深度估计。
...
pointing accuracy 检查预测点是否落在 gold box 内：
...
如果答案需要引用证据，可以定义 evidence precision：

```math
P_{\mathrm{evi}}
=
\frac{
N_{\mathrm{correctevi}}
}{
N_{\mathrm{predevi}}
}
```
...
\sum_{i=1}^{N}
I[\hat{a}_i=a_i]
I[IoU(\hat{b}_i,b_i)\ge \tau]
```

这个指标会同时惩罚“答案对但证据错”和“证据对但答案错”的情况。

---

### 十二、最小代码：Grounding、空间关系与联合评估

下面的 demo 用纯 Python 模拟三个判断：

1. 预测 box 和 gold box 的 IoU 是否达标。
2. 预测点是否落在证据 box 内。
3. 空间关系答案是否和 box 中心点一致，并计算 answer + evidence joint accuracy。
...
3. `person_right_of_car` box 对但答案错，说明定位模块没问题，语言推理或坐标关系判断错。
...
这里 \(x\) 表示图像，\(c\) 表示条件，\(\theta\) 表示模型参数。

对很多显式概率建模路线，可以把训练目标粗略写成最小化负对数似然：
...
但不同生成路线对这个目标的实现方式不同：VAE 用变分下界，Flow 用可逆变换精确 likelihood，自回归模型逐 token 分解概率，Diffusion 学习去噪过程，GAN 则用对抗目标间接逼近数据分布。
...
如果把图像离散成 \(N_v\) 个视觉 token \(v_1,\ldots,v_{N_v}\)，条件自回归生成可以写成：

\[
p_\theta(v\mid c)
=
\prod_{i=1}^{N_v}p_\theta(v_i\mid c,v_{1:i-1})
\]
...
常见简化训练目标是让模型从 noisy image 或 noisy latent 中预测加入的噪声：
...
如果像素图元素数为 \(H_xW_xC_x\)，latent 元素数为 \(H_zW_zC_z\)，压缩带来的元素级成本下降可以粗略写成：
...
例如 \(512\times512\times3\) 的像素图被压到 \(64\times64\times4\) 的 latent 后，元素数大约减少 \(48\) 倍。实际网络 FLOPs 还取决于 U-Net/DiT 结构、通道数和 attention 分辨率，但这个比例能解释为什么 latent diffusion 比像素 diffusion 更容易落地。
...
在 diffusion 文生图里，条件通常影响噪声预测器：

\[
\epsilon_\theta(x_t,t,c)
\]

Classifier-free guidance 可以抽象为把无条件预测和有条件预测做插值：
...
其中 \(s\) 是 guidance scale。它越大，prompt 约束通常越强，但过大也更容易带来过饱和、畸变和细节不自然。
...
如果把额外结构条件记为 \(u\)，例如边缘图、深度图或姿态骨架，那么可控 diffusion 的训练目标可以抽象为：
...
\left[
\lVert \epsilon-\epsilon_\theta(x_t,t,c,u)\rVert_2^2
\right]
\]
...
T_{\mathrm{sample}}
\approx
K\cdot T_{\mathrm{denoise}}
+
T_{\mathrm{decode}}
\]

其中 \(K\) 是采样步数。少步采样、蒸馏和更快 sampler 的核心目标，就是在不明显牺牲质量的前提下降低 \(K\) 或单步去噪成本。
...
### 十二、最小代码：生成路线目标与采样成本对比

下面这个 demo 不生成真实图片，而是用小向量和假设成本把几条路线的训练目标和推理成本放到同一个视角里。

你要重点观察五件事：

1. VAE 同时看 reconstruction 和 KL。
2. GAN 同时优化 discriminator 和 generator。
3. Diffusion 训练时常见的是预测噪声的 MSE。
4. 自回归图像生成的解码调用数随 image token 数线性增长。
5. Latent diffusion 的元素数比 pixel diffusion 小很多。
...
kl_loss = 0.5 * sum(math.exp(lv) + m * m - 1.0 - lv for m, lv in zip(mu, log_var))
vae_objective = recon_loss + 0.1 * kl_loss

# 2) GAN: discriminator classifies real/fake; generator wants fake to look real.
d_real = [0.88, 0.76, 0.91]
d_fake = [0.21, 0.35, 0.18]
d_loss = mean(-safe_log(p) for p in d_real) + mean(-safe_log(1.0 - p) for p in d_fake)
g_loss = mean(-safe_log(p) for p in d_fake)

# 3) Diffusion: predict the noise added to a clean image or latent.
epsilon = [0.12, -0.35, 0.08, 0.40]
epsilon_pred = [0.10, -0.30, 0.11, 0.33]
diffusion_loss = mse(epsilon, epsilon_pred)

# 4) Autoregressive image model: high resolution means many visual tokens.
prompt_tokens = 18
image_tokens = 32 * 32
decode_calls = image_tokens
ar_token_budget = prompt_tokens + image_tokens

# 5) Latent diffusion: compare element counts and a simple denoising-work proxy.
pixel_elements = 512 * 512 * 3
latent_elements = 64 * 64 * 4
steps = 30
cfg_network_calls = 2
pixel_work = pixel_elements * steps * cfg_network_calls
latent_work = latent_elements * steps * cfg_network_calls

result = {
    "vae": {
        "recon_mse": rounded(recon_loss),
        "kl": rounded(kl_loss),
        "objective_beta_0.1": rounded(vae_objective),
    },
    "gan": {
        "discriminator_loss": rounded(d_loss),
        "generator_loss": rounded(g_loss),
    },
    "diffusion": {
        "noise_prediction_mse": rounded(diffusion_loss),
        "steps": steps,
        "cfg_network_calls": cfg_network_calls,
    },
    "autoregressive": {
    ...
    "latent_diffusion": {
        ...
        "element_reduction": rounded(pixel_elements / latent_elements, 2),
        "work_reduction": rounded(pixel_work / latent_work, 2),
    },
}
...
```text
VAE、GAN、自回归和 diffusion 不是同一种训练目标的微小变体，而是不同概率建模和优化路线。现代文生图偏向 latent diffusion，是因为它把 diffusion 的质量和稳定性，与 latent space 的低成本结合起来；自回归路线统一但 token 级串行生成成本高，GAN 采样快但对抗训练和复杂文本控制更难。
```
...
这条公式很重要：它说明训练时不需要真的从 \(x_0\) 一步步加噪到 \(x_t\)，可以随机采样 \(t\) 后一次性构造训练样本。
...
=
\mathcal{N}(\mu_\theta(x_t,t,c),\sigma_t^2I)
\]

如果模型预测噪声，就可以先反推出干净样本估计：
...
然后 sampler 再根据 \(\hat{x}_0\)、当前 \(x_t\) 和噪声 schedule 计算下一步 \(x_{t-1}\)。
...
\[
\epsilon_\theta(x_t,t,c)\rightarrow\epsilon
\]
...
一些现代实现也会预测 \(x_0\) 或 velocity \(v\)。常见的 \(v\) prediction 可以理解为在噪声和数据之间换一个更稳定的参数化：
...
面试里不必把所有参数化细节背成推导，但要知道：预测噪声、预测 \(x_0\)、预测 \(v\) 是不同输出参数化，背后仍服务于同一个逐步去噪过程。
...
一个常用观察量是 signal-to-noise ratio：
...
当 \(t\) 变大，\(\bar{\alpha}_t\) 下降，信号占比降低，噪声占比升高。不同 schedule 本质上是在安排模型应该如何从高信噪比细节修复，逐步过渡到低信噪比全局结构建模。
...
1. 条件生成：\(\epsilon_{\mathrm{cond}}\)。
2. 无条件生成：\(\epsilon_{\mathrm{uncond}}\)。
...
工程上还要注意成本：如果一次采样同时跑条件和无条件分支，粗略网络调用数会接近翻倍。
...
其中 \(S\) 是采样步数，\(C_{\mathrm{net}}\) 是一次 denoising network 前向成本。
...
抽象地说，每一步都从 \(p_\theta(x_{t-1}\mid x_t,c)\) 中采样：
...
用一个从 \(t\) 跳到 \(t'\) 的简化形式看，DDIM 可以先估计 \(\hat{x}_0\)，再构造较早时间步的样本：
...
+
\sqrt{1-\bar{\alpha}_{t'}}\epsilon_\theta(x_t,t,c)
\]

当选择更稀疏的时间步序列时，采样步数可以明显少于训练时的总扩散步数。实际 DDIM 还有随机项参数，常见确定性版本可以把随机项关掉。
...
所以线上文生图系统优化延迟时，通常会同时看 sampler 步数、单步网络成本、是否启用 CFG、VAE decode 和后处理成本。
...
如果 latent feature 大小为 \(H_z\times W_z\)，patch size 为 \(P\)，那么 DiT token 数大约是：
...
这解释了为什么高分辨率 latent、较小 patch、长条件序列都会显著影响 DiT 训练和推理成本。
...
### 十三、最小代码：加噪、预测噪声、CFG 与采样成本

下面这个 demo 用一维小向量模拟 diffusion 的关键动作：

1. 根据 beta schedule 计算 \(\alpha_t\)、\(\bar{\alpha}_t\) 和 SNR。
2. 用闭式公式从 \(x_0\) 一次性构造 \(x_t\)。
3. 计算噪声预测 MSE。
4. 根据预测噪声反推 \(\hat{x}_0\)。
5. 模拟 CFG 合成和采样成本。
...
guidance_scale = 5.0
epsilon_guided = add(epsilon_uncond, scale(guidance_scale, sub(epsilon_cond, epsilon_uncond)))
...
latent_w = 64
patch = 2
dit_tokens = latent_h * latent_w // (patch * patch)
...
result = {
    ...
    "training": {
        ...
        "x0_hat": round_vec(x0_hat),
        "x0_reconstruction_mse": round(x0_mse, 6),
    },
    "cfg": {
        "scale": guidance_scale,
        "guided_epsilon": round_vec(epsilon_guided),
...
```text
Forward process 可以闭式构造任意噪声强度的训练样本；训练目标常见形式是预测噪声；CFG 能把去噪方向往 prompt 条件方向推，但也让网络调用成本接近翻倍；DiT 的 token 数和 attention cell 会随 latent 分辨率与 patch 设置快速变化。
```
...
如果采样步数为 \(S\)，单步像素空间去噪成本粗略记为 \(C_{\mathrm{pix}}\)，总成本可以写成：
...
以 \(512\times512\times3\) 到 \(64\times64\times4\) 为例：
...
这不是完整 FLOPs 公式，但能很好解释 latent diffusion 为什么比 pixel diffusion 更容易训练、推理和部署。
...
训练 autoencoder 时，通常会综合重建、感知质量和潜变量正则：

\[
L_{\mathrm{ae}}
=
\lambda_{\mathrm{rec}}\lVert x-\hat{x}\rVert_1
+
\lambda_{\mathrm{perc}}L_{\mathrm{perc}}(x,\hat{x})
+
\lambda_{\mathrm{kl}}D_{\mathrm{KL}}(q_\psi(z\mid x)\Vert \mathcal{N}(0,I))
\]
...
\left[
\lVert \epsilon-\epsilon_\theta(z_t,t,\tau_\phi(c))\rVert_2^2
\right]
\]

其中 \(\tau_\phi(c)\) 表示 text encoder 对 caption 或 prompt 的编码。
...
其中 \(h\) 是图像或 latent 特征，\(e\) 是文本 token embedding。

Cross-attention 的计算是：
...
如果 latent token 数为 \(N_z\)，文本 token 数为 \(N_c\)，cross-attention 的 score cell 数量大约是：
...
对于一个线性层权重 \(W\in\mathbb{R}^{d_{\mathrm{out}}\times d_{\mathrm{in}}}\)，LoRA 不直接全量更新 \(W\)，而是学习一个低秩增量：
...
训练目标可以抽象成主体样本 loss 加 prior preservation loss：
...
其中 prior preservation 用来减轻模型把类别整体过拟合到少量主体图片的问题。
...
可以理解为冻结扩散模型参数，只优化新 token embedding \(e_\star\)：

\[
\min_{e_\star}
\mathbb{E}
\left[
\lVert \epsilon-\epsilon_\theta(z_t,t,\tau_\phi(c,e_\star))\rVert_2^2
\right]
\]
...
如果额外结构条件记为 \(u\)，例如边缘、深度或姿态，ControlNet 类方法可以抽象为给 denoising backbone 注入控制残差：
...
其中 \(h_l\) 是第 \(l\) 层原始特征，\(g_l(u)\) 是控制分支产生的残差，\(s_l\) 是控制强度。对应训练目标仍然可以写成条件噪声预测：
...
\mathbb{E}
\left[
\lVert \epsilon-\epsilon_\theta(z_t,t,\tau_\phi(c),u)\rVert_2^2
\right]
\]
...
如果 \(m=1\) 表示需要生成的区域，\(m=0\) 表示保留原图区域，一个简化 latent 融合可以写成：
...
### 十三、最小代码：Latent 压缩、Cross-Attention、LoRA 与 Control

下面这个 demo 不依赖深度学习框架，用小向量演示 Stable Diffusion 相关机制：

1. pixel 与 latent 元素数和压缩比。
2. latent 加噪和噪声预测 MSE。
3. tiny cross-attention 如何把文本条件注入 latent token。
4. LoRA 参数量相对全量矩阵的节省。
5. ControlNet 风格控制强度如何影响特征。
6. inpainting mask 如何只替换局部 latent。
...
alpha_bar = 0.82
zt = add(scale(math.sqrt(alpha_bar), z0), scale(math.sqrt(1.0 - alpha_bar), epsilon))
...
for query in latent_tokens:
    ...
    attention_maps.append(round_vec(weights))
    out = [sum(weights[i] * values[i][j] for i in range(len(values))) for j in range(2)]
    attended.append(round_vec(out))
...
control_scale = 0.7
controlled_feature = add(base_feature, scale(control_scale, control_residual))
...
result = {
    ...
    "controlnet": {
        "scale": control_scale,
        "controlled_feature": round_vec(controlled_feature),
    },
    "inpainting": {
        "mask": mask,
        "mixed_latent": round_vec(inpaint_latent),
    },
}
...
```text
Stable Diffusion 的工程价值来自多层组合：VAE latent 显著降低去噪空间，LDM loss 仍是预测噪声，cross-attention 注入文本条件，LoRA 用低秩增量降低微调成本，ControlNet 用额外结构残差提升控制，inpainting 用 mask 约束只修改局部区域。
```
...
def nll(probs):
    return -sum(math.log(max(p, 1e-12)) for p in probs) / len(probs)
...
\mathbb{I}
\left[
\mathrm{id}_{k}^{\mathrm{before}}
=
\mathrm{id}_{k}^{\mathrm{after}}
\right]
\cdot
\mathbb{I}
\left[
...
其中 \(N_{\mathrm{occ}}\) 是遮挡后重新出现的对象数量。真实评估会更复杂，还要看位置连续性、外观 embedding 和遮挡时长。
...
公开材料中相对确定的是：Sora 技术报告把视频生成模型放在“世界模拟器”语境下讨论，并披露了视频/图像会被表示为时空 patch、模型需要处理不同时长和分辨率；Sora 2 官方说明进一步强调了物理准确性、可控性和音视频同步等方向。除此之外，具体训练配方、数据细节、推理系统和内部安全策略都不能臆测。
...
其中 \(A_{\mathrm{cf}}\) 是反事实一致性，\(A_{\mathrm{control}}\) 是可控性，\(E_{\mathrm{phys}}\) 是物理残差。面试中要强调：这种分数只是审计框架，不是行业统一指标。
...
def audit_world(name, world):
    ...
    return {
        ...
        "passes_gate": world_score >= 0.80 and physics["floor_violations"] == 0,
    }
...
$$
H_{\mathrm{audio}}=\operatorname{Enc}_{\theta}(Z)
$$
...
$$
\operatorname{RTF}=\frac{T_{\mathrm{proc}}}{T_{\mathrm{audio}}}
$$

当 \(\operatorname{RTF}<1\) 时，系统平均处理速度快于音频播放速度，才有接近实时或批量高吞吐的基础。低延迟流式 ASR 还要看首字延迟和最终确认延迟：
...
$$
S_{\mathrm{asr}}=
w_1(1-\operatorname{WER})+
w_2(1-\operatorname{CER})+
w_3R_{\mathrm{hot}}+
w_4A_{\mathrm{bd}}(\tau)-
w_5R_{\mathrm{hall}}-
w_6\operatorname{RTF}
$$
...
def wer(ref_text, hyp_text):
    ...
    return (substitutions + deletions + insertions) / max(1, len(ref))
...
$$
\hat{d}_i=\operatorname{Dur}_{\theta}(p_i)
$$
...
$$
\operatorname{LR}(p,d)=(\underbrace{p_1,\ldots,p_1}_{d_1},\underbrace{p_2,\ldots,p_2}_{d_2},\ldots,\underbrace{p_L,\ldots,p_L}_{d_L})
$$

FastSpeech 2 的关键是把语音变化信息也作为条件输入，例如 pitch 和 energy：

$$
\hat{M}=F_{\theta}(\operatorname{LR}(p,\hat{d}),\hat{f},\hat{e},s)
$$
...
$$
\operatorname{MOS}=\frac{1}{R}\sum_{i=1}^{R}m_i
$$

对比两个系统时也常看 CMOS：

$$
\operatorname{CMOS}=\frac{1}{R}\sum_{i=1}^{R}(m_i^{A}-m_i^{B})
$$

如果要报告不确定性，可以给 MOS 加 95% 置信区间：

$$
\operatorname{CI}_{95}=1.96\frac{\sigma_m}{\sqrt{R}}
$$
...
$$
\operatorname{WER}_{\mathrm{tts}}=\frac{S_w+D_w+I_w}{N_w}
$$
...
$$
S_{\mathrm{spk}}=\operatorname{cos}(s_{\mathrm{ref}},s_{\mathrm{gen}})
$$
...
$$
\operatorname{EER}=R_{\mathrm{fa}}(\tau^\star)=R_{\mathrm{fr}}(\tau^\star)
$$
...
$$
\operatorname{RTF}_{\mathrm{tts}}=\frac{T_{\mathrm{gen}}}{T_{\mathrm{audio}}}
$$
...
$$
S_{\mathrm{tts}}=
w_1\operatorname{MOS}-
w_2\operatorname{WER}_{\mathrm{tts}}+
w_3S_{\mathrm{spk}}-
w_4E_{\mathrm{prosody}}-
w_5\operatorname{RTF}_{\mathrm{tts}}-
w_6R_{\mathrm{unauth}}+
w_7R_{\mathrm{wm}}
$$
...
### 十一、最小代码：Duration、Speaker Similarity 与安全 Gate

下面的 demo 不合成真实音频，而是把 TTS/Voice Cloning 里最容易被问到的机制串起来：duration length regulator、语速控制、pitch/energy 韵律误差、codec bitrate、MOS/CMOS、speaker similarity、ASR 可懂度和安全 gate。
...
def mean_absolute_error(gold, pred):
    return sum(abs(a - b) for a, b in zip(gold, pred)) / max(1, len(gold))
...
report = {
    ...
    "safe_gate": safety_gate(safe_request),
    "unsafe_gate": safety_gate(unsafe_request),
}

print(report)
```

这个 demo 的重点是：TTS 质量不是单一“像不像真人”。duration 决定语速和漏读风险，pitch/energy 决定韵律，speaker similarity 决定音色相似，ASR WER 粗略衡量可懂度，安全 gate 决定 voice cloning 能不能上线。
...
所以这一讲不要把“端到端语音对话”理解成简单去掉 ASR 或 TTS。更准确的理解是：系统把语音理解、对话推理、语音生成、低延迟交互和安全审计放进同一个实时闭环中优化。
...
$$
\hat{x}=\operatorname{ASR}(a^{\mathrm{in}})
$$

$$
\hat{y}=\operatorname{LLM}(\hat{x},c)
$$

$$
\hat{a}^{\mathrm{out}}=\operatorname{TTS}(\hat{y})
$$
...
$$
\hat{a}^{\mathrm{out}}=\operatorname{TTS}(\hat{y},r)
$$
...
也可以用 real-time factor 表示生成效率：

$$
\operatorname{RTF}=\frac{T_{\mathrm{compute}}}{D_{\mathrm{audio}}}
$$

若 \(\operatorname{RTF}<1\)，系统平均上能快于实时生成；若 \(\operatorname{RTF}>1\)，长语音输出就容易积压。
...
前者看多少次插话被正确处理，后者看从用户开始插话到助手真正停下来的延迟。
...
def rtf(compute_seconds, audio_seconds):
    return compute_seconds / max(1e-9, audio_seconds)
...
metrics = {
    ...
    "safe_gate": audio_safety_gate(safe_request),
    "unsafe_gate": audio_safety_gate(unsafe_request),
    "mos": 4.2,
}
weights = {
    "semantic": 0.25,
...
这里的关键不是只检测到用户说话，而是状态要一致：已经播给用户听的内容保留，尚未播放的助手音频不能继续留在对话历史里，否则模型会以为自己说过用户其实没听到的话。
...
\sum_{i=1}^{N}
\mathbf{1}[\hat{y}_i=y_i]
\mathbf{1}[\operatorname{IoU}(\hat{b}_i,b_i)\ge \tau]
$$
...
下面的 demo 把 VLM 评估拆成任务分桶、claim 支持、对象幻觉、拒答、安全拦截、工具调用和综合上线分。真实系统会用更复杂的数据结构，但这个最小版本能帮助你把指标关系讲清楚。
...
def grounding_accuracy(samples, threshold=0.5):
    ...
    hits = sum(1 for sample in scoped if box_iou(sample["pred_box"], sample["gold_box"]) >= threshold)
    return hits / max(1, len(scoped))
...
def abstention_metrics(samples):
    ...
    abstain_recall = sum(sample["abstained"] for sample in unanswerable) / max(1, len(unanswerable))
    over_refusal = sum(sample["abstained"] for sample in answerable) / max(1, len(answerable))
...
def safety_metrics(samples):
    ...
    attack_success = sum(sample["followed_attack"] for sample in attacks) / max(1, len(attacks))
    block_rate = sum(sample["blocked"] for sample in attacks) / max(1, len(attacks))
    benign_loss = sum(sample["blocked"] for sample in benign) / max(1, len(benign))
    privacy_leak = sum(sample["privacy_leak"] for sample in samples) / max(1, len(samples))
...
def tool_error_rate(samples):
    ...
    return errors / max(1, len(tool_cases))


def release_score(metrics):
    return (
...
metrics["release_score"] = round(release_score(metrics), 3)
...
这里说的 “OpenAI 风格开放题” 不是官方题库，也不是在猜闭源模型内部实现。更稳妥的依据是公开材料中的共性信号：

1. Model Spec 这类模型行为规范，提醒你把指令层级、事实性、可定制性和真实有害风险分开讨论。
2. Preparedness Framework 这类前沿安全框架，提醒你把能力评估、风险阈值、部署 gate 和部署后监控连起来。
3. GPT-4o System Card 这类系统卡，提醒你多模态系统评估不只看文本任务，还要覆盖语音、视觉、延迟、安全和部署缓解。
4. OpenAI Evals 这类公开评估工具，提醒你为具体用例写 eval，而不是只引用通用榜单。

所以本讲的目标不是复述某篇资料，而是把这些公开信号转成面试中可执行的研究判断框架。
...
其中 `g` 是目标，`u` 是用户和场景，`c` 是约束，`b` 是 baseline，`r` 是风险集合，`e` 是评估集合。候选方案可以写成：

```math
a=(D,M,T,E,S,O)
```

其中 `D` 是数据方案，`M` 是模型或系统方案，`T` 是训练/对齐方案，`E` 是 eval 设计，`S` 是安全方案，`O` 是工程落地方案。
...
-\lambda_v R_{\mathrm{vague}}
```

其中 `q_k` 覆盖问题定义、瓶颈识别、baseline、数据、训练、评估、安全和迭代；`R_risk` 是剩余风险，`C_cost` 是成本，`R_vague` 是空泛表述惩罚。

但开放题不能只看软分，还要看 hard gate：

```math
G_{\mathrm{proposal}}
=
I[A_{\mathrm{eval}}\ge \tau_e]
I[R_{\mathrm{safety}}\le \tau_s]
I[R_{\mathrm{hall}}\le \tau_h]
I[C_{\mathrm{serve}}\le \tau_c]
I[A_{\mathrm{audit}}=1]
```

如果方案没有 baseline、没有 eval、没有 safety gate，即使听起来很前沿，也不应该通过。

| 评分维度 | 满分答案特征 | 扣分信号 | 对应讲次 |
|---|---|---|---|
| 问题定义 | 目标、用户、约束和失败代价清楚 | 只复述题目，没有界定场景 | 101、119 |
| 瓶颈识别 | 能指出感知、grounding、延迟、工具或安全瓶颈 | 一概归因于模型不够大 | 103、104、109、119 |
| Baseline | 有可落地基线和升级路径 | 只有愿景，没有第一版系统 | 105、108、116 |
| 数据与训练 | 说明数据构造、负样本、偏好和对齐目标 | 只说收集更多数据 | 106、107 |
| 评估 | 有分桶指标、hard negatives、不可回答样本和线上监控 | 只引用一个榜单或总分 | 119 |
| 安全 | 明确隐私、注入、声音克隆、工具误用和审计 | 只讲效果，不讲滥用 | 117、118、119 |
| 迭代 | 短期可做，中期扩展，长期研究问题清楚 | 路线不可验证或不可交付 | 120 |
...
其中 `Q_task` 是任务质量，`A_ground` 是证据支持和 grounding，`A_tool` 是工具调用正确率，`S_safety` 是安全分，`R_hall` 是幻觉率，`L_p95` 是 p95 延迟，`C_req` 是单请求成本。
...
因此优化目标不能只惩罚幻觉，还要同时约束可回答样本上的有效回答：

```math
J_{\mathrm{vlm}}
=
A_{\mathrm{task}}
+\lambda_1A_{\mathrm{support}}
+\lambda_2R_{\mathrm{abstain}}
-\lambda_3R_{\mathrm{hall}}
-\lambda_4R_{\mathrm{over}}
```

其中 `A_support` 是 claim 级证据支持率，`R_abstain` 是不可回答样本上的正确拒答率，`R_over` 是可回答样本上的过度拒答率。开放题里说“降低幻觉”时，一定要主动补上这个 trade-off。
...
G_{\mathrm{doc}}
=
I[A_{\mathrm{field}}\ge a_{\min}]
I[A_{\mathrm{cite}}\ge c_{\min}]
I[R_{\mathrm{hall}}\le h_{\max}]
I[R_{\mathrm{priv}}\le p_{\max}]
I[A_{\mathrm{audit}}=1]
```

如果字段抽取准确率高但 citation 不可信，或者隐私泄露率不可控，这个系统仍然不适合直接上线到合同、票据或医疗文档场景。
...
-w_6R_{\mathrm{unsafe}}
-w_7R_{\mathrm{copy}}
-w_8R_{\mathrm{artifact}}
```

其中 `Q_visual` 是视觉质量，`A_prompt` 是 prompt 对齐，`D_div` 是多样性，`C_temp` 是视频时间一致性，`H_human` 是人类偏好，`R_unsafe` 是安全违规，`R_copy` 是版权或训练样本近似复现风险，`R_artifact` 是结构、手部、文字、闪烁等 artifact 风险。
...
S_{\mathrm{worldopen}}
=
w_pA_{\mathrm{persist}}
+w_oA_{\mathrm{order}}
+w_fA_{\mathrm{phys}}
+w_cA_{\mathrm{counter}}
+w_iA_{\mathrm{interact}}
-w_eE_{\mathrm{fail}}
```

如果模型只在 open-loop 视频生成上表现好，但 `A_interact` 和 `A_counter` 很低，就应该谨慎称为“有世界模拟信号”，而不是直接等同于可交互世界模型。
...
实时语音助手的 release gate 可以写成：

```math
G_{\mathrm{voiceopen}}
=
I[L_{\mathrm{ttfa},p95}\le l_{\max}]
I[\operatorname{RTF}\le 1]
I[A_{\mathrm{sem}}\ge a_{\min}]
I[A_{\mathrm{barge}}\ge b_{\min}]
I[R_{\mathrm{unsafe}}\le r_{\max}]
```

这能提醒面试官：端到端语音不是只追求模型新颖性，还要满足首音频延迟、实时率、语义正确、打断处理和安全审计。
...
多模态 Agent 的核心风险来自“错误感知 + 真实动作”。可以用：

```math
R_{\mathrm{agent}}
=
\sum_{j=1}^{J}
p_jc_jI_{\mathrm{permission},j}I_{\mathrm{action},j}
```

其中 `p_j` 是第 `j` 类错误发生概率，`c_j` 是损失，`I_permission,j` 表示权限 gate 是否放行，`I_action,j` 表示是否触发真实动作。降低风险不是只让模型“更聪明”，还要降低错误动作被执行的概率和影响成本。
...
下面的 demo 把开放研究题回答转成可执行的评分器。它不依赖真实模型，只模拟三个动作：

1. 对候选研究方向打软分。
2. 检查 baseline、eval、安全、ablation、成本和风险 gate。
3. 从通过 gate 的方案里选出优先级最高的下一步。
...
selected = next((item["name"] for item in ranking if item["gate"]), None)
...
```text
{'ranking': [{'name': 'multimodal_tool_safety_eval', 'score': 0.811, 'gate': True, 'failures': []}, {'name': 'grounded_reasoning_doc_agent', 'score': 0.776, 'gate': True, 'failures': []}, {'name': 'native_speech_dialogue', 'score': 0.644, 'gate': False, 'failures': ['safety', 'risk']}, {'name': 'bigger_model_only', 'score': 0.329, 'gate': False, 'failures': ['verifiable', 'eval', 'safety', 'risk', 'cost', 'ablation']}], 'selected': 'multimodal_tool_safety_eval'}
```

这个 demo 的面试价值是：它展示了为什么“更大模型”不等于好研究方案。真正好的开放题回答，要能说明目标、瓶颈、验证路径、安全 gate 和下一轮迭代。
...
更具体地说，第 120 讲的开放题可以反向引用前面讲次：

| 开放题方向 | 主要承接讲次 | 回答时要抓住的关键词 |
|---|---|---|
| 下一代多模态助手 | 101、104、105、118、119 | encoder、connector、LLM core、tool layer、低延迟、安全 gate |
| 降低 VLM 幻觉 | 107、109、119 | 偏好优化、grounding、claim support、hard negatives、过度拒答 |
| OCR 和文档理解 | 108、109、119 | OCR、layout、table、citation、隐私、审计 |
| 文生图和文生视频评估 | 110、111、112、113、114、115、119 | prompt 对齐、时间一致性、对象持久性、安全和版权 |
| Sora 类世界模型讨论 | 114、115 | open-loop、对象状态、物理一致性、反事实和可交互性 |
| 实时语音助手 | 116、117、118 | ASR、TTS、speech token、TTFA、barge-in、安全审计 |
| 多模态 Agent | 108、109、118、119 | 证据、工具调用、权限、prompt injection、真实动作风险 |
...
下一步，可以进入第三册第二轮精修，或对第二册做一次跨章节术语、公式和 demo 索引级收尾。
