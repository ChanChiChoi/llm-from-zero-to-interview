...
下面用纯 PyTorch 写一个最小线性回归训练过程，展示 autograd 的完整闭环。
...
x = torch.randn(128, 2)
y = x @ true_w + true_b + 0.1 * torch.randn(128)
...
for step in range(100):
    ...
    if step % 20 == 0:
        print(step, loss.item(), w.detach(), b.detach())
```

这里有几个细节：

1. `w` 和 `b` 是叶子张量，`requires_grad=True`。
2. `loss.backward()` 把梯度写入 `w.grad` 和 `b.grad`。
3. 参数更新放在 `torch.no_grad()` 里，否则更新操作也会被记录进计算图。
4. 每步更新后手动清空梯度。
5. 打印参数时使用 `detach()`，避免日志逻辑牵扯计算图。

真实训练中通常不手动更新参数，而使用 optimizer：
