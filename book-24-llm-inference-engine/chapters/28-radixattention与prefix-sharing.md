...
3. SGLang Attention Backend：<https://docs.sglang.io/docs/advanced_features/attention_backend.md>
4. SGLang Runtime 总览相关文档：<https://docs.sglang.io/llms.txt>
...
$$
X_i=(x_{i,1},x_{i,2},\ldots,x_{i,L_i})
$$

RadixAttention 在树中找到的最长 token prefix 命中长度记为 `H_i`。如果底层 KV cache 以 page / block 为粒度，page size 为 `S`，则 page 对齐后真正可复用的 token 数可以写成：

$$
H_i^{\mathrm{page}}=S\left\lfloor \frac{H_i}{S}\right\rfloor
$$

当 page size 为 1 时，`H_i^{\mathrm{page}}=H_i`，也就是 token-level prefix reuse。page size 更大时，只有完整 page 可以进入 prefix cache；这也是为什么短 prompt 或不足一页的 suffix 可能无法被缓存命中。

实际需要 prefill 的 token 数为：

$$
P_i^{\mathrm{run}}=L_i-H_i^{\mathrm{page}}
$$

总 saved prefill tokens 为：

$$
S_{\mathrm{save}}=\sum_i H_i^{\mathrm{page}}
$$

复用比例为：

$$
R_{\mathrm{reuse}}=\frac{S_{\mathrm{save}}}{\max(1,\sum_i L_i)}
$$

如果 radix tree 中每个节点 `v` 关联一段 token segment，节点占用 KV token 数记为 `m_v`，当前缓存 token 数可以写成：

$$
M_{\mathrm{cache}}=\sum_{v\in \mathcal{T}}m_v
$$

Eviction 只能选择没有 active request 引用的节点：
...
1. `G_{\mathrm{match}}`：能做 longest prefix match，并输出 hit tokens。
2. `G_{\mathrm{split}}`：新路径在旧边中间分叉时，能 split 出共享中间节点。
3. `G_{\mathrm{page}}`：能说明 page size / block size 对完整页命中的影响。
4. `G_{\mathrm{ref}}`：active request 正在用的节点不能被淘汰。
5. `G_{\mathrm{evict}}`：显存紧张时优先淘汰 ref count 为 0 的叶子节点。
6. `G_{\mathrm{schedule}}`：scheduler 使用 `P_i^{\mathrm{run}}` 而不是原始 prompt 长度估算 prefill 成本。
7. `G_{\mathrm{metric}}`：能输出 saved prefill tokens、reuse ratio、split count、cached tokens 和 eviction list。

下面这个 0 依赖 demo 用压缩 radix tree 模拟 4 条请求：第一条长 prompt 建树，第二条和第一条共享 root 后分叉，第三条在第二条边中间分叉并触发 split，第四条沿第一条路径继续变深。随后 demo pin 住第四条路径，执行 leaf LRU eviction，证明 active path 不会被淘汰。
...
class ToyRadixAttentionCache:
    def __init__(self, page_size=1, capacity_tokens=18):
    ...
    def cached_tokens(self):
        return sum(node.kv_tokens for node in self._walk())
    ...
    def lookup(self, tokens):
        ...
        while pos < len(tokens):
            ...
            if child is None:
                break
            hit = self.common_prefix_len(tokens[pos:], child.segment)
            if hit == 0:
                break
            pos += hit
            owner = child.name
            if hit < len(child.segment):
                break
            node = child
        page_hit = self.page_size * (pos // self.page_size)
        return pos, page_hit, owner

    def insert(self, tokens, name):
        node = self.root
        pos = 0
        while pos < len(tokens):
            child = node.children.get(tokens[pos])
            if child is None:
                suffix = tokens[pos:]
                node.children[suffix[0]] = RadixNode(
                    suffix, name, parent=node, kv_tokens=len(suffix)
                )
                self._touch(node.children[suffix[0]])
                return len(suffix)
            ...
            old_child = RadixNode(
                ...
                kv_tokens=len(old_suffix),
            )
            for grandchild in old_child.children.values():
                grandchild.parent = old_child

            child.segment = child.segment[:hit]
            child.name = child.name + "_shared"
            child.children = {old_suffix[0]: old_child}
            child.ref_count = 0
            child.kv_tokens = len(child.segment)
            pos += hit
            ...
            new_suffix = tokens[pos:]
            new_child = RadixNode(
                new_suffix, name, parent=child, kv_tokens=len(new_suffix)
            )
            child.children[new_suffix[0]] = new_child
            self._touch(child)
            self._touch(new_child)
            return len(new_suffix)

        self._touch(node)
        return 0

    def pin_path(self, tokens):
        node = self.root
        pos = 0
        pinned = []
        while pos < len(tokens):
    ...
    def evict_until_fit(self):
        while self.cached_tokens() > self.capacity_tokens:
            ...
            del victim.parent.children[victim.segment[0]]
            self.evicted_nodes.append(victim.name)
        return list(self.evicted_nodes)


ROOT = tuple(f"root_{i}" for i in range(8))
A = ("clause_a", "risk", "draft", "tail")
B = ("clause_b", "risk", "draft", "tail")
C = ("clause_b", "risk", "law", "tail")
D = ("deep", "followup", "json", "tail")
...
cache = ToyRadixAttentionCache(page_size=1, capacity_tokens=18)
rows = []
for name, tokens in requests:
    token_hit, page_hit, owner = cache.lookup(tokens)
    new_tokens = cache.insert(tokens, name)
    rows.append(
        {
            "name": name,
            ...
            "page_hit": page_hit,
            "hit_owner": owner,
            "run_prefill_tokens": len(tokens) - page_hit,
            "new_cached_tokens": new_tokens,
        }
    )

pinned_nodes = cache.pin_path(ROOT + A + D)
before_evict = {
    "node_count": cache.node_count(),
    "cached_tokens": cache.cached_tokens(),
    "split_count": cache.split_count,
}
evicted = cache.evict_until_fit()
after_evict = {
    "node_count": cache.node_count(),
    "cached_tokens": cache.cached_tokens(),
...
naive_prefill = sum(row["input_tokens"] for row in rows)
radix_prefill = sum(row["run_prefill_tokens"] for row in rows)
saved_prefill = naive_prefill - radix_prefill
page_examples = {
    "page_size": 4,
    "three_tokens_cacheable": 4 * (3 // 4),
    "nine_tokens_cacheable": 4 * (9 // 4),
    "ten_token_hit_reusable": 4 * (10 // 4),
}
summary = {
...
gates = {
    "longest_prefix_match": rows[2]["token_hit"] == 10,
    "split_visible": before_evict["split_count"] == 2,
    "page_boundary_checked": page_examples["ten_token_hit_reusable"] == 8,
    "ref_count_blocks_active_path": "prompt_d" not in evicted,
    "leaf_eviction_visible": evicted == ["prompt_b_tail", "prompt_c"],
    "scheduler_uses_suffix_cost": rows[3]["run_prefill_tokens"] == 4,
    "metrics_ready": summary["reuse_ratio"] > 0.5,
}
gates["radix_attention_gate"] = all(gates.values())

print("radix_attention_rows=", rows)
print("radix_attention_summary=", summary)
print("radix_attention_gates=", gates)
```

这段 demo 应该输出：

```text
radix_attention_summary= {'naive_prefill_tokens': 52, 'radix_prefill_tokens': 22, 'saved_prefill_tokens': 30, 'reuse_ratio': 0.577, 'before_evict': {'node_count': 6, 'cached_tokens': 22, 'split_count': 2}, 'after_evict': {'node_count': 4, 'cached_tokens': 18, 'evicted_nodes': ['prompt_b_tail', 'prompt_c']}, 'pinned_nodes': ['prompt_a_shared', 'prompt_a_tail', 'prompt_d'], 'page_examples': {'page_size': 4, 'three_tokens_cacheable': 0, 'nine_tokens_cacheable': 8, 'ten_token_hit_reusable': 8}}
radix_attention_gates= {'longest_prefix_match': True, 'split_visible': True, 'page_boundary_checked': True, 'ref_count_blocks_active_path': True, 'leaf_eviction_visible': True, 'scheduler_uses_suffix_cost': True, 'metrics_ready': True, 'radix_attention_gate': True}
```

读这段代码时要注意：

1. `prompt_c` 的 `token_hit=10` 说明 radix tree 能在边中间发现共享 prefix，并通过 split 把中间共享段变成节点。
2. `radix_prefill_tokens=22` 而不是 `52`，说明 scheduler 应该按未命中的 suffix 估算 prefill 成本。
3. `page_examples` 说明 page size 为 4 时，3 个 token 不足一页不能作为完整 page 命中，10 个 token 的命中也只能按 8 个完整 page tokens 复用。
4. `prompt_d` 被 pin 后没有被 eviction，说明 ref count 是正确性边界，不是性能优化选项。
5. eviction 只删掉 ref count 为 0 的叶子，避免破坏共享内部节点和 active path。
