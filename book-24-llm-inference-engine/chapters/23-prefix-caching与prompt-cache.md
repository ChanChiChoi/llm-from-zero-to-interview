...
1. vLLM Automatic Prefix Caching design：<https://docs.vllm.ai/en/latest/design/prefix_caching/>
2. vLLM Automatic Prefix Caching feature：<https://docs.vllm.ai/en/latest/features/automatic_prefix_caching/>
3. vLLM metrics：<https://docs.vllm.ai/en/latest/design/metrics/>
...
$$
T_{\mathrm{ttft},i}=T_{\mathrm{queue},i}+T_{\mathrm{tok},i}+T_{\mathrm{prefill},i}+T_{\mathrm{first},i}+T_{\mathrm{flush},i}
$$

其中 prefix cache 主要减少 `T_{\mathrm{prefill},i}`。

如果 block size 为 `S`，请求 `i` 命中的连续前缀长度为 `L_{\mathrm{hit},i}`，因为只复用 full blocks，实际可跳过的 token 数是：

$$
H_i=S\left\lfloor \frac{L_{\mathrm{hit},i}}{S}\right\rfloor
$$

所以本次需要真正 prefill 的 token 数可以写成：

$$
P_i^{\mathrm{run}}=\max(0,L_i-H_i)
$$

整体 saved prefill token ratio 可以写成：

$$
R_{\mathrm{save}}=\frac{\sum_i H_i}{\sum_i L_i}
$$
...
其中 `X_j` 是第 `j` 个 full block 的 token ids，`E_j` 是 model / tokenizer / LoRA / media hash / cache salt 等 extra hashes。prefix cache hit rate 可以写成：

$$
R_{\mathrm{hit}}=\frac{N_{\mathrm{hit}}}{N_{\mathrm{lookup}}}
$$

cached-but-free blocks 是 cache set 和 free queue 的交集：
...
1. `G_{\mathrm{hash}}`：block hash 包含 parent hash、block tokens 和 extra hashes。
2. `G_{\mathrm{full}}`：只缓存 full blocks，partial suffix 不作为 cache entry。
3. `G_{\mathrm{touch}}`：命中 cached-free block 后会从 free queue 移除并增加引用。
4. `G_{\mathrm{ref}}`：active request 的 cached block 不会被淘汰。
5. `G_{\mathrm{salt}}`：cache salt、LoRA 和 media hash 改变时不会误命中。
6. `G_{\mathrm{evict}}`：free cached block 可以按 LRU 淘汰并移除 hash entry。
7. `G_{\mathrm{metric}}`：命中率、saved prefill tokens、cached-free blocks 和 eviction count 可观测。

下面这个 0 依赖 demo 用固定大小 physical blocks 模拟 prefix cache。它故意让第一个请求留下 cached-free blocks，第二个请求命中并 touch，第三个请求因为 salt 不同 miss，第四个请求因为 LoRA 和 media hash 不同 miss，并在 free queue 中触发一次 LRU eviction。
...
class KVBlock:
    block_id: int
    tokens: tuple = ()
    parent_hash: tuple = None
    block_hash: tuple = None
    extra_hashes: tuple = ()
    ref_count: int = 0
    in_free_queue: bool = True


class ToyPrefixCacheManager:
    def __init__(self, total_blocks, block_size):
        self.block_size = block_size
        self.blocks = [KVBlock(block_id=i) for i in range(total_blocks)]
    ...
    def full_blocks(self, tokens):
        limit = len(tokens) - len(tokens) % self.block_size
        return [
            tuple(tokens[start:start + self.block_size])
            for start in range(0, limit, self.block_size)
        ]

    def make_hash(self, parent_hash, block_tokens, extra_hashes):
        return (parent_hash, tuple(block_tokens), tuple(extra_hashes))

    def touch(self, block):
        if block.ref_count == 0 and block.in_free_queue:
            self.free_queue.remove(block.block_id)
            block.in_free_queue = False
        block.ref_count += 1

    def allocate_block(self, block_tokens, parent_hash, extra_hashes):
        if not self.free_queue:
            raise RuntimeError("no free block available")
        block_id = self.free_queue.pop(0)
        block = self.blocks[block_id]
        block.in_free_queue = False
        if block.block_hash in self.cache:
            self.cache.pop(block.block_hash, None)
            self.eviction_count += 1
            self.evicted_blocks.append(block_id)

        block.tokens = tuple(block_tokens)
        block.parent_hash = parent_hash
        block.extra_hashes = tuple(extra_hashes)
        block.block_hash = self.make_hash(parent_hash, block.tokens, block.extra_hashes)
        block.ref_count = 1
        self.cache[block.block_hash] = block
        return block

    def process_request(self, request_id, tokens, extra_hashes):
        blocks = self.full_blocks(tokens)
        table = []
        hit_blocks = []
        block_hashes = []
        parent_hash = None
        miss_index = len(blocks)

        for index, block_tokens in enumerate(blocks):
            block_hash = self.make_hash(parent_hash, block_tokens, extra_hashes)
            self.prefix_lookups += 1
            cached_block = self.cache.get(block_hash)
            if cached_block is None:
                miss_index = index
                break
            self.prefix_hits += 1
            self.touch(cached_block)
            table.append(cached_block.block_id)
            hit_blocks.append(cached_block.block_id)
            block_hashes.append(block_hash)
            parent_hash = block_hash

        for block_tokens in blocks[miss_index:]:
            new_block = self.allocate_block(block_tokens, parent_hash, extra_hashes)
            table.append(new_block.block_id)
            block_hashes.append(new_block.block_hash)
            parent_hash = new_block.block_hash

        saved_tokens = len(hit_blocks) * self.block_size
        self.saved_prefill_tokens += saved_tokens
        self.request_tables[request_id] = list(table)
        return {
            ...
            "run_prefill_tokens": max(0, len(tokens) - saved_tokens),
            "cacheable_full_tokens": len(blocks) * self.block_size,
            "free_queue_after_touch": list(self.free_queue),
        }

    def release_request(self, request_id):
        released = []
        for block_id in self.request_tables.pop(request_id, []):
            block = self.blocks[block_id]
            block.ref_count -= 1
    ...
    def cached_free_blocks(self):
        return [
            ...
            if block.block_hash in self.cache and block.ref_count == 0 and block.in_free_queue
        ]

    def metrics(self):
        hit_rate = self.prefix_hits / self.prefix_lookups if self.prefix_lookups else 0.0
        return {
...
tokens_a = [1, 2, 3, 4, 5, 6, 7, 8, 99, 100]
tokens_b = [1, 2, 3, 4, 5, 6, 7, 8, 42, 43]
base_extra = ("model:v1", "tokenizer:v1", "tenant:A", "lora:none", "media:none")
other_tenant = ("model:v1", "tokenizer:v1", "tenant:B", "lora:none", "media:none")
lora_media = ("model:v1", "tokenizer:v1", "tenant:A", "lora:finance", "media:image-cat")

manager = ToyPrefixCacheManager(total_blocks=5, block_size=4)

a = manager.process_request("A", tokens_a, base_extra)
manager.release_request("A")

b = manager.process_request("B", tokens_b, base_extra)
b_free_queue_after_touch = list(b["free_queue_after_touch"])
manager.release_request("B")

c = manager.process_request("C", tokens_a, other_tenant)
manager.release_request("C")

d = manager.process_request("D", tokens_a, lora_media)
manager.release_request("D")

metrics = manager.metrics()
summary = {
    "a_table": a["table"],
    "a_cacheable_full_tokens": a["cacheable_full_tokens"],
    "b_hit_blocks": b["hit_blocks"],
    "b_saved_prefill_tokens": b["saved_prefill_tokens"],
    "b_run_prefill_tokens": b["run_prefill_tokens"],
    "b_free_queue_after_touch": b_free_queue_after_touch,
    "c_salt_hit_blocks": c["hit_blocks"],
    "d_lora_media_hit_blocks": d["hit_blocks"],
    "d_table_after_lru": d["table"],
    "evicted_blocks": list(manager.evicted_blocks),
    "cached_free_blocks": manager.cached_free_blocks(),
    "metrics": metrics,
}

gates = {
    "hash_uses_parent": a["block_hashes"][1][0] == a["block_hashes"][0],
    "full_blocks_only": a["cacheable_full_tokens"] == 8 and a["run_prefill_tokens"] == 10,
    "same_prefix_hits": b["hit_blocks"] == [0, 1] and b["saved_prefill_tokens"] == 8,
    "touch_removes_cached_free": b_free_queue_after_touch == [2, 3, 4],
    "salt_isolation": c["hit_blocks"] == [],
    "lora_media_isolation": d["hit_blocks"] == [],
    "lru_eviction_visible": manager.evicted_blocks == [0] and metrics["eviction_count"] == 1,
    "metrics_ready": metrics["prefix_hit_rate"] == 0.4 and metrics["cached_free_blocks"] == 5,
}
gates["prefix_cache_gate"] = all(gates.values())

print("prefix_cache_summary=", summary)
print("prefix_cache_gates=", gates)
```

这份输出应当体现：

1. 请求 A 只把前 8 个 token 的两个 full blocks 写入 cache，最后 2 个 partial suffix token 不作为 cache entry。
2. 请求 B 与 A 共享前两个 full blocks，命中 `[0, 1]`，saved prefill tokens 为 8，真实需要 prefill 的只剩 2 个 suffix tokens。
3. 请求 B 命中时，cached-free blocks `[0, 1]` 被 touch 出 free queue，`b_free_queue_after_touch=[2, 3, 4]`。
4. 请求 C 只改 cache salt，D 改 LoRA 和 media hash，都会 miss，说明 extra hashes 隔离生效。
5. 请求 D 分配第二个 block 时淘汰最旧的 cached-free block 0，`eviction_count=1`。
6. `prefix_cache_gate=True`，说明 hash、full block、touch、ref count、隔离、eviction 和 metrics 都能复盘。
