...
3. SGLang Sampling Parameters 文档对 `json_schema`、`regex`、`ebnf`、`structural_tag`、`choices`、`temperature`、`top_p`、`top_k`、`max_new_tokens` 和 streaming 参数的公开口径。
...
2. SGLang Structured Outputs：<https://docs.sglang.io/docs/advanced_features/structured_outputs.md>
3. SGLang Sampling Parameters：<https://docs.sglang.io/docs/references/sampling_params.md>
4. SGLang Server Arguments：<https://docs.sglang.io/docs/advanced_features/server_arguments.md>
5. SGLang Observability：<https://docs.sglang.io/docs/advanced_features/observability.md>
6. SGLang Production Metrics：<https://docs.sglang.io/docs/references/production_metrics.md>
...
Grammar state 记为 `s_t`，当前状态下合法 token 集合为：

$$
\mathcal{A}_t=g(s_t)\subseteq \mathcal{V}
$$

Grammar mask 后的 logits 可以写成：

$$
\tilde{z}_{t,j}=
\begin{cases}
z_{t,j}, & j\in \mathcal{A}_t\\
-\infty, & j\notin \mathcal{A}_t
\end{cases}
$$
...
$$
p_{t,j}=\frac{\exp(\tilde{z}_{t,j}/\tau)}{\sum_{k\in \mathcal{A}_t}\exp(\tilde{z}_{t,k}/\tau)}
$$

如果 `\mathcal{A}_t` 为空，runtime 不能继续正常采样，应返回错误、终止请求或显式 fallback，不能静默输出“看似结构化”的结果。

Grammar state 接收采样 token 后更新：
...
1. `G_{\mathrm{schema}}`：JSON schema-like grammar 能约束字段、冒号、逗号、类型和结束符。
2. `G_{\mathrm{mask}}`：sampler 最终候选集合不包含非法 token。
3. `G_{\mathrm{regex}}`：简单分类或枚举可用 regex / choices 约束。
4. `G_{\mathrm{ebnf}}`：自定义 DSL-like sequence 可用 EBNF-like grammar 约束。
5. `G_{\mathrm{tag}}`：structural tag 能约束 tool call 的 begin tag、内容和 end tag。
6. `G_{\mathrm{empty}}`：空合法 token 集合能被显式检测。
7. `G_{\mathrm{fact}}`：能说明格式合法不等于事实正确。
8. `G_{\mathrm{stream}}`：streaming 中间 chunk 不被误当成完整对象。
9. `G_{\mathrm{metric}}`：能输出 mask calls、blocked invalid tokens、parseability 和 gate。

下面这个 0 依赖 demo 用 toy logits 和 toy grammar state 模拟 constrained decoding。它故意让非法 token 的 logit 更高，证明 grammar mask 会阻止 sampler 选它；同时让 JSON 的 `population` 字段生成一个格式合法但事实错误的整数，证明 constrained decoding 不保证事实正确。

```python
import json


class TinyJSONGrammar:
    def __init__(self):
        self.stage = 0
        self.history = []
        self.valid_history = []
    ...
    def valid_next_tokens(self):
        ...
        return table[self.stage] if self.stage < len(table) else set()

    def accept(self, token):
        valid = self.valid_next_tokens()
        if token not in valid:
            raise ValueError(f"invalid token {token!r} at stage {self.stage}")
        self.valid_history.append(sorted(valid))
...
class ChoiceGrammar:
    ...
    def valid_next_tokens(self):
        return set() if self.token is not None else set(self.choices)

    def accept(self, token):
        valid = self.valid_next_tokens()
        if token not in valid:
            raise ValueError(f"invalid choice {token!r}")
        self.valid_history.append(sorted(valid))
        self.token = token
...
class SequenceGrammar:
    def __init__(self, steps):
        self.steps = [set(step) for step in steps]
        self.stage = 0
        self.history = []
        self.valid_history = []

    def valid_next_tokens(self):
        return self.steps[self.stage] if self.stage < len(self.steps) else set()
    ...
    def done(self):
        return self.stage == len(self.steps)
...
def constrained_argmax(logits, valid_tokens):
    if not valid_tokens:
        raise ValueError("empty valid token set")
    chosen = max(valid_tokens, key=lambda tok: logits.get(tok, -10**9))
    chosen_score = logits.get(chosen, -10**9)
    blocked = sorted(
        tok for tok, score in logits.items()
        if tok not in valid_tokens and score > chosen_score
    )
    return chosen, blocked


def run_constrained(grammar, logits_plan):
    tokens = []
    blocked_count = 0
    mask_calls = 0
    while not grammar.done():
        logits = logits_plan[min(mask_calls, len(logits_plan) - 1)]
        valid = grammar.valid_next_tokens()
        token, blocked = constrained_argmax(logits, valid)
    ...
    return {
        ...
        "blocked_high_score_invalid": blocked_count,
        "valid_history": getattr(grammar, "valid_history", []),
    }
...
json_logits = [
    ...
    {'"population"': 9, '"name"': 2},
    {":": 1, "=": 8},
    {"Sure": 8, '"Paris"': 3, '"London"': 2},
    {",": 1, "}": 9},
    {'"name"': 8, '"population"': 2},
    {":": 1, "=": 9},
    {"999999999": 8, "2148000": 5, '"many"': 9},
    {"}": 1, ",": 7},
]
json_result = run_constrained(TinyJSONGrammar(), json_logits)
parsed = json.loads(json_result["text"])
fact_check = parsed == {"name": "Paris", "population": 2148000}

choice_result = run_constrained(
    ChoiceGrammar(["positive", "negative", "neutral"]),
    [{"very_positive": 9, "positive": 3, "negative": 1}],
)
ebnf_result = run_constrained(
    SequenceGrammar([
        {"Paris", "London"},
        {" is "},
        {"capital"},
...
tag_result = run_constrained(
    ...
    [
        {"hello": 9, "<function=get_weather>": 1},
        {"[": 8, "{": 1},
        {'"city"': 1, '"unit"': 9},
        {":": 1, "=": 7},
        {'"Paris"': 1, '"London"': 0},
...
try:
    constrained_argmax({"anything": 1}, BrokenGrammar().valid_next_tokens())
    empty_valid_detected = False
except ValueError:
    empty_valid_detected = True

stream_chunks = [json_result["text"][:end] for end in (1, 7, 16, len(json_result["text"]))]
partial_parseable = []
for chunk in stream_chunks:
    try:
        json.loads(chunk)
        partial_parseable.append(True)
    except json.JSONDecodeError:
        partial_parseable.append(False)

summary = {
    "json_text": json_result["text"],
    "json_parseable": isinstance(parsed, dict),
    "json_fact_check": fact_check,
    "choice_text": choice_result["text"],
    "ebnf_text": ebnf_result["text"],
    "tag_text": tag_result["text"],
    "mask_calls": (
        json_result["mask_calls"]
        + choice_result["mask_calls"]
        + ebnf_result["mask_calls"]
        + tag_result["mask_calls"]
    ),
    "blocked_high_score_invalid": (
        json_result["blocked_high_score_invalid"]
        + choice_result["blocked_high_score_invalid"]
        + ebnf_result["blocked_high_score_invalid"]
        + tag_result["blocked_high_score_invalid"]
    ),
    "stream_partial_parseable": partial_parseable,
    "empty_valid_detected": empty_valid_detected,
}
gates = {
    "json_schema_mask_blocks_prefix": json_result["tokens"][0] == "{",
    "sampler_only_uses_valid_tokens": all(
        token in valid
        for result in [json_result, choice_result, ebnf_result, tag_result]
        for token, valid in zip(result["tokens"], result["valid_history"])
    ),
    "regex_choice_constrained": choice_result["text"] == "positive",
    "ebnf_sequence_constrained": ebnf_result["text"] == "Paris is capital of France",
    "structural_tag_closed": tag_result["text"].startswith("<function=")
    and tag_result["text"].endswith("</function>"),
    "empty_valid_detected": empty_valid_detected,
    "format_not_fact": summary["json_parseable"] and not summary["json_fact_check"],
    "streaming_partial_not_json": partial_parseable[:-1] == [False, False, False]
    and partial_parseable[-1] is True,
    "metrics_ready": summary["blocked_high_score_invalid"] >= 8,
}
gates["structured_generation_gate"] = all(gates.values())

print("structured_generation_summary=", summary)
print("structured_generation_gates=", gates)
```

这段 demo 应该输出：

```text
structured_generation_summary= {'json_text': '{"name":"Paris","population":999999999}', 'json_parseable': True, 'json_fact_check': False, 'choice_text': 'positive', 'ebnf_text': 'Paris is capital of France', 'tag_text': '<function=get_weather>{"city":"Paris"}</function>', 'mask_calls': 22, 'blocked_high_score_invalid': 21, 'stream_partial_parseable': [False, False, False, True], 'empty_valid_detected': True}
structured_generation_gates= {'json_schema_mask_blocks_prefix': True, 'sampler_only_uses_valid_tokens': True, 'regex_choice_constrained': True, 'ebnf_sequence_constrained': True, 'structural_tag_closed': True, 'empty_valid_detected': True, 'format_not_fact': True, 'streaming_partial_not_json': True, 'metrics_ready': True, 'structured_generation_gate': True}
```

读这段代码时要注意：

1. 第一步 logits 里 `Sure` 比 `{` 分数高，但 JSON grammar 只允许 `{`，所以输出不会带自然语言前缀。
2. `choice_text='positive'` 说明简单分类最好用 choices / regex 这类轻约束，不必上复杂 JSON schema。
3. `tag_text` 展示 structural tag 能把工具名、参数 JSON 和结束标签绑在同一个约束里。
4. `empty_valid_detected=True` 说明合法 token 集合为空时必须显式失败，不能继续采样。
5. `json_fact_check=False` 说明 constrained decoding 只保证格式和类型边界，业务事实仍要单独校验。
6. `stream_partial_parseable` 的前三项为 `False`，说明 streaming 中间 chunk 通常不是完整 JSON，程序消费要等 finish 或使用专门的增量 parser。
