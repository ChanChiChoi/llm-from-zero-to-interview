...
2. vLLM optimization and tuning：<https://docs.vllm.ai/en/latest/configuration/optimization/>
3. vLLM V1 guide：<https://docs.vllm.ai/en/latest/usage/v1_guide/>
...
可以用一个简单公式估算单机 online serving 的最小进程数。如果 API server 数量是 `N_{\mathrm{api}}`，engine core 数量是 `N_{\mathrm{core}}`，GPU worker 数量是 `N_{\mathrm{worker}}`，DP coordinator 数量是 `N_{\mathrm{dpcoord}}`，则：

$$
N_{\mathrm{proc}}=N_{\mathrm{api}}+N_{\mathrm{core}}+N_{\mathrm{worker}}+N_{\mathrm{dpcoord}}
$$

单 replica、无 DP coordinator 的常见直觉是：

$$
N_{\mathrm{proc}}\approx 2+N_{\mathrm{gpu}}
$$

这也是为什么第 22 章不只讨论 GPU，还要讨论 CPU 资源和进程边界。
...
$$
C_{\mathrm{layer}}=\frac{N_{\mathrm{api}}^{\mathrm{ok}}+N_{\mathrm{core}}^{\mathrm{ok}}+N_{\mathrm{exec}}^{\mathrm{ok}}+N_{\mathrm{worker}}^{\mathrm{ok}}+N_{\mathrm{runner}}^{\mathrm{ok}}}{5}
$$

这里的 `ok` 不是进程数量，而是职责是否清晰：API 不直接控制 GPU，engine core 不直接做 forward，executor 不做业务调度，worker 不处理 HTTP，model runner 不决定请求生命周期。
...
如果 tensor parallel 大小是 `T_{\mathrm{tp}}`，pipeline parallel 大小是 `T_{\mathrm{pp}}`，data parallel 大小是 `T_{\mathrm{dp}}`，则一个简化 worker 数估算是：

$$
N_{\mathrm{worker}}=T_{\mathrm{tp}}T_{\mathrm{pp}}T_{\mathrm{dp}}
$$

每个 data-parallel replica 内的 worker group 大小是：

$$
N_{\mathrm{group}}=T_{\mathrm{tp}}T_{\mathrm{pp}}
$$

这只是教学抽象，真实系统还会受 expert parallel、driver worker、调度拓扑和部署方式影响。
...
Model runner 的输入完整性可以写成：

$$
G_{\mathrm{runner}}=\prod_{i\in S_{\tau}}G_{\mathrm{ids},i}G_{\mathrm{pos},i}G_{\mathrm{block},i}G_{\mathrm{slot},i}G_{\mathrm{attn},i}G_{\mathrm{sample},i}
$$

只要其中一项缺失，本轮 worker input 就不应该进入 GPU forward。
...
其中 `G_{\mathrm{rank}}` 检查 worker rank / local rank / device 映射，`G_{\mathrm{merge}}` 检查多 worker 输出能被正确聚合。
...
这里 `C_{\mathrm{cpu}}` 是可用 CPU cores 的教学抽象。生产中当然还要考虑 tokenization、detokenization、multimodal processing、OS 调度、线程池、通信线程和后台 metrics，但这个式子能提醒你：GPU serving 并不是只堆 GPU。
...
`G_{\mathrm{worker}}=0` 时，engine core 应该标记相关请求 failed、清理状态并触发重启或摘流，而不是继续等待一个不会返回的 rank。
...
1. `G_{\mathrm{layer}}`：API server、engine core、executor、worker、model runner 职责边界清晰。
2. `G_{\mathrm{dispatch}}`：executor 能把 scheduler output 派发给 worker group。
3. `G_{\mathrm{runner}}`：worker input metadata 包含 input ids、positions、block table、slot mapping、attention metadata 和 sampling metadata。
4. `G_{\mathrm{rank}}`：rank、local rank、device 映射一致。
5. `G_{\mathrm{merge}}`：TP workers 的 shard 输出能正确聚合。
6. `G_{\mathrm{cpu}}`：CPU process budget 能覆盖 API、engine core 和 workers。
7. `G_{\mathrm{failure}}`：worker failure 会让相关请求 failed，而不是挂住。
8. `G_{\mathrm{metric}}`：trace 能复盘 dispatch、worker inputs、outputs、merge 和 failure。

下面这个 0 依赖 demo 模拟一个 TP=2、DP=1 的 toy executor。Engine core 产出 scheduler output，executor 把它派发到两个 GPU workers；每个 worker 校验 metadata，model runner 返回一个 shard score；executor 聚合两个 shard 得到 token；随后模拟 worker 1 failure，验证 engine core 能把请求标记为 failed。
...
class ToyModelRunner:
    ...
    def metadata_complete(self, worker_input):
        return (
            bool(worker_input.input_ids)
            and len(worker_input.input_ids) == len(worker_input.positions)
            and bool(worker_input.block_tables)
            and bool(worker_input.slot_mapping)
            and bool(worker_input.attention_metadata)
            and bool(worker_input.sampling_metadata)
        )

    def forward(self, worker_input):
        if not self.metadata_complete(worker_input):
            raise ValueError("incomplete model runner metadata")
        shard_scores = {}
        for request_id in worker_input.request_ids:
            token_sum = sum(worker_input.input_ids)
            block_sum = sum(worker_input.block_tables[request_id])
            shard_scores[request_id] = token_sum + block_sum + self.rank
        return {"rank": self.rank, "shard_scores": shard_scores}


class ToyWorker:
    def __init__(self, rank, local_rank, device):
        self.rank = rank
        self.local_rank = local_rank
        self.device = device
        self.runner = ToyModelRunner(rank)
        self.healthy = True

    def execute_model(self, worker_input):
        if not self.healthy:
            raise RuntimeError(f"worker {self.rank} failed")
        if worker_input.worker_rank != self.rank:
            raise ValueError("rank mismatch")
        if worker_input.local_rank != self.local_rank or worker_input.device != self.device:
            raise ValueError("device mapping mismatch")
        return self.runner.forward(worker_input)


class ToyExecutor:
    def __init__(self, workers, tp_size, pp_size, dp_size):
    ...
    def prepare_worker_input(self, worker, scheduler_output):
        attention_metadata = {
            "query_start_loc": [0, len(scheduler_output.input_ids)],
            "max_seq_len": max(len(v) for v in scheduler_output.block_tables.values()),
        }
        return WorkerInput(
            ...
            input_ids=list(scheduler_output.input_ids),
            positions=list(scheduler_output.positions),
            block_tables={key: list(value) for key, value in scheduler_output.block_tables.items()},
            slot_mapping={key: list(value) for key, value in scheduler_output.slot_mapping.items()},
            attention_metadata=attention_metadata,
            sampling_metadata=dict(scheduler_output.sampling_params),
        )

    def merge_worker_outputs(self, outputs):
        merged = {}
        for output in outputs:
            for request_id, score in output["shard_scores"].items():
                merged.setdefault(request_id, 0)
                merged[request_id] += score
        return {
            request_id: {
                "merged_score": score,
                "sampled_token": f"tok_{score % 7}",
            }
            for request_id, score in merged.items()
        }
    ...
    def execute_model(self, scheduler_output):
        ...
        for worker in self.workers:
            worker_input = self.prepare_worker_input(worker, scheduler_output)
            worker_inputs.append(worker_input)
            outputs.append(worker.execute_model(worker_input))
        merged = self.merge_worker_outputs(outputs)
        self.trace.append(
            {
                "request_ids": scheduler_output.request_ids,
                "worker_ranks": [worker.rank for worker in self.workers],
                "metadata_complete": all(worker.runner.metadata_complete(item) for worker, item in zip(self.workers, worker_inputs)),
                "merged": merged,
            }
        )
        return merged
...
class ToyEngineCore:
    def __init__(self, executor, api_servers, cpu_cores):
    ...
    def process_budget(self):
        return self.api_servers + 1 + len(self.executor.workers)

    def cpu_budget_ok(self):
        return self.cpu_cores >= self.process_budget()

    def make_scheduler_output(self):
        return SchedulerOutput(
            request_ids=["A", "B"],
            input_ids=[11, 12, 21],
            positions=[0, 1, 5],
            block_tables={"A": [0, 3], "B": [4]},
            slot_mapping={"A": [0, 1], "B": [2]},
            phases={"A": "prefill", "B": "decode"},
            sampling_params={"temperature": 0.0, "top_p": 1.0},
        )

    def step(self):
        scheduler_output = self.make_scheduler_output()
        for request_id in scheduler_output.request_ids:
            self.request_status[request_id] = "RUNNING"
        try:
            merged = self.executor.execute_model(scheduler_output)
            for request_id in scheduler_output.request_ids:
                self.request_status[request_id] = "DECODED"
            self.trace.append({"event": "step_ok", "merged": merged})
            return merged
        except RuntimeError as err:
            for request_id in scheduler_output.request_ids:
                self.request_status[request_id] = "FAILED"
            self.trace.append({"event": "worker_failure", "error": str(err)})
            return {}


workers = [
    ToyWorker(rank=0, local_rank=0, device="cuda:0"),
    ToyWorker(rank=1, local_rank=1, device="cuda:1"),
]
executor = ToyExecutor(workers=workers, tp_size=2, pp_size=1, dp_size=1)
engine = ToyEngineCore(executor=executor, api_servers=1, cpu_cores=4)

first_outputs = engine.step()
workers[1].healthy = False
failure_outputs = engine.step()

summary = {
    "expected_worker_count": executor.expected_worker_count(),
    "actual_worker_count": len(workers),
    "process_budget": engine.process_budget(),
    "cpu_budget_ok": engine.cpu_budget_ok(),
    "first_outputs": first_outputs,
    "failure_outputs": failure_outputs,
    "request_status": dict(engine.request_status),
    "executor_trace": executor.trace,
    "engine_trace": engine.trace,
}

gates = {
    "layer_boundary_clear": engine.api_servers == 1 and executor.expected_worker_count() == len(workers),
    "dispatch_to_all_workers": executor.trace[0]["worker_ranks"] == [0, 1],
    "runner_metadata_complete": executor.trace[0]["metadata_complete"],
    "rank_device_mapping_ok": all(worker.rank == worker.local_rank for worker in workers),
    "tp_merge_visible": first_outputs["A"]["merged_score"] == 95 and first_outputs["B"]["sampled_token"] == "tok_6",
    "cpu_process_budget_ok": engine.cpu_budget_ok(),
    "worker_failure_marks_failed": failure_outputs == {} and set(engine.request_status.values()) == {"FAILED"},
    "metrics_trace_ready": len(executor.trace) == 1 and len(engine.trace) == 2,
}
gates["executor_architecture_gate"] = all(gates.values())

print("executor_architecture_summary=", summary)
print("executor_architecture_gates=", gates)
```

这份输出应当体现：

1. `expected_worker_count=2` 和 `actual_worker_count=2`，说明 TP=2 的 worker group 完整。
2. 第一次执行同时派发到 rank 0 和 rank 1，metadata 完整，TP shard 输出被 merge。
3. `process_budget=4` 且 `cpu_budget_ok=True`，说明 1 API server + 1 engine core + 2 workers 的 CPU 预算足够。
4. 第二次执行模拟 worker failure，相关请求被标记为 `FAILED`，不会继续挂起。
5. `executor_architecture_gate=True`，说明 layer boundary、dispatch、runner metadata、rank mapping、merge、CPU budget、failure 和 trace 都可复盘。
