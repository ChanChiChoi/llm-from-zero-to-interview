...
FlashInfer 可以作为这一层的代表来理解。它不是 vLLM、SGLang、TensorRT-LLM 这种完整 serving engine，而是服务 attention、sampling、KV cache 等推理算子的 kernel / backend 组件。公开论文把它定位为 customizable attention engine for LLM serving，重点是适配不同 KV cache layout、batching 形态和 attention 场景。

面试中可以用三层来区分：

```text
kernel / backend：FlashAttention、FlashInfer，优化单步算子和 metadata contract
runtime / engine：vLLM、SGLang，管理请求、scheduler、KV、streaming、metrics
platform：router、quota、observability、canary、cost、SLO、incident response
```

因此，FlashInfer 的价值不是“替代 serving engine”，而是让 engine 在特定 workload 下有更好的 attention backend 选择。真正上线时还要看框架集成、fallback、CUDAGraph 约束、KV layout 兼容、长上下文、并行生成和可观测指标。
...
其中 `c_j` 是能力类别，`E_j` 是证据集合，`X_j` 是实验集合，`D_j` 是依赖的前置能力。

能力覆盖率：

```math
C_{\mathrm{cap}}=\frac{|R_{\mathrm{seen}}\cap R_{\mathrm{req}}|}{\max(1,|R_{\mathrm{req}}|)}
```
...
这组公式强调：读 mini-sglang 不是重复“有 scheduler、有 KV cache”的通用 serving engine 学习，而是要证明 SGLang-style runtime 的特色能被实验观察。
...
class ToyMiniSGLangLearningAuditor:
    def __init__(self):
        self.required_capabilities = [
            ...
            "tensor_parallel",
            "kernel_backend",
            "structured_generation_boundary",
            "tool_agent_boundary",
            "production_gap_map",
        ]
        self.required_experiments = [
            "shared_prefix_radix_split",
            "prefix_hit_metrics",
            "chunk_size_ablation",
            "overlap_ablation",
            "online_streaming_trace",
            "multi_turn_shell_reuse",
            "structured_mask_design",
            "tool_call_parser_trace",
            "abort_cleanup_check",
            "sglang_gap_report",
        ]
    ...
    def audit(self, capabilities):
        completed = set()
        rows = []
        for capability in capabilities:
            deps_ready = all(dep in completed for dep in capability.depends_on)
            row = {
                "capability": capability.name,
                "category": capability.category,
                "evidence": list(capability.evidence),
                "experiments": list(capability.experiments),
                "deps_ready": deps_ready,
            }
            rows.append(row)
            if deps_ready:
                completed.add(capability.name)
        names = [capability.name for capability in capabilities]
        evidence = sorted({item for capability in capabilities for item in capability.evidence})
        experiments = sorted({item for capability in capabilities for item in capability.experiments})
        summary = {
            "capabilities": len(rows),
            "ready_capabilities": sum(row["deps_ready"] for row in rows),
            "capability_coverage": self.coverage(names, self.required_capabilities),
            "experiment_coverage": self.coverage(experiments, self.required_experiments),
            "evidence_count": len(evidence),
            "runtime_specific": sum(
                capability.category == "sglang_runtime" for capability in capabilities
            ),
            "dependency_order_ok": all(row["deps_ready"] for row in rows),
            "missing_capabilities": sorted(set(self.required_capabilities) - set(names)),
            "missing_experiments": sorted(set(self.required_experiments) - set(experiments)),
        }
        gates = {
            "radix_cache_visible": (
                "radix_cache" in names and "shared_prefix_radix_split" in experiments
            ),
            "chunked_prefill_visible": (
                "chunked_prefill" in names and "chunk_size_ablation" in experiments
            ),
            "overlap_scheduling_visible": (
                "overlap_scheduling" in names and "overlap_ablation" in experiments
            ),
            "online_serving_visible": (
                "online_serving" in names and "online_streaming_trace" in experiments
            ),
            "structured_tool_boundaries_visible": all(
                name in names for name in ["structured_generation_boundary", "tool_agent_boundary"]
            ),
            "abort_cleanup_visible": "abort_cleanup_check" in experiments,
            "production_gap_visible": (
                "production_gap_map" in names and "sglang_gap_report" in experiments
            ),
            "dependency_order_ok": summary["dependency_order_ok"],
        }
        gates["mini_sglang_learning_gate"] = all(gates.values())
...
capabilities = [
    ...
    RuntimeCapability(
        "radix_cache",
        "sglang_runtime",
        ("longest_prefix_match", "tree_split", "kv_ownership"),
        ("shared_prefix_radix_split", "prefix_hit_metrics"),
        ("runtime_request_state",),
    ),
    RuntimeCapability(
        "chunked_prefill",
        "scheduler",
        ("suffix_chunk", "position_continuity", "partial_kv_cleanup"),
        ("chunk_size_ablation",),
        ("radix_cache",),
    ),
    RuntimeCapability(
        "overlap_scheduling",
        "scheduler",
        ("cpu_schedule_next", "gpu_forward_current", "metadata_ready"),
        ("overlap_ablation",),
        ("chunked_prefill",),
    ),
    RuntimeCapability(
        "online_serving",
        "serving_api",
        ("openai_api", "streaming", "client_abort"),
        ("online_streaming_trace", "abort_cleanup_check"),
        ("runtime_request_state",),
    ),
    RuntimeCapability(
        "interactive_shell",
        "serving_api",
        ("multi_turn_history", "reset_state", "stream_print"),
        ("multi_turn_shell_reuse",),
        ("radix_cache",),
    ),
    RuntimeCapability(
        "tensor_parallel",
        "parallelism",
        ("tp_rank", "weight_shard", "head_partition"),
        ("sglang_gap_report",),
        ("runtime_request_state",),
    ),
    RuntimeCapability(
        "kernel_backend",
        "kernel",
        ("attention_backend", "kv_layout_contract", "fallback_path"),
        ("sglang_gap_report",),
        ("chunked_prefill",),
    ),
    RuntimeCapability(
        "structured_generation_boundary",
        "sglang_runtime",
        ("grammar_state", "valid_token_mask", "sampler_boundary"),
        ("structured_mask_design",),
        ("runtime_request_state",),
    ),
    RuntimeCapability(
        "tool_agent_boundary",
        "sglang_runtime",
        ("tool_schema_prompt", "streaming_parser", "app_layer_tool_exec"),
        ("tool_call_parser_trace",),
        ("structured_generation_boundary",),
    ),
    RuntimeCapability(
        "production_gap_map",
        "readiness",
        ("distributed_gap", "metrics_gap", "fault_recovery_gap"),
        ("sglang_gap_report",),
        ("online_serving", "tool_agent_boundary"),
    ),
]
rows, summary, gates = ToyMiniSGLangLearningAuditor().audit(capabilities)
print("mini_sglang_learning_summary=", summary)
print("mini_sglang_learning_gates=", gates)
```

一次运行的核心输出类似：

```text
mini_sglang_learning_summary= {'capabilities': 11, 'ready_capabilities': 11, 'capability_coverage': 1.0, 'experiment_coverage': 1.0, 'evidence_count': 32, 'runtime_specific': 4, 'dependency_order_ok': True, 'missing_capabilities': [], 'missing_experiments': []}
mini_sglang_learning_gates= {'radix_cache_visible': True, 'chunked_prefill_visible': True, 'overlap_scheduling_visible': True, 'online_serving_visible': True, 'structured_tool_boundaries_visible': True, 'abort_cleanup_visible': True, 'production_gap_visible': True, 'dependency_order_ok': True, 'mini_sglang_learning_gate': True}
```

这个 demo 的重点是把 mini-sglang 的学习对象和前两个项目区分开：

1. `radix_cache_visible=True`：必须能观察最长前缀匹配、tree split 和 KV ownership。
2. `chunked_prefill_visible=True`：必须能做 chunk size ablation，并检查 position / partial KV。
3. `overlap_scheduling_visible=True`：必须能对比 CPU 调度和 GPU forward 是否重叠。
4. `online_serving_visible=True`：必须能观察 streaming、client abort 和请求排队。
