...
其中 `p_j` 是迁移阶段，`C_j` 是该 step 交付的 capability 集合，`D_j` 是前置依赖，`r_j` 是风险等级。

接口 readiness 可以用一个最小覆盖率表达：

```math
R_{\mathrm{iface}}=\frac{I_{\mathrm{stage}}+I_{\mathrm{runner}}+I_{\mathrm{kv}}}{3}
```

其中 `I_{\mathrm{stage}}` 表示 request stage 已显式化，`I_{\mathrm{runner}}` 表示 model runner 已拆出 prefill / decode 接口，`I_{\mathrm{kv}}` 表示 KV metadata contract 已稳定。

PD path 的队列压力不能只看一个 waiting queue，而要看多段 backlog：
...
1. `explicit_request_stage`：单进程内先把 request stage 显式化。
2. `model_runner_interface_split`：把 model runner 拆出 prefill / decode 接口。
3. `kv_metadata_contract`：稳定 KV metadata 和 KV manager 边界。
4. `scheduler_split`：先在逻辑上拆 prefill scheduler 和 decode scheduler。
5. `router_state_machine`：让 router 接管 request lifecycle。
...
class ToyPDMigrationAuditor:
    def __init__(self):
        self.required_capabilities = [
            "explicit_stage",
            "runner_prefill_decode",
            "kv_metadata",
            "prefill_scheduler",
            "decode_scheduler",
    ...
    def audit(self, steps):
        ...
        for index, step in enumerate(steps):
            deps_ready = all(dep in completed for dep in step.dependencies)
            ready = deps_ready and step.risk <= 2
            row = {
                "step": step.name,
                "phase": step.phase,
        ...
        capability_ready = {
            capability: any(
                row["ready"] and capability in row["capabilities"]
                for row in rows
            )
            for capability in self.required_capabilities
        }
        same_node_before_cross_node = (
            positions["same_node_pd_prototype"] < positions["cross_node_pd_readiness"]
        )
        summary = {
            "steps": len(rows),
            "ready_steps": sum(row["ready"] for row in rows),
            "interfaces_ready": sum(
                capability_ready[name]
                for name in ["explicit_stage", "runner_prefill_decode", "kv_metadata"]
            ),
            "state_machine_ready": capability_ready["router_state_machine"],
            "cleanup_paths": sum(
                capability_ready[name]
                for name in ["transfer_cleanup", "client_cancel"]
            ),
            "fallback_ready": capability_ready["fallback"],
            "same_node_before_cross_node": same_node_before_cross_node,
        }
        gates = {
            "request_stage_explicit": capability_ready["explicit_stage"],
            "runner_interface_split": capability_ready["runner_prefill_decode"],
            "kv_metadata_contract": capability_ready["kv_metadata"],
            "scheduler_split_ready": (
                capability_ready["prefill_scheduler"] and capability_ready["decode_scheduler"]
            ),
            "router_state_machine_ready": capability_ready["router_state_machine"],
            "same_node_pd_first": capability_ready["same_node_pd"] and same_node_before_cross_node,
            "cleanup_and_cancel_ready": (
                capability_ready["transfer_cleanup"] and capability_ready["client_cancel"]
            ),
            "observability_fallback_ready": (
                capability_ready["metrics"]
                and capability_ready["backpressure"]
                and capability_ready["fallback"]
            ),
        }
        gates["pd_migration_gate"] = all(gates.values()) and summary["ready_steps"] == len(rows)
        return rows, summary, gates


steps = [
    MigrationStep("explicit_request_stage", "single_process_refactor", ("explicit_stage",), (), 1),
    MigrationStep(
        "model_runner_interface_split",
        "single_process_refactor",
        ("runner_prefill_decode",),
        ("explicit_request_stage",),
        1,
    ),
    MigrationStep(
        "kv_metadata_contract",
        "single_process_refactor",
        ("kv_metadata", "kv_manager_split"),
        ("model_runner_interface_split",),
        1,
    ),
    MigrationStep(
        "scheduler_split",
        "logical_pd",
        ("prefill_scheduler", "decode_scheduler"),
        ("explicit_request_stage",),
        1,
    ),
    MigrationStep(
        "router_state_machine",
        "logical_pd",
        ("router_state_machine", "request_store"),
        ("kv_metadata_contract", "scheduler_split"),
        1,
    ),
    MigrationStep(
        "same_node_pd_prototype",
        "same_node_pd",
        ("same_node_pd", "decode_reservation", "kv_transfer"),
        ("router_state_machine",),
        2,
    ),
    MigrationStep(
        "cleanup_cancel_paths",
        "same_node_pd",
        ("transfer_cleanup", "client_cancel"),
        ("same_node_pd_prototype",),
        1,
    ),
    MigrationStep(
        "metrics_backpressure_fallback",
        "same_node_pd",
        ("metrics", "backpressure", "fallback"),
        ("cleanup_cancel_paths",),
        1,
    ),
    MigrationStep(
        "cross_node_pd_readiness",
        "cross_node_pd",
        ("topology_gate", "cross_node_pd"),
        ("metrics_backpressure_fallback",),
        2,
    ),
]
rows, summary, gates = ToyPDMigrationAuditor().audit(steps)
print("pd_migration_rows=", rows)
print("pd_migration_summary=", summary)
print("pd_migration_gates=", gates)
```

一次运行的核心输出类似：

```text
pd_migration_summary= {'steps': 9, 'ready_steps': 9, 'interfaces_ready': 3, 'state_machine_ready': True, 'cleanup_paths': 2, 'fallback_ready': True, 'same_node_before_cross_node': True}
pd_migration_gates= {'request_stage_explicit': True, 'runner_interface_split': True, 'kv_metadata_contract': True, 'scheduler_split_ready': True, 'router_state_machine_ready': True, 'same_node_pd_first': True, 'cleanup_and_cancel_ready': True, 'observability_fallback_ready': True, 'pd_migration_gate': True}
```

这个 demo 要表达的不是“9 个 step 全打勾就能上线”，而是一个顺序约束：

1. 先把单机 engine 内部的 request stage、runner interface 和 KV metadata 稳住。
2. 再把 scheduler 和 router 逻辑拆开。
