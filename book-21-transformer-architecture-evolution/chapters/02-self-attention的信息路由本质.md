...
3. 补一个 0 依赖 Python demo，审计 attention 路由命中、causal mask 未来泄漏、softmax 行归一化、缩放后的 entropy 变化、lost-in-the-middle 和解释风险。
...
```math
S=QK^\top,\qquad S\in\mathbb{R}^{N\times N}
```
...
其中 $B$ 是 batch size，$N$ 是 sequence length，$D$ 是 hidden size，$H$ 是 head 数，$D_h$ 是 head dim。
...
```math
X^{(\ell)}=[x_1^{(\ell)},x_2^{(\ell)},\ldots,x_N^{(\ell)}]
```

经过 attention 后：

```math
\tilde{x}_i^{(\ell)}=\sum_{j=1}^{N}a_{ij}^{(\ell)}v_j^{(\ell)}
```
...
## 2.16 Self-Attention 信息路由审计指标

为了把 Self-Attention 从公式讲成可检查的信息路由系统，可以把一个 attention routing case 抽象成：

```math
r_i=(q_i,K_i,V_i,M_i,y_i,z_i,e_i)
```

其中 $q_i$ 是当前 Query，$K_i,V_i$ 是候选 token 的 Key / Value 集合，$M_i$ 是可见性 mask，$y_i$ 是期望路由目标，$z_i$ 是缩放、softmax 和输出聚合后的记录，$e_i$ 是人工或任务层面的证据标签。

路由命中率可以写成：

```math
C_{\mathrm{route}}=\frac{1}{N}\sum_{i=1}^{N}\mathbb{1}[\arg\max_j a_{ij}=y_i]
```

causal mask 的未来泄漏率：

```math
R_{\mathrm{future}}=\frac{1}{N}\sum_{i=1}^{N}\mathbb{1}\left[\sum_{j\in F_i}a_{ij}>\epsilon\right]
```

softmax 行归一化检查：

```math
C_{\mathrm{row}}=\mathbb{1}\left[\forall i,\left|\sum_{j=1}^{N}a_{ij}-1\right|\le\epsilon\right]
```

缩放带来的稳定性可以用 entropy 变化粗略观察：

```math
\Delta H=\frac{1}{N}\sum_{i=1}^{N}\left(H(A_i^{\mathrm{scaled}})-H(A_i^{\mathrm{raw}})\right)
```

解释风险率用于提醒不要把 attention weight 直接当成最终因果解释：
...
这组指标的重点不是替代真实模型评估，而是帮助面试表达更严谨：attention 要同时检查路由是否命中、mask 是否正确、softmax 是否归一化、缩放是否缓解过尖分布，以及解释边界是否被夸大。

### 2.16.1 最小可运行 Attention 路由审计 demo

下面的 demo 不依赖 PyTorch，只用 Python 标准库模拟 4 个 routing case：代词指代、causal mask 阻断未来 token、lost-in-the-middle 路由失败，以及 attention weight 不足以作为因果解释的样本。
...
cases = [
    {
        ...
        "expected": "animal",
        "visible": ["animal", "street", "it", "tired"],
        "mask_future": False,
        "interpretation_support": True,
    },
    {
        "id": "causal_mask_blocks_future",
        "query": [0.0, 1.0, 0.1],
        "keys": {
            "x0": [0.1, 0.9, 0.0],
            "x1": [0.0, 1.1, 0.0],
            "future_answer": [0.0, 1.4, 0.2],
            "future_noise": [1.0, 0.1, 0.0],
        },
        "expected": "x1",
        "visible": ["x0", "x1"],
        "mask_future": True,
    ...
    {
        "id": "lost_middle_route_miss",
        "query": [1.0, 0.0, 0.0],
        "keys": {
            "doc_start": [0.9, 0.0, 0.0],
            "doc_middle_answer": [0.75, 0.0, 0.0],
            "doc_end": [0.95, 0.0, 0.0],
        },
        "expected": "doc_middle_answer",
        "visible": ["doc_start", "doc_middle_answer", "doc_end"],
        "mask_future": False,
        "interpretation_support": True,
    },
    {
        "id": "attention_weight_not_causal_explanation",
        "query": [0.4, 0.8, 0.0],
        "keys": {
            "format_token": [0.2, 1.0, 0.0],
            "evidence_token": [1.0, 0.2, 0.0],
            "distractor": [0.3, 0.7, 0.0],
        },
        "expected": "evidence_token",
        "visible": ["format_token", "evidence_token", "distractor"],
        "mask_future": False,
...
def attend(case, scaled):
    labels = list(case["keys"])
    d_k = len(case["query"])
    scores = []
    for label in labels:
        score = dot(case["query"], case["keys"][label])
        if scaled:
            score /= math.sqrt(d_k)
        if label not in case["visible"]:
            score = -1e9
        scores.append(score)
...
records = {}
for case in cases:
    labels, raw_weights = attend(case, scaled=False)
    _, scaled_weights = attend(case, scaled=True)
    top_label = labels[max(range(len(labels)), key=lambda i: scaled_weights[i])]
    future_mass = sum(
        weight for label, weight in zip(labels, scaled_weights)
        if label not in case["visible"]
    )
    records[case["id"]] = {
        "top": top_label,
        "expected": case["expected"],
        "route_hit": top_label == case["expected"],
        "future_mass": round(future_mass, 6),
        "raw_entropy": round(entropy(raw_weights), 3),
        "scaled_entropy": round(entropy(scaled_weights), 3),
        "row_sum": round(sum(scaled_weights), 6),
        "interpretation_support": case["interpretation_support"],
    }

metrics = {
    "route_hit_rate": round(sum(r["route_hit"] for r in records.values()) / len(records), 3),
    "future_leak_rate": round(sum(r["future_mass"] > 1e-6 for r in records.values()) / len(records), 3),
    "row_sum_ok": all(abs(r["row_sum"] - 1.0) < 1e-6 for r in records.values()),
    "avg_entropy_gain": round(
        sum(r["scaled_entropy"] - r["raw_entropy"] for r in records.values()) / len(records), 3
    ),
    "interpretation_risk_rate": round(
        sum(not r["interpretation_support"] for r in records.values()) / len(records), 3
    ),
}
failed_cases = [case_id for case_id, r in records.items() if not r["route_hit"] or r["future_mass"] > 1e-6]
failed_gates = []
if metrics["route_hit_rate"] < 0.75:
    failed_gates.append("route_hit")
if metrics["future_leak_rate"] > 0.0:
    failed_gates.append("causal_mask")
if not metrics["row_sum_ok"]:
    failed_gates.append("softmax_row_sum")
if metrics["avg_entropy_gain"] <= 0.0:
    failed_gates.append("scale_stability")
if metrics["interpretation_risk_rate"] > 0.0:
...
print(f"attention_routing_gate_pass={not failed_gates}")
```

运行结果应类似：

```text
records={'pronoun_coref_scaled': {'top': 'animal', 'expected': 'animal', 'route_hit': True, 'future_mass': 0, 'raw_entropy': 1.329, 'scaled_entropy': 1.364, 'row_sum': 1.0, 'interpretation_support': True}, 'causal_mask_blocks_future': {'top': 'x1', 'expected': 'x1', 'route_hit': True, 'future_mass': 0.0, 'raw_entropy': 0.688, 'scaled_entropy': 0.691, 'row_sum': 1.0, 'interpretation_support': True}, 'lost_middle_route_miss': {'top': 'doc_end', 'expected': 'doc_middle_answer', 'route_hit': False, 'future_mass': 0, 'raw_entropy': 1.095, 'scaled_entropy': 1.097, 'row_sum': 1.0, 'interpretation_support': True}, 'attention_weight_not_causal_explanation': {'top': 'format_token', 'expected': 'evidence_token', 'route_hit': False, 'future_mass': 0, 'raw_entropy': 1.09, 'scaled_entropy': 1.096, 'row_sum': 1.0, 'interpretation_support': False}}
metrics={'route_hit_rate': 0.5, 'future_leak_rate': 0.0, 'row_sum_ok': True, 'avg_entropy_gain': 0.012, 'interpretation_risk_rate': 0.25}
failed_cases=['lost_middle_route_miss', 'attention_weight_not_causal_explanation']
failed_gates=['route_hit', 'interpretation_boundary']
attention_routing_gate_pass=False
```

这段 demo 的重点是：mask 和 softmax 可以完全正确，但路由目标仍可能失败；attention map 可以提供线索，但不能自动变成因果解释。面试中如果能讲清楚这两点，就比单纯背公式更接近工程真实问题。
...
8. 用本章的 0 依赖 demo 增加一个样本，要求 mask 正确但 route miss，并解释它对应真实长上下文中的什么问题。
