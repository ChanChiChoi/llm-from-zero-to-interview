...
```text
softmax([2.0, 1.0, -inf]) = [0.731, 0.269, 0.000]
```
...
```text
第 i 个位置只能看第 0 到第 i 个位置，不能看 i 之后的未来 token。
```
...
对于长度 `n`，attention score 矩阵大小是 `n x n`，计算和显存都是 O(n^2)。
...
```text
第 i 个 token 只能看 [i - w + 1, ..., i] 范围内的历史 token。
```
...
## 8.22 面向专家：Bidirectional 不是不能生成，Causal 也不是不能理解
