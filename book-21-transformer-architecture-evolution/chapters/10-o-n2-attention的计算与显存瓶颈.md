...
7. Kwon et al., 2023, *PagedAttention/vLLM*。讨论 LLM serving 中 KV cache 动态增长和内存管理问题。它更偏第 11 章，但本章会用来区分 FlashAttention 和 PagedAttention。
...
```text
attention score matrix = batch * heads * seq_len * seq_len
```
...
heads = 32
seq_len = 32768
dtype = bf16/fp16, 每个元素 2 bytes
```

一个 attention score/probability 张量大小约为：

```text
1 * 32 * 32768 * 32768 * 2 bytes
≈ 68.7 GB
```

这只是一个张量，还没算 Q/K/V、其他层、梯度、optimizer state。
