...
```text
直接用 text[len(prompt):] 对某些 tokenizer 不一定完全可靠。
教学项目可以先这么写。
...
def build_rag_prompt(query, retrieved_chunks):
    context = "\n\n".join([f"[{i+1}] {x['text']}" for i, x in enumerate(retrieved_chunks)])
...
def build_json_cited_prompt(query, referenced_chunks):
    context = "\n\n".join(
        [f"[{x['ref_id']}] {x['text']}" for x in referenced_chunks]
    )
