...
6. 强规则：后续进行任何正式章节扩写、第二版重写、专题大纲设计、关键知识点补全、面试题补充、练习设计和项目路线更新前，都必须先联网搜索相关资料，把获取到的资料作为辅助材料扩展知识广度和深度，再结合本项目既有结构和写作目标进行写作；不能只依赖已有记忆或旧资料直接定稿。
7. 联网检索不是机械摘抄，而是用于发现遗漏视角、最新进展、官方定义、工程实现、论文结论、失败案例和真实系统 trade-off。写作时必须把网络资料消化为适合小白理解、适合面试表达、适合工程落地的内容。
8. 对优化器、DPO/RLHF/RLAIF、reasoning、Agent、推理引擎、主流大模型架构创新等快速变化主题，写作前要优先联网核验最新论文、官方技术报告、项目文档和高可信技术博客，避免只依赖已有记忆或旧资料。
9. 对新近模型、闭源模型和社区传闻，要区分“官方披露”“论文复现”“社区推测”和“待核验”，不能把未确认信息写成确定事实。
10. 联网资料优先级应为官方论文、官方技术报告、官方 GitHub 仓库、官方文档、主流框架文档、标准或协议原文、知名研究者或工程团队博客、可信社区讨论。低可信来源只能作为线索，不能直接当作结论。
11. 使用网络资料时必须考虑可信度和投毒风险。对来源不明、营销化、标题党、无法复现、与官方资料冲突或只有单一社区帖子支持的信息，应降级为“待核验”或不写入正文。重要结论应尽量交叉验证，优先以官方材料、论文、源码和可复现实验为准。
...
1. 24 本主书正文第一版已完成，后续以第二轮全系列精修为主。
2. 第二轮精修已持续推进；其中第二十四册 `book-24-llm-inference-engine/` 当前落盘第 1-60 章已完成第二轮公式、demo、资料校准和纵向文件同步。
3. 当前优先继续推进未收口章节的公式、变量说明、GitHub Markdown 渲染问题和小白友好解释；已完成第二轮的专题册进入总体验收和索引一致性检查。
4. 对适合代码辅助理解的重点知识点，同步补充 demo 级 Python 示例，并尽量验证可运行性。
5. 对第一册到第二十四册中的重要概念，同步在第四册补充或回填百科条目。
6. 对涉及代码、项目或系统设计的章节，同步在第三册、专题书或 `PROJECTS.md` 中补充实战版本。
7. 每次新增内容都要检查是否符合本文件中的写作规范。
8. 每次扩写正式章节后，要按 `WRITING_PLAN.md` 和 `SECOND_PASS_REVIEW_PLAN.md` 的质量自查标准检查是否合格。
9. 每次扩写核心章节后，要同步维护百科、题库、练习、术语、项目路线、论文路线和知识图谱中相关内容。
10. 每次扩写、重写或校订正式内容前，都必须先联网搜索相关资料；正文中涉及具体模型、算法版本、系统特性或工程实践时，应标注来源边界和更新时间语境。
11. 每次设计新书大纲、专题目录或关键章节时，必须主动搜索权威资料补充遗漏知识点，避免目录和正文只反映旧知识结构；但应控制搜索粒度，优先查会影响知识结构和结论准确性的资料，而不是机械地逐句检索。
12. 对网络资料要做可信度筛选，不能全信。若资料来源可疑、证据不足或存在冲突，应在正文中谨慎表达，必要时标注“待核验”“社区说法”或直接舍弃。
13. 每次联网后，要把资料转化为本项目自己的知识结构：补充历史背景、方法谱系、优缺点、适用边界、工程 trade-off、面试追问和练习，而不是堆链接或复制原文。
14. 每完成一轮正文精修，必须同步检查并更新当前目录下所有根目录总控文件，包括 `README.md`、`BOOK_SERIES.md`、`ROADMAP.md`、`PROJECTS.md`、`PAPERS.md`、`INTERVIEW_BANK.md`、`EXERCISES.md`、`GLOSSARY_EN_ZH.md`、`PROGRESS.md` 等，避免完成度、学习路线、题库、练习、术语和后续计划互相脱节。
