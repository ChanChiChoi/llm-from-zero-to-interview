...
40ZZZE. 大模型训练平台的 checkpoint 策略怎么设计和审计？
       考察点：RPO / RTO、save trigger policy、resume / eval / release checkpoint、async snapshot、manifest commit、shard metadata、restore selection、preemption coupling、eval best、release readiness、retention lifecycle、storage budget、monitoring alerts、restore drill、permission protection、checkpoint strategy gate。
       回答框架：先说明 checkpoint 策略不是固定“每 1000 step 保存一次”，而是用可接受的 I/O 和存储成本控制失败后的最大丢失进度，并支撑恢复、评估、抢占、回滚和发布。第一层先定 RPO/RTO：RPO 决定最多能回退多少训练时间或 GPU-hours，RTO 决定从失败到恢复训练多久；保存触发可以同时支持 step interval、time interval、milestone、preemption notice 和发布候选。第二层按用途分 checkpoint：resume checkpoint 包含模型、optimizer、scheduler、RNG、dataloader 和分布式状态；eval checkpoint 更轻，绑定 eval config 和报告；release checkpoint 要包含 tokenizer、config、generation config、eval report、metadata 和加载测试证据。第三层讲一致性：大模型要分片保存，异步保存必须在一致边界生成 snapshot，写临时目录，校验 checksum，最后提交 manifest，恢复时只读 committed checkpoint；manifest 要记录 checkpoint id、global step、shard、checksum、状态类型和 format version。第四层讲策略治理：保留最近 N 个 resume、周期 milestone、best eval、release 和 failed temporary 清理，按 hot / warm / archive 分层控制成本；抢占时要求 checkpoint 新鲜度，收到 notice 尽量触发保存，调度器优先抢占损失小的任务；评估后标记 best checkpoint，发布前做转换、推理加载测试、安全审核和 registry 注册。最后用 Checkpoint Strategy Gate 收束：RPO/RTO、保存触发、用途分层、异步 commit、恢复选择、抢占、评估、发布、保留、成本、监控、恢复演练和权限保护都过线，才算 checkpoint 策略可靠。

40ZZZF. 大模型训练平台的训练容错和自动重试怎么设计和审计？
       考察点：fault taxonomy、retryability、max attempts、backoff、checkpoint resume、node reschedule、GPU isolation、NCCL hang、OOM policy、data permission、checkpoint commit、NaN rollback、diagnosis report、blacklist precision、user action、fault tolerance gate。
       回答框架：先说明训练容错不是“失败就重试”，而是故障分类、损失控制、自动恢复和清晰诊断。第一层做 fault taxonomy：把 node failure、GPU Xid / ECC、transient network、NCCL hang、OOM、data permission、checkpoint corruption、config error、code bug 和 loss NaN 分开；node / transient network / eviction 可以自动重试，permission、config、持续 OOM、uncommitted checkpoint 和 NaN 要快速失败或回滚诊断。第二层讲自动重试边界：每个任务有 max attempts、timeout、backoff / jitter、retryable allowlist、non-retryable blocklist 和 attempt_id；重试必须从最新 committed checkpoint 恢复，并校验 manifest、checksum、配置兼容、optimizer、scheduler、RNG 和 dataloader cursor。第三层讲故障恢复：节点失败要 cordon / blacklist 原节点并调度到健康节点；GPU 多次 Xid / ECC 要隔离，不能让坏硬件反复影响任务；NCCL timeout 要看 rank heartbeat、首个失败 rank、NCCL 日志、worker 退出状态、网络和 OOM 事件；OOM 通常不能原样重试，要给显存峰值、micro batch、sequence length、activation checkpointing、FSDP / ZeRO 建议，自动降 batch 必须用户允许。第四层讲诊断和体验：生成失败时间、阶段、rank、错误类型、日志、指标、checkpoint、重试次数、损失 step 和建议动作，用户能看到是否正在 retry、从哪个 checkpoint 恢复、是否需要自己改配置。最后用 Training Fault Tolerance Gate 收束：分类、retryability、预算、backoff、checkpoint、节点/GPU隔离、hang、OOM、权限、数值、诊断、黑名单和用户动作全部过线，才算训练容错可靠。

40ZZZG. 大模型训练平台的数据供给系统怎么设计和审计？
       考察点：dataset manifest、shard format、small files、streaming、prefetch、local cache、distributed cache、shuffle seed、rank assignment、data locality、DataLoader workers、H2D copy、bad samples、observability、dataloader state、permission version、data supply gate。
       回答框架：先说明训练数据供给不是“能读到数据”，而是在版本、权限、shuffle 和可复现正确的前提下，把数据以足够高且稳定的吞吐送到 GPU。第一层设计数据产物：原始数据离线清洗、去重、过滤、tokenize、packing，写成适合训练的 shard；manifest 要记录 dataset id、version、preprocessing version、shard 列表、token 数、checksum、来源和权限，避免训练只引用一个可变路径。第二层讲读取和缓存：超大数据可以流式读取，但必须有预取、timeout、retry 和吞吐监控；重复使用或热点数据预热到本地 NVMe，并用版本隔离、LRU、容量和 cache hit rate 治理；多任务共享热点数据时可以加分布式缓存，但要防止缓存层成为新瓶颈。第三层讲 shuffle 和分布式切分：大规模流式训练通常做 shard 级 shuffle、shuffle buffer、epoch seed 和 `set_epoch`，不是追求全局完全随机；每个 rank 的 shard 要不重不漏、负载均衡，checkpoint 里要保存 epoch、shard index、sample offset、shuffle seed 和 worker state，防止恢复后重复或跳过数据。第四层讲性能和质量：DataLoader 要看 `num_workers`、persistent workers、prefetch、CPU 预处理、batch packing、pinned memory、异步 H2D copy；坏样本要记录位置和比例，超过阈值失败，不能无限跳过。最后用 Training Data Supply Gate 收束：manifest、shard、小文件、streaming、cache、shuffle、rank、locality、DataLoader、H2D、坏样本、观测、恢复状态和权限版本都过线，才算数据供给可靠。

40ZZZH. 大模型训练平台的权限、安全和审计怎么设计？
       考察点：identity、service account、workload identity、RBAC / ABAC、least privilege、dataset purpose、artifact classification propagation、quota / priority、image supply chain、runtime isolation、secret manager、network egress、log redaction、model artifact permission、high risk approval、audit integrity、incident response、security gate。
       回答框架：先说明训练平台安全不是登录鉴权，而是控制谁能以什么运行身份提交任务、读什么数据、用什么资源、运行什么镜像和代码、产生和下载什么模型产物，并且全流程可审计。第一层是身份和权限：接入 IdP，区分用户身份、项目身份、TrainingJob 运行身份、service account 和平台服务身份；任务运行时用最小权限 workload identity，避免共享管理员身份；权限用 RBAC + ABAC，把角色和 project、team、data classification、resource pool、purpose、environment 等属性结合，避免 wildcard。第二层是数据和产物：训练提交时校验 dataset id、version、classification、allowed project 和 allowed purpose；使用 restricted 数据训练出的 checkpoint、final model、adapter、eval report、日志和样本输出继承最高敏感等级，不能绕过原始数据权限下载模型。第三层是运行和供应链：资源池、优先级和 GPU-hour 预算要受配额和审批控制；镜像必须来自可信 registry，绑定 digest，完成扫描、签名、provenance / SBOM 和 secret exclusion；运行时禁止 privileged、hostPath、root，按 Pod 安全基线限制权限；密钥通过 Secret Manager 注入短期最小权限凭证并轮换；网络默认拒绝，egress allowlist，生产服务和外部访问要记录。第四层是日志、审批和审计：日志要 RBAC、脱敏、下载审计和留存；模型 artifact registry 要做下载权限、阶段策略、发布审批和 lineage；高风险动作如 restricted 数据训练、模型导出、生产发布、关闭日志、未审核镜像、外联和删除关键 checkpoint 要审批；审计日志覆盖 actor、action、resource、decision、reason、request id、timestamp，防篡改并保留；事故响应要能冻结任务、撤销密钥、隔离产物、查询审计和复盘。最后用 Training Platform Security Gate 收束，证明越权阻断率、过度授权率、产物等级传播、密钥泄露率、审计覆盖率和审批覆盖率都达标。

40ZZZI. 如何设计一个企业级大模型训练平台？
       考察点：requirement clarification、TrainingJob contract、lifecycle state machine、admission / quota、queue / scheduler、gang scheduling、topology-aware placement、distributed launcher、data config lineage、checkpoint recovery、observability / experiment tracking、fault recovery、security audit、cost capacity、developer self-service、artifact contract、trade-off、design gate。
       回答框架：先说明这不是“用 Kubernetes 跑 Job”，而是设计 TrainingJob 生命周期管理系统。第一层做需求澄清：训练类型是预训练、SFT、RLHF 还是评估，集群规模和 GPU 类型，多租户和队列策略，是否支持抢占，数据是否敏感，RPO / RTO、模型发布和成本预算是什么。第二层给核心对象和状态机：TrainingJob 绑定 owner、project、image digest、code commit、dataset version、manifest、final config、resources、distributed config、checkpoint policy、observability、security、queue 和 priority；状态机覆盖 submitted、validated、queued、scheduled、starting、running、checkpointing、retrying、failed、completed 和 cancelled，并记录 attempt。第三层讲控制面和调度：API / CLI / SDK 提交后做 schema、镜像、代码、数据权限、checkpoint 路径、quota dry run、priority approval 和 idempotency 校验；调度层负责 queue、quota、fair share、gang scheduling、拓扑感知、backfilling、preemption、fragmentation 和 starvation control。第四层讲运行闭环：launcher adapter 支持 `torchrun`、DeepSpeed、Megatron、Ray 或 MPI，把 scheduler handoff 转成 rank、local rank、world size、rendezvous、GPU binding 和 NCCL 配置；数据和配置血缘绑定镜像、代码、数据、环境、seed 和 experiment run；checkpoint 必须有 committed manifest、checksum、restore drill、scheduler preemption hook 和最大 stale 时间；可观测性把 logs、metrics、events、traces、rank 维度、dashboard、alerts、experiment tracking 和 artifact registry 连起来；容错按故障分类、retry budget、backoff、checkpoint resume、fast fail 和诊断报告处理。第五层讲治理和体验：安全覆盖身份、RBAC / ABAC、数据用途、运行时隔离、镜像供应链、审计和事件响应；成本按 owner / project / job / queue 归因 GPU-hours、存储、网络和利用率；开发者自助提供 Portal、CLI、SDK、模板、状态页、错误报告和从 checkpoint 重跑。最后用 Training Platform Design Gate 收束：需求、契约、状态机、准入、调度、launcher、血缘、checkpoint、观测、容错、安全、成本、自助、接口、trade-off 和 P0 风险全部过线，才算训练平台系统设计可靠。

40ZZZJ. 如何设计一个大模型推理平台？
       考察点：request lifecycle、TTFT、TPOT、P95 / P99、tokens/s、Model Registry、Model Router、Inference Runtime、prefill / decode、continuous batching、KV cache、autoscaling、warm pool、canary / rollback、rate limit、degrade、safety、cost attribution、observability trace、multi-model governance、inference platform gate。
       回答框架：先说明推理平台不是把模型包成 HTTP 服务，而是围绕在线请求生命周期、延迟吞吐、版本治理、安全成本和可用性的模型服务系统。第一层做需求澄清：模型数量、QPS、输入 / 输出 token 分布、上下文长度、是否流式返回、TTFT / TPOT / P99 SLO、多租户、成本预算、敏感数据和发布频率。第二层讲请求链路：请求进入 API Gateway 后做 auth、tenant quota、rate limit 和 input safety；Model Router 根据租户、模型版本、请求类型、prompt 长度、延迟 SLO、成本预算、当前负载和安全等级选择 endpoint；scheduler 控制 queue、priority、batching 和长请求隔离；runtime 执行 tokenization、prefill、decode、KV cache、continuous batching 和 streaming；输出再过 safety、metrics、cost 和 audit。第三层讲核心组件：Model Registry 记录 model version、weight URI、tokenizer、runtime、context limit、quantization、eval report、stage 和 rollback version；Router 不是普通负载均衡，要理解请求 token 成本和模型能力差异；Runtime 可以是 vLLM、TGI、Triton、TensorRT-LLM、SGLang 或自研，但平台不要把治理逻辑塞进 runtime；KV cache 要有 block manager、prefix cache、eviction、max context、admission control 和 KV pressure 指标。第四层讲稳定性和发布：扩缩容不能只看 GPU utilization，要同时看 queue、TTFT、tokens/s、KV pressure、cold start 和 warm pool；灰度 / A/B / 回滚要看质量、安全、延迟、成本和业务指标；保护机制包括 max input / output、queue timeout、execution timeout、circuit breaker、小模型降级和缓存降级。第五层讲治理：observability 要有 metrics、logs、traces、request id、route / queue / prefill / decode / stream spans、dashboard 和 alerts；成本按 tenant、model、version、input / output tokens、GPU seconds 和 cache savings 归因；安全覆盖 authz、输入输出策略、PII / log redaction、retention 和 audit；多模型平台还要有 route / scaling / quota / safety / cost policy。最后用 Inference Platform Gate 收束：TTFT、TPOT、P99、KV pressure、错误率、成本、质量回归、回滚准备和 P0 风险都过线，才算推理平台可靠。

40ZZZK. 如何选择大模型模型服务运行时？
       考察点：Triton、vLLM、TGI、SGLang、TensorRT-LLM、自研 runtime、model hardware fit、runtime boundary、model format / tokenizer、prefill / decode、continuous batching、KV cache、streaming、quantization、distributed inference、observability、benchmarking、deployment rollback、maintenance、cost capacity、self-development threshold、runtime selection gate。
       回答框架：先说明 runtime 选型不是“哪个最快”，而是把模型、硬件、SLO、成本和团队能力统一审计。第一层做需求澄清：模型是 decoder LLM、embedding、reranker、多模态还是传统模型；权重大小、上下文长度、输入 / 输出 token 分布、并发、TTFT / TPOT / P99 SLO、是否 streaming、是否多租户、是否需要量化和多 GPU。第二层讲候选定位：Triton 是通用多模型推理服务框架，适合多后端、动态 batching、模型仓库和传统 DL serving；vLLM 更偏 LLM serving，重点看 PagedAttention / KV cache、continuous batching、OpenAI-compatible API 和易用性；SGLang 更强调结构化生成、复杂 LLM program、cache 复用和服务端执行；TGI 与 Hugging Face 生态结合好，但新项目要考虑当前维护状态和后续更新责任；TensorRT-LLM 适合 NVIDIA 生态下的深度性能优化；自研只有在规模收益、特殊模型 / 硬件 / 调度和团队系统能力都成立时才考虑。第三层讲统一审计表：检查 model hardware fit、runtime boundary clarity、model format tokenizer fit、prefill decode scheduler fit、continuous batching fit、KV cache memory manager fit、streaming API fit、quantization accuracy fit、distributed inference fit、observability metrics fit、benchmarking method fit、deployment rollback fit、ecosystem maintenance fit、cost capacity fit 和 self-development threshold。第四层讲 benchmark：必须固定同一模型、权重格式、硬件、精度、输入输出长度分布、并发、sampling 参数、streaming 设置和 SLO，同时看 TTFT、TPOT、E2E、tokens/s、KV pressure、OOM、错误率、质量回归和每 1k token 成本。最后用 Runtime Selection Gate 收束：功能覆盖、延迟吞吐、质量、安全、维护、回滚、成本容量和 P0 风险全部过线，才算选型可靠。

40ZZZL. 如何分析 Prefill、Decode、KV Cache 与推理资源画像？
       考察点：request token profile、input / output tokens、TTFT、TPOT、prefill phase、decode phase、KV cache formula、KV pressure、PagedAttention block、prefix cache、long context、tenant KV isolation、continuous batching、PD disaggregation、streaming backpressure、observability phase metrics、cost capacity、resource profile gate。
       回答框架：先说明大模型推理不能只看 QPS，因为每个请求的输入 / 输出 token、上下文长度和流式输出差异很大。第一层拆阶段：请求进入后经历 gateway、route、queue、tokenize、prefill、first token、decode、sample、stream 和释放 KV cache；TTFT 主要覆盖排队、tokenize、prefill 和首 token，TPOT 主要覆盖 decode、sampling 和 streaming。第二层讲资源差异：prefill 处理完整 prompt，长输入会显著增加首 token 等待；decode 逐 token 生成，长输出会让 GPU 和 KV cache 被占用更久；KV cache 近似为 `2 * layers * active_sequences * context_length * kv_heads * head_dim * dtype_bytes`，所以长上下文和高并发会先打满显存或显存带宽。第三层讲治理：KV cache 要有 block / paged 管理、admission control、KV pressure、prefix cache、eviction、取消释放和 OOM 保护；prefix cache 要看命中 token、节省 prefill 时间、租户隔离、权限和错误复用风险；长上下文要有 max context、RAG budget、压缩 / 截断、长短队列和专用实例；多租户要有上下文配额、KV 配额、优先级和成本归因。第四层讲调度和架构：continuous batching 解决动态加入退出；PD 分离只有在 prefill / decode 资源特征差异、流量规模和 KV transfer 成本都支持时才值得引入；streaming 要有 backpressure、cancel、flush metric 和 timeout。最后用 Resource Profile Gate 收束：TTFT P95、TPOT P95、KV pressure、prefix savings、OOM、错误率、质量回归、成本和 P0 风险都过线，才算推理资源画像可靠。

40ZZZM. 如何设计 Continuous Batching、PagedAttention 和队列调度？
       考察点：request arrival profile、dynamic join / exit、iteration-level batching、token budget、max active sequences、chunked prefill、decode-first policy、PagedAttention block table、free list、KV admission、long-short isolation、tenant fairness、cancellation cleanup、streaming backpressure、scheduler metrics、scheduler gate。
       回答框架：先说明生成式 LLM 不适合静态 batch，因为请求到达时间、输入长度、输出长度和取消时间都不同。第一层讲 continuous batching：调度器维护 waiting queue、running set 和 finished set，在每个 decode iteration 动态移除完成请求、加入新请求，并用 max batch tokens 和 max active sequences 控制本轮计算和 KV 压力。第二层讲 PagedAttention：它不是新的 attention 数学，而是把逻辑连续的 KV cache 映射到固定大小物理 blocks，用 block table、free list、碎片指标和回收策略支持动态加入、退出、取消和 prefix 复用。第三层讲 phase policy：prefill 偏计算密集，decode 偏显存带宽和 KV 访问；长 prefill 会拉高已有请求 TPOT，过度优先 decode 会拉高新请求 TTFT，所以要用 chunked prefill、decode-first、prefill token budget、长短请求隔离和必要的专用实例。第四层讲多租户和稳定性：按 token share、GPU seconds、KV blocks、优先级和成本归因做公平性，准入控制要看 KV watermark、queue timeout、tenant quota、context limit 和 reject/degrade 策略；取消、超时和客户端断开必须释放 KV blocks 与 streaming 状态。最后用 Scheduler Gate 收束：TTFT P95、TPOT P95、scheduled tokens、active sequences、KV waste、reject rate、cancellation cleanup、tenant share、指标覆盖和 P0 风险都过线，才算调度可靠。

40ZZZN. 如何设计模型路由：模型选择、能力路由、成本路由和降级路由？
       考察点：request intent profile、model capability profile、permission candidate filter、cost budget policy、latency SLO policy、quality safety policy、fallback chain governance、canary routing stability、realtime health load、route trace coverage、degrade policy control、route config versioning、model routing gate。
       回答框架：先说明模型路由不是普通负载均衡。负载均衡通常在等价实例之间分流，而模型路由要先在不同能力、成本、延迟、版本、安全等级和租户权限的模型之间做选择，再选 endpoint。第一层做请求画像：任务类型、租户、输入 / 输出 token、多模态、tool calling、结构化输出、上下文长度、延迟 SLO、成本预算、风险等级和是否允许降级。第二层做模型能力画像：支持任务、最大上下文、模态、tool calling、质量评估、安全等级、成本、TTFT / TPOT 基线、健康负载、版本和回滚状态。第三层讲候选过滤和打分：先按权限、能力、健康、预算和 SLO 硬过滤，再用 utility score 在质量、成本、延迟和风险之间排序，不能让高质量模型绕过权限或预算。第四层讲治理：fallback chain 要有最大尝试次数、总时间预算、总成本预算、错误类型和熔断；灰度要稳定哈希分桶并支持快速回滚；降级要明确哪些业务可降到小模型、缓存、文本-only 或简化流程。第五层讲可观测性和配置：route trace 记录 request id、tenant、task、tokens、selected model、version、reason、fallback、degrade、cost、latency 和 config version；路由配置要版本化、dry-run、审计和回滚。最后用 Model Routing Gate 收束：权限正确、能力匹配、预算可控、SLO 可达、质量安全、fallback 有界、灰度稳定、trace 完整、降级可解释和 P0 风险清零，才算路由系统可靠。

40ZZZO. 如何设计大模型推理缓存体系？
       考察点：prompt cache、prefix cache、KV cache、semantic cache、result cache、request cache profile、cache key、model / tokenizer / parameter version、tenant isolation、TTL、semantic false hit、stale hit、eviction、streaming cache、saved tokens、cost latency savings、cache governance gate。
       回答框架：先说明推理缓存不是单个 Redis 缓存，而是多个层次的缓存机制。第一层讲缓存边界：result cache 缓存完整请求和完整输出，命中最快但只适合确定性任务；semantic cache 用 embedding 相似检索复用相似问题，适合 FAQ 但有误命中风险；prompt / prefix cache 复用 system prompt、工具说明、RAG 文档或代码上下文前缀，主要降低 prefill 和 TTFT；KV cache 是 runtime 内部 attention 状态，支撑 decode、continuous batching 和 prefix reuse，但占 GPU 显存。第二层讲 key 和版本：cache key 不能只用 prompt，必须包含模型、模型版本、tokenizer、生成参数、prompt / prefix hash、租户、权限域、安全策略、数据版本、TTL 和 sampling 配置。第三层讲安全和一致性：跨租户、跨权限、跨数据分类不能复用私有缓存；模型升级、prompt 模板升级、知识库更新、安全策略更新和权限变化都要触发失效或版本隔离；敏感请求要短 TTL 或禁用结果缓存。第四层讲语义缓存：相似度阈值只是第一道门，还要检查任务等价、权限、数据版本、用户上下文、风险等级和人工评估误命中率。第五层讲指标：不能只看 hit rate，要看 saved input/output tokens、saved TTFT、saved GPU time、cache latency、eviction、stale hit、semantic false hit、tenant hit rate、net cost saved 和 trace coverage。最后用 Cache Governance Gate 收束：收益为正、误命中可控、过期命中可控、租户隔离正确、trace 完整、驱逐和配额可控、P0 风险清零，才算缓存体系可靠。

40ZZZP. 如何设计大模型推理服务的自动扩缩容？
       考察点：traffic token profile、QPS、input / output tokens/s、TTFT、TPOT、queue wait、active sequences、KV pressure、GPU utilization、cold start、warm pool、multi-metric recommendation、draining、router / admission coupling、tenant quota、cost guard、autoscaling trace、autoscaling gate。
       回答框架：先说明 LLM serving 不能直接照搬普通 Web 服务的 CPU / QPS HPA，因为同样 QPS 的请求可能 token 数和 KV 占用相差几十倍，GPU utilization 也不等于用户体验。第一层做流量画像：按模型、租户、任务类型统计 QPS、input tokens/s、output tokens/s、上下文长度、active sequences、长短请求比例和突发流量。第二层定义 SLO：TTFT、TPOT、E2E、queue wait、p95 / p99、timeout 和 error rate 都要有目标，队列和 pending prefill / decode 是比 p99 更早的拥塞信号。第三层讲容量指标：同时看 GPU utilization、GPU memory、KV pressure、free KV blocks、eviction、OOM、tokens/s 和 per-GPU capacity；副本推荐按 QPS、input tokens、output tokens、queue wait、TTFT、TPOT、KV 和 GPU 多指标取最大，而不是单指标触发。第四层讲冷启动和热池：扩容要估算调度、拉镜像、加载权重、初始化 runtime、分配显存、kernel 预热和路由注册的 `T_ready`；warm pool 可以降低响应时间，但会增加空闲 GPU 成本。第五层讲缩容：扩容快、缩容慢，缩容前要 router 摘除、draining、不接新请求、等待 active / streaming 完成、设置 cooldown，避免振荡和中断。第六层讲联动：autoscaler 要和 router、admission、限流、缓存、降级、租户配额和成本预算共享信号，过载时既可扩容，也可路由转移、低优先级降级或限流。最后用 Autoscaling Gate 收束：SLO、队列、KV、有效容量、冷启动、warm pool、draining、租户公平、成本、trace 和 P0 风险都过线，才算扩缩容可靠。

40ZZZQ. 如何设计大模型推理服务的限流、熔断、重试、超时和降级？
       考察点：request/token rate limit、concurrency quota、admission capacity、circuit breaker state、retry classification、retry budget、idempotency、timeout budget、degradation policy、tenant fairness、streaming cleanup、protection trace、protection gate。
       回答框架：先说明保护机制的目标不是让每个请求都成功，而是在过载和故障时控制失败范围。第一层做限流：不能只按 QPS，要按租户、API key、模型、endpoint、input tokens、output tokens、最大上下文、并发和成本预算做多维配额。第二层做 admission control：限流回答“这个租户是否允许这么用”，admission 回答“系统现在接不接得住”，要看 waiting queue、queue wait、KV pressure、endpoint 健康、剩余 timeout 和 SLO。第三层讲熔断：对模型实例、endpoint、工具或地域维护 closed / open / half-open 状态，用滑动窗口错误率、慢调用率、p99、队列、OOM 和 health check 触发 open，half-open 只放探测流量。第四层讲重试：只对尚未进入 runtime 执行的临时错误重试，权限失败、配额不足、安全拒绝、用户取消、已开始 streaming 或已有 partial output 都不重试；必须有 max attempts、指数退避、jitter、retry budget、幂等键和总 deadline。第五层讲 timeout budget：把总超时拆给 gateway、queue、prefill、decode、streaming、工具调用和后处理，剩余预算不足时拒绝、减少输出或降级。第六层讲降级：按业务风险选择小模型、缓存、截断上下文、减少输出、关闭工具调用、异步处理或转人工，并记录用户是否可见、质量影响和成本收益。最后用 Protection Gate 收束：限流正确、准入可靠、熔断隔离、重试有界、超时受控、降级可解释、streaming / cancel 释放资源、trace 完整、成本可归因且 P0 风险清零，才算保护策略可靠。

40ZZZR. 如何设计大模型推理平台的灰度发布、A/B 测试、回滚和模型版本管理？
       考察点：release bundle trace、artifact integrity、tokenizer / prompt / runtime binding、offline eval、safety eval、contract test、stable bucket、canary ramp、A/B design、sample ratio health、guardrail metrics、rollback readiness、release audit trace、post-release monitoring、release governance gate。
       回答框架：先说明模型发布不是把权重文件切成 100% 流量，而是发布一个由模型权重、tokenizer、prompt、runtime config、推理参数、安全策略、路由策略、eval report、owner、approver 和 rollback target 组成的 release bundle。第一层讲版本管理：ModelVersion 要绑定 checksum、artifact URI、训练数据 / SFT / RLHF / DPO 信息、能力声明、上下文长度、默认参数、评估报告、安全报告、状态机和审批记录；线上 trace 要能复现“这个输出来自哪个模型、tokenizer、prompt、runtime 和策略”。第二层讲发布前门禁：权重完整性、tokenizer 兼容、runtime 兼容、离线质量 eval、安全 eval、结构化输出和 tool call 契约测试、压测、成本和延迟都要过线。第三层讲灰度：用稳定 hash 对用户、租户或会话做 stable bucket，按 1%、5%、10%、25%、50% 逐步放量，每一步看质量、安全、p99、TTFT / TPOT、成本、工具失败率、解析失败率、投诉率和业务指标，失败就 pause / abort / rollback。第四层讲 A/B：灰度关注风险，A/B 关注效果；实验要定义随机单位、baseline、treatment、样本量、主指标、guardrail、sample ratio mismatch、切片分析和停止规则，不能看到短期提升就全量。第五层讲回滚：回滚对象不只是模型权重，还包括 tokenizer、prompt、runtime 参数、路由权重、安全策略、实验配置和 feature flag；回滚目标要预先验证，回滚时间要满足 SLO。最后用 Release Governance Gate 收束：版本包完整、artifact 完整、离线 / 安全 / 契约门禁、稳定分桶、canary、A/B、guardrail、回滚、审计 trace、上线后监控和 P0 风险都过线，才算发布可靠。

40ZZZS. 如何回答“大模型推理平台系统设计”这类综合题？
       考察点：requirement clarification、request lifecycle、SLO token contract、model registry、router policy、runtime prefill decode、KV scheduler capacity、cache isolation、autoscaling capacity、protection policy、release governance、tenant security、observability trace、cost capacity、trade-off、inference platform design gate。
       回答框架：先说明推理平台系统设计不是罗列组件，而是证明在线请求从入口到模型输出、监控、计费和审计都能被统一治理。第一层做需求澄清：模型类型、模型规模、QPS、input / output token 分布、上下文长度、TTFT、TPOT、p99、streaming、多租户、安全、成本、发布频率和可用性目标。第二层给总体架构：API Gateway、Auth / Quota / Rate Limit、Safety Filter、Model Router、Cache Layer、Admission Controller、Inference Runtime Cluster、Model Registry、Deployment Controller、Autoscaler、Observability 和 Cost / Audit。第三层讲请求链路：认证、配额、安全、路由、缓存、准入、queue、prefill、decode、streaming、输出安全、trace、成本和审计都要有阶段指标。第四层讲容量和公式：端到端延迟拆成 gateway、auth、route、cache、queue、prefill、decode、safety 和 stream；token 负载用请求速率乘平均输入输出 token；KV 容量用层数、KV heads、head dimension、元素字节数、上下文 token 和活跃请求数估算；副本数要按 QPS、input tokens/s、output tokens/s 和 KV 并发取最大值。第五层讲治理闭环：路由要看权限、能力、成本、SLO、健康和灰度；缓存要绑定模型、tokenizer、prompt、参数、租户、权限和数据版本；扩缩容要看队列、TTFT、TPOT、KV pressure、冷启动和 warm pool；保护机制要有多维限流、准入、熔断、重试预算、timeout budget 和降级；发布要有版本包、canary、A/B、guardrail 和回滚；多租户要有权限、配额、缓存隔离、日志脱敏、审计和成本归因。最后用 Inference Platform Design Gate 收束：需求覆盖、SLO、KV 容量、trace、发布回滚、多租户安全、成本和 P0 风险全部可验收，才算完整答案。

40ZZZT. 如何设计大模型数据平台与训练数据供给链路？
       考察点：source registry、raw data lake、cleaning policy、dedup contamination、PII redaction、quality scoring、dataset builder、manifest、dataset version、streaming shard、permission purpose、lineage、observability、read throughput、data wait、data platform supply gate。
       回答框架：先说明数据平台不是对象存储目录，也不等于训练时 DataLoader；它要证明一批数据从进入平台到被训练任务消费的全过程都可信、可复现、可授权、可观测和可追溯。第一层讲数据入口：所有 source 要登记 owner、license、classification、allowed purpose、raw URI、ingest time 和 checksum；raw lake 要不可随意覆盖，保留原始对象、写入事件、校验和和审计日志，支持后续清洗策略变化时重建。第二层讲数据治理：cleaning、dedup、PII redaction、quality scoring 都要版本化，去重不只看重复率，还要隔离 train / eval overlap 和 benchmark contamination；PII、密钥和 restricted 数据要在进入训练集前阻断或审批。第三层讲 dataset builder 和 manifest：builder 配方记录来源集合、清洗 / 去重 / PII 策略、质量过滤、采样权重、split、格式和权限约束；manifest 至少包含 dataset id、version、sources、recipe version、shard 列表、样本数、token 数、checksum、权限版本、lineage run id 和质量统计；dataset version 必须冻结，不允许 latest 路径漂移。第四层讲供给和接口：训练平台通过 dataset registry 引用 dataset version / manifest，而不是拼路径；shard 要适合 streaming、shuffle、预取、重试和缓存版本隔离；用读取吞吐 `Q_read` 和等数据比例 `R_wait` 证明数据平台能喂饱 GPU。第五层讲权限和血缘：提交训练任务时校验 actor、project、dataset version、classification 和 allowed purpose；lineage 要能从 source、raw object、cleaning run、dedup run、builder run、manifest、training job、checkpoint、eval report 追到线上模型。最后用 Data Platform Supply Gate 收束：source、raw lake、cleaning、dedup、PII、quality、builder、manifest、version、streaming shard、permission、lineage、observability、throughput、data wait 和 P0 风险都过线，才算数据平台可靠。

40ZZZU. 如何设计数据版本管理、数据血缘和数据质量监控平台？
       考察点：dataset version contract、manifest checksum、immutable snapshot、delta version diff、lineage graph、impact analysis、schema quality、content quality、safety compliance、distribution drift、train/eval leakage、annotation quality、synthetic data monitoring、quality alert routing、TrainingJob / ModelVersion linkage、data version quality gate。
       回答框架：先说明数据版本不是目录名，而是 dataset id、version、parent version、manifest、shard checksum、schema、tokenizer、清洗 / 去重 / 采样规则、split seed、权限、质量统计和审计记录共同组成的不可变契约。第一层讲版本管理：dataset 发布后 snapshot 不应覆盖，修复或规则变化要生成新版本；manifest checksum 和每个 shard checksum 要能证明文件、样本数、token 数和规则版本没有漂移；增量版本要生成 diff report，说明新增、删除、token delta、来源比例、语言 / 领域分布、重复率、PII、质量分和采样配方变化。第二层讲血缘：用 lineage graph 连接 raw source、ingestion job、raw dataset、cleaning job、dedup job、dataset version、training job、checkpoint、eval report、model version 和 deployment；血缘事件要有 job、run id、inputs、outputs、时间和 owner；当数据源撤权、删除请求、许可证风险或污染发现时，平台能沿图做 impact analysis，列出受影响 dataset、训练任务、checkpoint、模型和线上部署。第三层讲质量监控：基础层看样本数、token 数、文件数、长度分布和 checksum；结构层看 schema 合法率、JSON 解析成功率、空字段和标签缺失；内容层看重复率、低质比例、乱码、语言 / 领域分布；安全层看 PII、secret、license、敏感等级和 train/eval overlap；标注数据看一致性、gold set 和指南版本；合成数据看生成模型、prompt 版本、比例、多样性、事实性和难度。第四层讲门禁和告警：发布前用 Data Version Quality Gate 阻断 manifest 缺失、checksum 失败、snapshot 可变、lineage 断边、schema 失败、PII 未阻断、overlap、漂移无解释、标注质量下降和合成比例过高；告警必须带 dataset、version、source、job、rule version 和 owner。最后讲联动：TrainingJob 必须记录 dataset version、manifest checksum、tokenizer、sampling seed、shuffle config 和 split；ModelVersion 必须能反向追溯训练任务和数据版本，这样线上模型事故才能复盘和回滚。

40ZZZV. 如何设计模型仓库，管理权重、tokenizer、配置、adapter 和量化版本？
       考察点：ModelVersion contract、weight manifest、safe weight format、tokenizer config binding、adapter base compatibility、merge lineage、quantization eval、runtime compatibility、eval report linkage、safety permission、release status、rollback alias、artifact lineage、load cache、lifecycle retention、model registry gate。
       回答框架：先说明模型仓库不是对象存储目录，而是训练、评估和推理平台之间的模型资产控制面。第一层讲对象模型：Model 是逻辑系列，ModelVersion 是具体版本，Artifact 是权重、tokenizer、config、adapter、量化包和报告的文件集合；ModelVersion 要记录 model id、version、architecture、参数规模、owner、stage、训练任务、数据版本、权重 manifest、tokenizer hash、config hash、eval report、安全报告、发布 alias、rollback alias 和审批记录。第二层讲产物完整性：权重要有 manifest、分片 checksum、总大小校验和安全扫描，生产优先用 safetensors 或经过审计的安全格式；tokenizer、chat template、special tokens、stop tokens 和 model config 必须和权重绑定，不能靠同名目录猜测。第三层讲 adapter、merge 和量化：adapter 要记录 base model、rank、target modules、训练任务和评估；merge 后要记录 base + adapter -> merged model 的 lineage 并重新评估；量化版本要记录源模型、方法、bits、校准数据、工具版本、目标 runtime、质量退化、延迟吞吐和 eval report，不能只看速度。第四层讲推理接口和发布治理：部署前校验 runtime 是否支持当前格式、量化、上下文长度、tensor parallel、CUDA / driver 和加载 smoke test；模型发布状态用 registered、validated、evaluated、staged、production、deprecated、archived 管理；生产发布要有安全权限、审批、canary / A/B、rollback alias、回滚加载测试和审计日志。最后用 Model Registry Gate 收束：版本契约、权重完整、安全格式、tokenizer / config 绑定、adapter / merge / quantization 血缘、runtime 兼容、eval / safety、发布回滚、缓存加载和生命周期策略全部可验收，才算模型仓库可靠。

40ZZZW. 如何设计 Artifact 管理系统，统一管理 dataset、checkpoint、eval report 和 deployment package？
       考察点：artifact metadata contract、dataset artifact manifest、checkpoint recoverability、eval report reproducibility、deployment package completeness、release manifest integrity、checksum integrity、artifact lineage、store / metadata split、permission access control、lifecycle retention、promotion gate、experiment tracking linkage、rollback readiness、deletion dependency safety、artifact management gate。
       回答框架：先说明 Artifact 管理比模型仓库更通用，目标是让训练、评估、部署、回滚和审计中所有关键产物都可定位、可校验、可复现、可追溯、可治理。第一层讲统一元数据：每个 artifact 至少要有 artifact id、type、name、version、URI、checksum、size、created time、producer job、input dependencies、permissions、lifecycle state 和 tags；大文件放 artifact store，metadata store 存索引、权限、状态、checksum 和依赖，不把大模型权重塞进数据库。第二层按类型讲契约：dataset artifact 要有 manifest、split、schema、样本数、token 数、来源组成、清洗 / 去重规则、质量指标和权限；checkpoint artifact 要区分断点恢复和候选模型，恢复型必须包含 optimizer、scheduler、dataloader、RNG 和 distributed metadata；eval report 要绑定 model / checkpoint、eval dataset、eval code、metric definitions、prompt template、inference config、runtime、sample outputs 和 failure cases；deployment package 要包含 weights、tokenizer、model config、runtime config、image digest、health check、generation config、安全策略、route policy 和 release manifest。第三层讲完整性和血缘：上传、下载、部署、归档前都要做 checksum / manifest 校验；lineage graph 要能从 dataset / config -> training job -> checkpoint -> eval report -> model version -> deployment package -> release manifest -> deployment 双向追溯。第四层讲生命周期和流转：checkpoint -> candidate model -> evaluated model -> release candidate -> production package 每次 promotion 都要过 integrity、eval、safety、runtime 和 approval；retention policy 要按 dataset、checkpoint、eval report、deployment package 分类型设计；删除前必须检查下游依赖、回滚、legal hold、合规和审计记录。最后用 Artifact Management Gate 收束：元数据、manifest、checksum、权限、lineage、promotion、experiment tracking、rollback、deletion safety 和成本生命周期全部过线，才算系统可靠。

40ZZZX. 如何设计大模型实验追踪系统，管理参数、指标、日志、样本和代码版本？
       考察点：run metadata contract、parameter config capture、metric curve step capture、log run linkage、sample result diagnosis、code version reproducibility、data version reproducibility、model checkpoint linkage、environment capture、prompt eval config、artifact output linkage、cost attribution、state transition、search index、lineage graph、decision note、experiment tracking gate。
       回答框架：先说明实验追踪不是保存最终分数，而是让训练、评估、prompt 实验和压测 run 可比较、可复现、可解释、可审计。第一层讲对象模型：Experiment 是一组相关实验，Run 是一次具体执行；Run metadata 至少包含 run id、experiment id、owner、status、created / ended time、tags、notes、hypothesis、decision 和 next action。第二层讲输入记录：参数要覆盖学习率、batch、optimizer、seed、sequence length、precision、parallel config、ZeRO stage；代码要记录 commit、branch、dirty flag、diff、image digest、entrypoint 和依赖；数据要记录 dataset version、manifest checksum、split、sampling recipe、shuffle seed 和 tokenizer；环境要记录 Python、框架、CUDA、driver、NCCL、GPU 和容器镜像。第三层讲过程观测：metrics 必须带 step / timestamp 和 metric definition；logs 必须和 run id 关联；样本级结果要保存 input hash、prediction、reference、score、error type、prompt version 和 evaluator version，否则无法定位退化。第四层讲输出和协作：checkpoint、final model、eval report、sample output 和 log artifact 要和 run 互相跳转；成本要按 GPU hours、存储、网络、eval calls 和 tokens 归因；状态机要区分 debug、candidate、evaluated、release candidate、rejected；搜索索引要覆盖 owner、model、dataset、commit、metric、tag 和 artifact。最后用 Experiment Tracking Gate 收束：元数据、参数、指标、日志、样本、代码、数据、环境、artifact、成本、状态、搜索、血缘和实验结论全部可验收，才算实验追踪系统可靠。

40ZZZY. 如何设计大模型评估平台，统一离线批量评测、人工评测和在线评测？
       考察点：eval dataset contract、offline batch reproducibility、metric definition versioning、LLM judge calibration、human review quality control、pairwise blinding randomization、online eval guardrail、release gate readiness、eval report completeness、slice regression detection、regression suite coverage、eval job scheduler reliability、eval cache key integrity、eval observability、artifact tracking linkage、evaluation platform gate。
       回答框架：先说明评估平台不是 benchmark 脚本，而是模型质量控制系统，要把模型版本、评估数据、prompt、推理配置、指标、人评、judge、在线反馈和发布决策连成可复现证据链。第一层讲对象模型：一次 EvalJob 至少绑定 ModelVersion、EvalDataset、PromptVersion、InferenceConfig、EvalCodeVersion、MetricDefinition、JudgeConfig 或 HumanReviewTask、PredictionOutputs、EvalReport 和 ReleaseDecision；这些字段不完整时，分数不可比较。第二层讲离线评测和数据：eval dataset 要记录 id、版本、任务、样本数、切片、标签、来源、构造方式、训练隔离和污染检查；离线批量评测要固定模型、数据、prompt、推理配置、评估代码、runtime 和指标定义；指标要记录方向、权重、阈值和版本，避免同名指标漂移。第三层讲人评和 judge：LLM-as-Judge 要有人类校准集、rubric、prompt、模型版本和长度 / 位置偏差检查；人工评测要有标注指南、gold set、多标注员重复标注、复审、仲裁和耗时监控；pairwise 要盲评、随机输出顺序、记录 tie 并按切片统计 win rate。第四层讲在线评估和发布：A/B 或 canary 要有稳定分桶、sample ratio health、质量 / 延迟 / 成本 / 安全 guardrail 和 rollback；Release Gate 要同时看离线、人评、在线、安全、性能、成本、审批和回滚。第五层讲平台能力：EvalReport 要包含总分、切片、样本级结果、失败案例和 baseline 对比；回归集要覆盖历史事故、关键业务、安全、工具和格式样本；评估调度要支持依赖、重试、预算和失败分类；缓存 key 必须包含模型、数据、prompt、配置、代码和 runtime；可观测性要覆盖状态、吞吐、错误率、成本、失败原因、队列和 trace；prediction outputs、sample table、judge run、人评结果、eval report 和 release decision 要进入 artifact / lineage。最后用 Evaluation Platform Gate 收束：数据、指标、judge、人评、在线、报告、回归、调度、缓存、观测、artifact、发布和回滚证据全部过线且无 P0 风险，才算评估平台可靠。

40ZZZZ. 如何设计 Feature Store、Embedding Store 与向量索引基础设施？
       考察点：feature definition contract、offline online consistency、point in time correctness、embedding version contract、chunk embedding lineage、vector index build readiness、ANN quality latency gate、metadata permission filter、shadow index switch readiness、retrieval trace completeness、feature embedding lineage graph、multi tenant isolation、quality monitoring metrics、cost capacity governance、RAG agent integration、feature embedding index gate。
       回答框架：先说明这不是单一向量数据库选型题，而是训练、推荐、搜索、RAG 和 Agent 共用的数据访问层设计题。第一层讲职责边界：Feature Store 管特征定义、实体键、offline / online 取值、transform、TTL、owner 和 point-in-time correctness；Embedding Store 管 embedding model、维度、pooling、normalize、预处理、版本、向量记录和 chunk / item 的 lineage；Vector Index 管索引版本、metric、HNSW / IVF / PQ 参数、构建任务、读写切换、metadata filter 和查询 trace。第二层讲一致性和防泄漏：offline / online 要比较同一实体同一时间窗口的值、类型、默认值和 transform 版本；训练样本只能读取 event time 之前的特征；chunk 要记录 source doc、offset、checksum、tenant、ACL 和数据等级，metadata filter 必须在召回、rerank、上下文构造和日志中持续生效。第三层讲索引和质量：索引构建前校验维度、metric、embedding version、删除 tombstone、增量边界和容量；ANN 不能只看延迟，要同时看 recall@k、P95 / P99、空结果率、过滤后候选数和 brute-force 抽样对照；shadow index 上线要有 canary、双读对比、rollback 和版本冻结。第四层讲平台治理：retrieval trace 至少记录 query id、query embedding version、index version、filter、candidate ids、scores、rerank、引用和最终上下文；lineage graph 要能从 source data / feature / chunk -> embedding -> index -> retrieval request -> answer / decision 追溯；多租户隔离、权限、质量监控、成本容量、构建队列和 RAG / Agent citation 都要进入同一张审计表。最后用 Feature Embedding Index Gate 收束：特征、embedding、chunk、索引、过滤、trace、血缘、租户、质量、成本、RAG / Agent 集成和回滚证据全部可验收，才算基础设施可靠。

40ZZZZA. 如何设计 RAG / Agent 平台中的知识库、工具和 trace 存储？
       考察点：knowledge base contract、document chunk version contract、sync delete propagation、ACL permission enforcement、retrieval trace completeness、prompt assembly trace、citation version binding、agent definition versioning、tool definition contract、tool permission gate、tool call trace completeness、execution trace replay readiness、memory privacy lifecycle、trace privacy retention、cost attribution governance、RAG agent storage gate。
       回答框架：先说明这不是“向量库加日志表”的题，而是 RAG / Agent 应用数据资产控制面设计题。第一层讲知识库对象：Knowledge Base 要有 tenant、owner、source connector、permission policy、embedding / chunking / index version、sync state 和 retention policy；Document 要有 source URI、document version、content hash、permission labels 和 parsing status；Chunk 要有 doc id、doc version、offset、embedding id、checksum 和 ACL；源系统删除、撤权和权限变化必须传播到 chunk store、vector index、citation store 和 cache。第二层讲 RAG trace：retrieval trace 要记录 query / rewrite、query embedding version、index version、filters、top-k、candidate ids、scores、rerank、final chunks、过滤原因和 latency；prompt assembly trace 要记录 system prompt version、template、chunk 顺序、token budget、截断和 prompt hash；citation 要把 answer span 绑定 chunk、document version、source URI、offset 和 index version。第三层讲 Agent 和工具：Agent Definition 要版本化 prompt、tool versions、model config、max steps、timeout、permission、memory 和 safety policy；Tool Definition 要版本化 name、description、input/output schema、endpoint、timeout、retry、permission requirement 和 side effect level；Tool Permission Gate 同时检查 user permission、agent permission、tenant、工具风险等级和 human confirmation；Tool Call Trace 要保存 run、step、tool version、参数、结果引用、状态、错误、权限决策、retry 和脱敏状态。第四层讲回放、隐私和成本：execution trace 应是 span tree，串起 retrieval、LLM、tool、memory、安全事件、artifact 和 final answer；回放要固定 model、prompt、agent、tool、tool output、KB、index 和 sampling 参数；Memory 要有 purpose、scope、TTL、用户可见性、删除和敏感写入阻断；trace 要支持字段级脱敏、访问控制、TTL、加密、删除请求和分层采样；成本要拆到 tenant、application、agent、run、model、tool 和 component。最后用 RAG Agent Storage Gate 收束：知识、工具、trace、memory、权限、隐私、成本、审计、删除传播和回滚证据全部可验收，才算平台存储可靠。

40ZZZZB. 如何设计 AI Infra 可观测性平台，统一 metrics、logs、traces 和 events？
       考察点：signal inventory coverage、metric contract completeness、SLO error budget readiness、latency quantile guard、trace span coverage、log event structure、correlation ID coverage、cardinality budget control、training observability、inference observability、data quality observability、RAG agent observability、cost observability、privacy redaction retention、alert actionability、observability platform gate。
       回答框架：先说明 AI Infra 可观测性不是监控曲线堆砌，而是让训练、推理、数据、模型、RAG、Agent、成本和平台事故可发现、可定位、可解释、可复盘。第一层讲四类信号：metrics 看趋势、阈值、SLO 和告警；logs 看错误细节、参数、堆栈和权限拒绝原因；traces 看一次请求或任务经过哪些阶段、慢在哪里、调用了哪些组件；events 看发布、回滚、扩容、降级、checkpoint、quality gate 等状态变化。第二层讲 AI Infra 专属指标：训练要看 loss、eval loss、learning rate、gradient norm、tokens/s、GPU、NCCL、data wait、checkpoint 和 retry；推理要看 QPS、TTFT、TPOT、p95 / p99、error、timeout、queue、active sequences、KV cache、cache hit、fallback 和 degradation；数据要看 ingestion、schema、PII、duplicate、corrupt shard、quality gate 和 data loader wait；RAG / Agent 要看 retrieval trace、prompt assembly trace、tool call trace、execution trace、loop rate、groundedness 和 cost per run。第三层讲平台治理：metric contract 要有 name、unit、labels、aggregation、owner、threshold；SLO 要有 SLI、窗口、目标、error budget 和 burn rate；metrics label 要控制 cardinality，request id、user id、prompt hash 进入 trace / logs，不进入 time series label；统一 trace id / run id / job id / model version / dataset version 把 metrics、logs、traces、events 和 costs 关联起来。第四层讲隐私和告警：prompt、检索片段、tool 参数和输出要脱敏、TTL、访问控制和审计；trace 对错误、慢请求和高成本请求全量保存，对普通成功请求采样；告警要有 owner、runbook、severity、影响面、去重和可能根因。最后用 Observability Platform Gate 收束：signals、SLO、trace、日志、事件、成本、隐私、告警、dashboard、incident process 和 postmortem 全部过线，才算可观测性平台可靠。

40ZZZZC. 如何系统定位大模型训练故障：loss 异常、hang、OOM、通信慢和 I/O 慢？
       考察点：training fault evidence coverage、lifecycle event timeline、rank log completeness、loss anomaly diagnosis、NaN Inf guard、update health check、hang straggler detection、OOM phase memory evidence、communication bottleneck diagnosis、IO data bottleneck diagnosis、checkpoint integrity、resume continuity、data fault isolation、config code diff readiness、minimal reproduction readiness、training fault diagnosis gate。
       回答框架：先说明训练故障定位不能只看最后一行 traceback，也不能先猜模型或框架，而要把故障当成证据链问题。第一层按现象分类：loss 不下降、loss spike、NaN / Inf、hang、OOM、step time 变慢、通信慢、data wait 高、checkpoint 保存失败、resume 后 loss 跳变；每类现象都要绑定 step、rank、batch、配置、数据版本和 checkpoint 状态。第二层讲排查顺序：先看 TrainingJob 生命周期事件和最早异常时间，再看所有 rank 日志、GPU / CPU / 网络 / 存储指标、loss / gradient / LR 曲线、step time 分解、NCCL 日志、dataloader queue、checkpoint manifest 和 config / code / dataset / environment diff。第三层讲关键判断：loss 异常要看异常 batch、label / mask、tokenizer、LR、precision、grad norm 和 optimizer step；hang 要找最早停止推进的 rank、heartbeat 和 collective wait；OOM 要先分 forward、backward、optimizer、eval、checkpoint 阶段；通信慢看通信占比、rank straggler 和网络错误；I/O 慢看 data wait、cache hit、worker、吞吐和坏样本。第四层讲恢复和复盘：checkpoint 不能只看文件存在，要校验 manifest、checksum、model、optimizer、scheduler、RNG 和 dataloader state；resume 要校验 global step、LR 和 loss 连续性；复杂问题要固定 seed、异常 batch、配置、数据版本和单机试验做最小复现。最后用 Training Fault Diagnosis Gate 收束：证据覆盖、时间线、rank 日志、数值、更新、hang、OOM、通信、I/O、checkpoint、resume、数据、diff、最小复现、owner、runbook 和 postmortem 全部过线，才算完成训练故障定位。

40ZZZZD. 如何系统定位大模型推理故障：TTFT、TPOT、吞吐下降和错误率上升？
       考察点：inference fault evidence coverage、request scope slice、trace stage coverage、TTFT decomposition、TPOT decode health、tail latency attribution、throughput token capacity、error taxonomy timeout stage、KV cache pressure guard、model artifact load readiness、streaming reliability、cache hit key governance、route trace correctness、tool dependency trace、quality release diff readiness、inference fault diagnosis gate。
       回答框架：先说明推理故障是在线用户直接感知的问题，不能只看 QPS、平均延迟或 GPU utilization，要把请求按 gateway、auth、router、cache、queue、tokenize、prefill、decode、streaming、safety 和 downstream 拆阶段。第一层判断影响范围：按 model、tenant、region、endpoint、runtime、streaming、input / output length bucket、release version 切分，区分全局故障、单模型故障、单租户流量突增、长上下文请求退化和发布后回归。第二层拆性能指标：TTFT 高重点看 gateway、router、cache miss、queue、tokenize、prefill、cold start 和 first flush；TPOT 高重点看 decode batch、active sequences、KV cache、显存带宽、runtime 版本和 sampling；p99 高看慢请求 trace、长短请求混跑、局部实例拥塞、tool call 和网络抖动；吞吐下降不能只看 QPS，要看 input tokens/s、output tokens/s、per GPU output tokens/s 和有效 batch。第三层拆错误和资源：错误率按 4xx、429、5xx、timeout、OOM、model load、tool failure、safety block 分类；timeout 必须记录 queue、prefill、decode、streaming idle 或 tool 阶段；KV cache 看 pressure、free blocks、eviction、admission reject 和 fragmentation；模型加载看 manifest、checksum、tokenizer、config、runtime / quantization compatibility、权限和 warmup。第四层讲版本和应急：cache hit 下降要看 prompt、tokenizer、model、safety、tenant permission 和 TTL / eviction；路由异常要看 candidate、selected、reason、canary、fallback 和 endpoint health；Agent 错误要看 tool schema、权限、timeout、retry 和下游状态；质量异常要把 model、prompt、retrieval、tool、safety、sampling 和 release event diff 关联到线上反馈。最后用 Inference Fault Diagnosis Gate 收束：证据、trace、容量、版本、应急动作、回滚、runbook 和 postmortem 全部过线，才算完成推理故障定位。

40ZZZZE. 如何设计 AI Infra 的 SLO、错误预算和生产值班体系？
       考察点：SLI contract completeness、SLO target measurability、SLA boundary clarity、inference SLO coverage、training SLO coverage、data eval SLO coverage、error budget accounting、burn rate alerting、alert actionability、oncall ownership escalation、incident severity routing、runbook executability、change event linkage、mitigation rollback authority、postmortem action closure、SLO cost tradeoff gate。
       回答框架：先区分 SLI、SLO 和 SLA：SLI 是实际观测指标，SLO 是内部目标，SLA 是对外承诺和责任边界。第一层定义指标：推理要看 availability、TTFT、TPOT、p99、error、timeout 和质量 / 安全 guardrail；训练要区分用户错误和平台错误，并看提交、调度、平台失败率、checkpoint 保存 / 恢复；数据和评估要看 dataset build、manifest、质量门禁、eval report 和 RAG / Agent success。第二层讲错误预算：用 `B=1-S_slo` 定义允许失败空间，用 budget used / remaining 决定常规灰度、减速、冻结高风险发布或只允许可靠性修复；用快慢窗口 burn rate 同时发现突发事故和慢性退化。第三层讲告警和值班：告警要围绕用户影响、SLO 和 burn rate，而不是围绕孤立资源阈值；每个高优告警要有 owner、runbook、severity、ACK SLO、升级链路和状态沟通。第四层讲事故处理：事故分级要结合用户影响、持续时间、服务重要性和风险；值班要有可审计的限流、降级、扩容、摘除、回滚和暂停队列权限；排查时先看最近模型、prompt、runtime、route、data、cluster、cache 变更。最后用 SLO Oncall Gate 收束：复盘必须有时间线、影响量化、根因、防线失效、行动项 owner、截止时间和回归验证；SLO 目标还要结合用户价值、风险降低、基础设施成本和值班成本，不能无脑追求 99.99%。

40ZZZZF. 如何设计 AI Infra 成本治理体系？
       考察点：usage metering coverage、cost attribution labels、GPU cost efficiency、training waste control、inference unit cost、cache savings accounting、storage lifecycle governance、network egress governance、artifact dependency safety、budget quota enforcement、cost anomaly alerting、tenant model chargeback、dashboard drilldown readiness、optimization recommendation trace、SLO quality cost tradeoff、cost governance gate。
       回答框架：先说明成本治理不是月底看账单，也不是简单砍资源，而是把资源消耗变成可观测、可归因、可预算、可优化的工程指标。第一层讲 usage metering：采集 GPU、CPU、memory、storage、network、request、token、artifact、logs、traces 用量，并保证 freshness。第二层讲 attribution：资源、任务、模型、endpoint、artifact 必须有 tenant、project、team、owner、model、job_type、environment 和 cost_center，未归因成本要被拦截或升级。第三层拆成本：GPU 看 GPU-hours、tokens per GPU hour、MFU、idle 和 failed job cost；训练看失败任务、无效实验、抢占丢失和 checkpoint / resume；推理看 cost per request、cost per 1k tokens、模型 / endpoint / tenant 分摊；缓存要计算节省成本减去维护成本；存储看 checkpoint、dataset、model、eval output、logs、traces 和 vector index 生命周期；网络看跨地域权重、数据、checkpoint、日志和远端索引访问。第四层讲治理动作：预算控制花多少钱，配额控制能用多少资源；成本异常告警要带 owner、recent change 和推荐动作；dashboard 要能下钻到 tenant、model、job、artifact、request slice。最后用 Cost Governance Gate 收束：优化建议必须有估算节省、证据、owner、回滚条件和质量 / SLO / 风险评估，不能为了降本牺牲用户体验、质量、安全和可恢复性。

40ZZZZG. 如何设计 AI Infra 资源利用率优化体系？
       考察点：resource metric contract、effective utilization accounting、fragmentation control、topology binpacking readiness、colocation SLO guard、preemption checkpoint safety、low priority reclaim policy、elastic training safety、inference capacity utilization、warm pool headroom policy、quota fairness borrowing、scheduler observability、dashboard drilldown readiness、reliability headroom guard、cost SLO tradeoff control、resource utilization gate。
       回答框架：先说明资源利用率优化不是把 GPU 打满，而是在优先级、SLO、隔离、公平性和成本约束下提高有效产出。第一层讲指标：区分 GPU allocation、GPU utilization、GPU memory、MFU、tokens/GPU hour、idle GPU hours、pending、queue wait、preemption、failed GPU-hours 和 fragmentation；再用 effective utilization 把失败任务、无效实验、SLO 失败请求和不可发布产物扣掉。第二层讲调度治理：碎片要按节点、GPU 型号、拓扑和任务规格计算；bin packing 要结合 GPU、NIC、rack、storage 和 gang scheduling；混部必须有 p99、step time、自动驱逐和关键任务隔离；抢占必须绑定 checkpoint 新鲜度、graceful shutdown、requeue、频率限制和 owner 通知；低优先级队列只能吃可回收余量。第三层讲弹性和推理：弹性训练要处理 world size、global batch、learning rate、optimizer state、dataloader 和 checkpoint；推理利用率要同时看 TTFT、TPOT、p99、KV pressure、continuous batching、autoscaling 和 warm pool。第四层讲公平和可靠性：quota borrowing 可以提高利用率，但要可回收、可补偿并受 dominant share 约束；利用率越高越要保留 headroom，避免故障恢复、突发流量和高优先级任务没有余量。最后用 Resource Utilization Gate 收束：dashboard 必须能按 cluster、GPU type、node、queue、tenant、team、priority、job type、model、endpoint 下钻，优化建议必须有 owner、节省估算、质量 / SLO / 风险评估、回滚条件和 P0 阻断。

40ZZZZH. 如何设计 AI Infra 安全治理体系？
       考察点：identity workload binding、RBAC ABAC policy fit、secret lifecycle management、data compliance classification、PII sensitive data control、tenant isolation boundary、model access control、model output safety gate、RAG permission enforcement、agent tool safety gate、supply chain integrity、model artifact integrity、runtime security boundary、log trace privacy governance、audit incident response readiness、security governance gate。
       回答框架：先说明 AI Infra 安全不是 API 鉴权中间件，而是覆盖身份、权限、密钥、数据、模型、RAG、Agent、供应链、运行时、日志、审计和响应的控制面。第一层讲身份和权限：人类用户、service account、训练任务、推理服务、Agent、CI/CD 和外部应用都要有 workload 绑定；RBAC 解决角色，ABAC 结合租户、数据等级、地域、项目、用途和模型等级做动态判断，默认拒绝并减少过度授权。第二层讲密钥和数据：密钥必须在 secret manager 中管理，使用短期凭证、最小权限、轮换、吊销和访问审计，不能进代码、镜像、命令行、日志、prompt 或 trace；数据要记录来源、授权、敏感等级、地域、用途、保留和删除 SLA，PII / 敏感信息要在数据、请求、日志、trace、评估和工具结果中检测、脱敏和阻断。第三层讲模型、RAG 和 Agent：模型调用、权重下载、部署、导出和 eval report 都要有 ACL 与审计；RAG 权限必须在检索和上下文组装前强制过滤，citation 和 trace 也不能泄露无权限来源；Agent 高风险工具必须有权限校验、参数校验、用户确认、限流、超时、审计和补偿机制。第四层讲供应链和运行时：镜像、依赖、模型权重、tokenizer、数据集和 runtime 要有 provenance、checksum、签名、扫描和许可证记录；训练 / 推理 workload 要非 root、只读文件系统、网络策略、egress 控制、最小 secret 注入、GPU 隔离和资源限制。最后用 Security Governance Gate 收束：日志 trace 要脱敏、字段级限权、TTL、加密和访问审计，关键事件审计要不可篡改，安全事件响应要能检测、隔离、吊销、定界、修复和复盘；任何 P0 风险、共享管理员身份、明文密钥、跨租户泄露、无签名 artifact 或无响应预案都应阻断上线。

40ZZZZI. 如何设计 AI Infra 的审计、日志留存、事故复盘和变更管理体系？
       考察点：audit event schema completeness、critical operation audit coverage、audit log integrity retention、log retention policy fit、log minimization redaction、trace privacy sampling、incident timeline completeness、impact detection response metrics、root cause systemic analysis、postmortem action item closure、change record completeness、high risk change approval、rollout rollback readiness、change freeze error budget policy、audit postmortem change linkage、governance loop gate。
       回答框架：先说明审计、留存、复盘和变更管理的目标，是让关键操作可追溯、关键证据可保留、事故能沉淀成系统改进、变更能控制风险。第一层讲审计：事件 schema 至少包含 timestamp、actor、tenant、action、resource、decision、status、request id、trace id 和 diff / hash；数据访问 / 导出、模型下载 / 发布、权限变更、secret 访问、高风险工具调用、知识库权限变更、发布、回滚和删除都要审计，deny 事件也要保留；审计日志要可靠写入、防篡改、加密、可查询、可导出，并且访问审计日志本身也要被审计。第二层讲留存和隐私：debug log、训练日志、推理请求日志、RAG / Agent trace、审计日志和安全事件日志要有不同 TTL、敏感等级、字段级权限、删除请求和成本预算；prompt、检索片段、工具参数、PII 和用户数据应在写入前脱敏或摘要化，普通成功 trace 可采样，错误、慢请求和高风险请求要保留更多证据。第三层讲事故复盘：复盘要有发生、检测、告警、响应、缓解、恢复、根因确认的时间线，并能计算 MTTD、MTTA、MTTR、影响范围和用户切片；根因分析要追到系统防线，不停在个人失误；行动项要有 owner、deadline、验证证据、回归测试和逾期跟踪。第四层讲变更管理：AI Infra 变更包括模型、prompt、runtime、路由、数据版本、权限策略、工具定义、RAG 知识库和 GPU 集群配置；每次变更要记录 owner、对象、原因、diff、风险、审批、灰度、回滚和观测指标；高风险变更要有审批、canary、rollback drill、owner 通知、变更冻结和错误预算约束。最后用 Governance Loop Gate 收束：audit log、change record、incident、postmortem、action item 和 regression test 必须能互相跳转；没有 owner、审计导出、dashboard、例外审批、P0 阻断或回归验证的治理体系，只是流程文档，不是平台能力。

40ZZZZJ. 如何设计一个企业级 LLMOps 平台？
       考察点：lifecycle boundary、application resource model、model gateway、prompt registry、RAG knowledge base、tool registry、Agent runtime、eval feedback loop、release manifest、trace observability、cost budget、security audit、multi-environment promotion、developer interface、production guardrail、LLMOps platform gate。
       回答框架：先说明企业级 LLMOps 平台不是模型部署平台，也不是 prompt 管理页面，而是大模型应用生命周期控制面。第一层做需求澄清：服务哪些团队，接哪些内部和外部模型，是否管理 prompt、RAG、Agent、工具、评估、发布、trace、反馈、安全、成本和多环境，是否需要 API / SDK / CLI / Console。第二层讲资源模型：Application 绑定 Environment、PromptTemplate、KnowledgeBase、ToolDefinition、AgentDefinition、EvalSuite、ReleaseManifest、Trace、Feedback、Policy 和 CostRecord；这些对象必须有 owner、version、stage、permission、lineage 和 audit。第三层讲核心模块：Model Gateway 统一模型路由、配额、日志和成本；Prompt Registry 管理模板、变量 schema、测试样例、评估、灰度和回滚；Knowledge Base / RAG Service 管理文档、chunk、embedding、索引、ACL、citation 和删除传播；Tool Registry / Agent Runtime 管理工具 schema、side effect level、权限、step limit、timeout、human confirmation 和 execution trace；Eval Platform 和 Feedback Loop 把离线评估、人评、LLM-as-Judge、A/B、用户反馈、bad case triage 和回归集更新连起来。第四层讲发布和治理：每次上线生成 release manifest，绑定模型路由、prompt、知识库、工具、Agent、安全策略和生成参数；dev -> staging -> production 通过评估、审批、灰度、guardrail 和 rollback；trace 覆盖 model call、prompt assembly、retrieval、tool call、agent step、safety event、feedback 和 cost；安全覆盖租户、模型、知识库、工具、trace、secret 和审计；成本按 app、tenant、user、model、KB、tool 和 agent run 归因。最后用 LLMOps Platform Gate 收束：资源模型、prompt / RAG / tool / agent 版本、评估回归、发布回滚、trace、反馈、权限、安全、成本、开发者接口和 P0 风险都可验收，才算企业级 LLMOps 平台完整。

40ZZZZK. 如何设计一个多租户 GPU 集群调度系统？
       考察点：tenant queue contract、quota borrowing policy、dominant fairness accounting、gang scheduling fit、topology aware placement、fragmentation control、preemption checkpoint safety、low priority reclaim、inference SLO isolation、heterogeneous GPU policy、resource snapshot health、placement explainability、cost utilization attribution、scheduler observability audit、trade-off boundary reasoning、multi tenant GPU scheduler gate。
       回答框架：先澄清集群规模、GPU 型号、训练 / 推理 / 评估比例、租户数量、队列和优先级、是否允许借用和抢占、训练是否需要 gang scheduling、推理是否有 SLO 隔离。第一层讲资源和任务模型：任务声明 tenant、queue、priority、GPU type、nodes、gpus per node、CPU、memory、storage、topology、checkpoint、preemptible 和 cost tag；集群快照记录 GPU 空闲量、健康、拓扑、MIG / sharing、队列占用和节点故障。第二层讲策略：Queue Manager 管 guaranteed / burst / borrowed quota；Policy Engine 用 priority、quota、dominant share 和 aging 选择任务；Placement Engine 用 gang feasibility、topology score、bin packing 和 fragmentation ratio 选节点；Preemption Engine 只抢占低优先级、可恢复、checkpoint 新鲜且释放资源足够的 victim。第三层讲运行治理：低优先级队列只吃可回收余量，推理服务用独立资源池或 SLO guardrail 隔离，异构 GPU 按显存、互联、任务适配和成本分配。第四层讲可解释和成本：每次 pending、placement、preemption 都记录候选、过滤原因、打分项、最终决策、影响和 owner；输出 queue wait、quota usage、dominant share、borrowed GPU、fragmentation、preemption loss、effective utilization、idle / failed GPU-hours 和 per-tenant cost。最后用 Multi Tenant GPU Scheduler Gate 收束：队列契约、借用回收、公平性、gang、拓扑、碎片、抢占、推理 SLO、异构策略、资源快照、解释、成本、观测、trade-off 和 P0 风险都可验收，才算完整系统设计。

40ZZZZL2. 如何准备和自评 AI Infra 面试高频问题？
       考察点：AI Infra interview readiness、topic coverage、formula coverage、demo evidence coverage、risk coverage、trade-off coverage、weak AI Infra question、revision plan、interview gate。
       回答框架：先把 AI Infra 面试题按训练平台、GPU 调度、训练故障、推理平台、TTFT / TPOT / KV cache、RAG / Agent、模型仓库、评估平台、可观测性、SLO、成本、安全和多租户分类；再检查每道回答是否同时覆盖目标、对象模型、核心链路、指标公式、demo 或项目证据、风险边界、成本安全可观测性和 trade-off。公式至少能写出 topic coverage、formula coverage、demo evidence coverage、risk coverage、trade-off coverage、平均得分和 red flag rate；demo 证据至少能指向 scheduler audit、inference platform audit、model registry release audit、eval platform audit、observability audit 和 security / cost audit。最后输出 weak questions 和 revision plan，把每个弱题绑定到一个缺失公式、一个可运行 toy demo、一个生产风险和一段标准回答，而不是只说“继续刷题”。

40ZZZZL3. LLM Serving Engine 到底是什么，和 `model.generate()` 有什么区别？
       考察点：serving engine request lifecycle、prefill / decode phase contract、KV cache footprint、scheduler batching readiness、streaming state management、engine metrics observability、runtime boundary clarity、cost capacity model、serving engine gate。
       回答框架：先说明 `model.generate()` 适合本地实验和单用户测试，而 LLM serving engine 是把离线生成函数变成高并发、低延迟、可流式、可调度、可观测在线执行系统。第一层讲请求生命周期：parse、tokenize、queue、scheduler、prefill、KV 分配、decode loop、sampling、streaming、finish / cancel / timeout 和 KV 释放都要可追踪。第二层讲核心阶段：prefill 主要决定 TTFT，decode 主要决定 TPOT，KV cache 随层数、活跃序列、上下文长度、KV heads、head dim 和 dtype 增长。第三层讲调度：continuous batching 不是简单增大 batch，而是在每轮 decode 动态加入、退出、取消和释放 KV，并用 token budget、max active sequences 和 admission control 控制显存压力。第四层讲工程边界：API server、request queue、scheduler、tokenizer、model executor、KV manager、sampler、streaming manager、metrics exporter 和 worker runtime 分工要清楚。最后用 Serving Engine Gate 收束：请求生命周期、prefill / decode、KV、scheduler、streaming、metrics、runtime 边界、成本容量和 P0 风险都过线，才算真正理解推理框架。

40ZZZZL4. 如何从零实现一个最小 LLM serving engine？
       考察点：naive generate loop、toy tokenizer、toy sampler、request state machine、waiting queue、running set、prefill / decode accounting、minimal scheduler、token streaming trace、KV cleanup gate、minimal engine gate。
       回答框架：先从单请求 generate loop 开始：tokenize prompt、prefill 建 cache、循环取 logits、sample next token、append token、遇到 EOS 或 max tokens 停止。然后把黑盒拆成对象：RequestState 记录 WAITING / PREFILLING / DECODING / FINISHED / ABORTED / FAILED，waiting queue 保存未执行请求，running set 保存正在 decode 的请求，finished set 保存完成请求。第三层加入最小 scheduler：按 max active sequences 和 KV token budget 做 admission，每轮 decode 后移除完成请求并释放 KV。第四层加入 streamer 和 metrics：每个 token 生成后立刻形成 chunk，记录 TTFT steps、TPOT steps、decode steps、tokens per step、event trace 和 KV cleanup。最后用 Minimal Engine Gate 收束：状态机、streaming chunks、prefill / decode 指标、KV 释放和事件 trace 全部过线，才算从 `generate()` 走到了最小推理框架。

40ZZZZL5. 一个 LLM 推理请求从进入 API Server 到释放资源，中间经历哪些生命周期阶段？
       考察点：request lifecycle object、parameter validation、tokenization、waiting queue、scheduler admission、prefill decode transition、streaming chunks、finish reason coverage、abort timeout cleanup、TTFT / TPOT metrics、KV cleanup、lifecycle gate。
       回答框架：先按链路回答：HTTP / gRPC / SSE 请求进入 API server 后，经过鉴权、参数校验、模型名映射、chat template、tokenization 和上下文长度检查，被封装成内部 request object；request object 至少要带 request id、tenant、model、input / output ids、sampling params、状态、到达时间、首 token 时间、完成时间、KV 资源引用、stream handle、finish reason 和 error。第二层讲队列和调度：请求先进入 waiting queue，scheduler 根据 max active sequences、KV token budget、优先级、deadline 和 prefill / decode 平衡决定是否 admission；第一次执行进入 PREFILLING，建立 KV cache 并得到首 token，之后进入 DECODING，每轮通常生成一个 token 并推送 streaming chunk。第三层讲指标：queue wait、prefill latency、TTFT、decode steps、TPOT、E2E、input / output tokens、KV blocks、finish reason 和 error type 都要记录，TTFT 主要拆 queue + tokenization + prefill + first decode / flush，TPOT 主要看 decode loop 和 streaming 链路。第四层讲异常路径：客户端断开要标记 ABORTED，从 waiting queue 或 running set 移除，停止 streaming，释放 KV cache 和 running slot；排队超时、首 token 超时、总超时和系统错误也要进入统一 cleanup，并返回 timeout / error / content_filter 等 finish reason。最后用 Request Lifecycle Gate 收束：状态机、正常和异常路径、TTFT / TPOT 指标、streaming 或 finish reason、KV cleanup 和 event trace 全部过线，才算真正理解推理请求生命周期。

40ZZZZL6. Prefill、Decode、KV Cache 和 Token Streaming 分别影响哪些推理指标？
       考察点：prefill phase accounting、decode phase accounting、TTFT decomposition、TPOT measurement、KV cache memory formula、KV pressure estimate、streaming backpressure signal、finish reason cleanup、phase gate。
       回答框架：先定义四个阶段：prefill 处理 prompt tokens，一次前向写入每层 KV cache 并得到首 token logits；decode 在已有 KV cache 上逐 token 生成；KV cache 保存历史 key / value，避免每轮重算历史 token；token streaming 把生成 token 增量解码并返回给客户端。第二层讲指标：TTFT 主要由 queue、tokenization、prefill 和 first decode / first flush 组成，长 prompt、cache miss、冷启动和 prefill batch 都会拉高 TTFT；TPOT 主要看 decode step time、active sequences、KV cache 读取、sampling 和 stream flush，长上下文和调度抖动会让输出一卡一卡。第三层讲 KV 公式：KV cache 显存粗略按 `2 * layers * active_tokens * kv_heads * head_dim * dtype_bytes` 估算，active tokens 是 prompt tokens 加 generated tokens，KV pressure 是已用 KV 显存除以 KV 预算；并发和上下文上来后，权重没变也会 OOM。第四层讲 streaming：streaming 不只是前端体验，还要处理增量 detokenization、Unicode 边界、stop sequence、SSE / WebSocket / gRPC、慢客户端 backpressure、客户端断开、usage 统计和 finish reason。最后用 Phase Gate 收束：prefill accounting、decode accounting、KV budget、streaming chunks、cleanup / finish reason 全部过线，才算真正理解这四个概念。

40ZZZZL7. 如何评估 LLM serving 的 TTFT、TPOT、吞吐、并发、显存和成本？
       考察点：TTFT decomposition、TPOT measurement、input output tokens per second、active sequence capacity、KV pressure、tail latency p95 p99、timeout rate、cost per 1k tokens、metric cost gate。
       回答框架：先说明不能只看 QPS，因为 LLM 请求的 input tokens、output tokens、streaming、prefill、decode 和 KV cache 占用差异很大；同样 1 QPS，短聊天和长上下文代码生成的系统压力可能差几个数量级。第一层讲用户体验指标：TTFT 拆成 queue、tokenize、prefill、first decode / first flush；TPOT 用 decode time 除以输出 token 间隔，流式体验还要看抖动、backpressure 和 finish reason；p95 / p99 比平均值更重要。第二层讲系统容量指标：压测报告必须给 input tokens/s、output tokens/s、total tokens/s、active sequences、waiting queue、KV pressure、admission reject、timeout rate、GPU utilization 和 memory，不把连接数等同于有效并发。第三层讲显存和并发：KV cache 按 `2 * layers * active tokens * kv heads * head dim * dtype bytes` 粗估，模型权重能放下不代表高并发能跑；active sequence capacity 要受 KV budget、headroom、上下文长度和临时 buffer 约束。第四层讲成本：cost per 1k tokens 可由 GPU-hour 成本除以有效 token 吞吐估算，但必须在 TTFT、TPOT、p99、错误率和质量 SLO 满足后再优化；大 batch 可能降成本但拉高 TTFT。最后用 Metric Cost Gate 收束：TTFT SLO、TPOT SLO、token throughput、KV budget、cost budget、tail latency 和 timeout rate 全部可验收，才算评估完整。

40ZZZZL8. 推理框架、推理平台和 AI Infra 的边界是什么？
       考察点：engine platform infra boundary、role coverage、interface contract coverage、metric handoff coverage、incident layer routing、cost capacity evidence、serving boundary gate。
       回答框架：先给三层定义：Serving Engine 负责模型执行路径，包括 request queue、scheduler、prefill / decode、KV cache、sampling、streaming、metrics 和 cleanup；Inference Platform 负责服务治理控制面，包括 API gateway、auth / quota、model registry、routing、rollout、SLO、observability、cost attribution、degrade / retry；AI Infra 负责底层资源，包括 GPU cluster、container runtime、Kubernetes 调度、driver / CUDA、网络、存储、节点健康、资源隔离和容量规划。第二层讲接口契约：platform 到 engine 至少有 request API、control drain、pause admission、reload / rollout；engine 到 platform 至少交接 TTFT、TPOT、queue length、KV pressure、tokens/s、error 和 trace；infra 到 platform / engine 至少交接 pod pending、GPU health、node ready、memory pressure、image pull 和网络存储状态。第三层讲指标公式：role coverage、interface coverage、metric handoff coverage 和 misroute rate 都要能算；平台扩缩容不能只看 QPS，而要看 token 长度、queue、KV、p95 / p99 和 warmup。第四层用事故例子收束：TTFT p99 变差要同时排查平台流量和路由、engine queue / prefill / KV、infra pod scheduling / GPU health；成本过高要同时看 model routing / cache / quota、engine batching / KV / quant / speculative decode 和 GPU 型号 / 利用率 / 容量。最后用 Serving Boundary Gate 收束：职责、接口、指标、事故归层和成本容量证据都过线，才算真正理解推理平台边界。

40ZZZZL9. 如何手写最小 tokenizer、model wrapper 和 generate loop？
       考察点：tokenizer contract、model wrapper contract、last logits selection、greedy select、append token、EOS stop、naive recompute work、minimal generate gate。
       回答框架：先把链路拆成 encode、forward、select、append、stop、decode：TokenizerWrapper 把 prompt 转成 token ids，并保存 EOS / special token 契约；ModelWrapper 接收 token ids，返回 logits，真实模型 logits 形状通常是 `[batch, seq_len, vocab_size]`；生成下一个 token 只看最后位置 logits，因为它表示“基于已有上下文预测下一个 token”。第二层写生成公式：自回归概率是逐 token 条件概率乘积，greedy 每轮取分数最高 token，然后把它 append 到序列，遇到 EOS 或 max new tokens 停止。第三层讲为什么 naive loop 慢：如果 prompt 长度是 `N`，生成 `M` 个 token，粗略重复计算量是 `sum(N + t)`；KV cache 后续会把历史 key / value 缓下来，减少重复计算。第四层讲工程注意点：推理时要 `eval()` 和关闭 autograd，token 保持在模型设备上，decode 要处理 special tokens，trace 要记录每轮 input length、next token、stop reason 和 metrics。最后用 Minimal Generate Gate 收束：tokenizer、model wrapper、last logits、stop condition、trace 和 naive recompute evidence 都过线，才算真正拆开了 `model.generate()`。

40ZZZZL10. greedy、temperature、top-k 和 top-p sampling 怎么实现，分别影响什么？
       考察点：stable softmax、temperature softmax、greedy argmax、top-k candidate filter、top-p nucleus filter、renormalization、seeded multinomial sampling、sampling gate。
       回答框架：先说明模型输出 logits，不是概率；要用 softmax 转成概率，工程上要先减最大 logits 避免指数溢出。Greedy 是直接取最大 logits，确定性强，适合 debug 和格式稳定任务，但可能重复、死板。Temperature 是把 logits 除以温度：低温让分布更尖锐，高温让分布更平坦，但不会提高模型能力，只会改变随机性。Top-k 是固定保留概率最高的 k 个 token，简单直观但不适应分布形状；top-p 是按概率排序后保留累计概率达到 p 的最小候选集合，能随分布尖锐或平坦动态变化。第二层讲实现顺序：取最后位置 logits，处理 temperature，再做 top-k / top-p filter，过滤后重新归一化，再 multinomial sample；temperature 为 0 通常单独走 greedy。第三层讲工程影响：sampling 参数影响输出质量、多样性、长度、是否命中 stop、decode 轮数、TPOT 和成本；serving engine 里每个请求可能有不同 sampling params、seed、logprobs 和 structured mask。最后用 Sampling Gate 收束：softmax 稳定、候选不过滤为空、top-k / top-p 逻辑正确、seed 在简单路径可复现、参数 trace 可审计，才算采样模块可靠。

40ZZZZL11. 如何实现最小版本的 KV Cache，为什么它能加速 decode？
       考察点：past key values contract、prefill cache build、decode cache append、last logits、attention mask、position ids、KV cache memory formula、KV cache gate。
       回答框架：先说明 KV Cache 保存每层历史 token 的 key / value，常见 shape 是 `[batch, num_kv_heads, seq_len, head_dim]`；历史 K/V 在 causal decoder 中不会因为后续 token 改变，所以可以复用。第二层讲流程：prefill 输入完整 prompt，建立初始 `past_key_values` 并取最后位置 logits；decode 阶段每步只输入最新 token 和已有 cache，模型只为新 token 计算 Q/K/V，把新 K/V 追加进 cache，再用最后位置 logits 采样下一个 token。第三层讲复杂度和显存：naive loop 对长度 `N` 的 prompt 生成 `M` 个 token，大致重复处理 `sum(N+t)` 个 token；KV cache 路径约是 `N+M` 个投影工作，但显存增加，公式粗略为 `2 * layers * active_tokens * kv_heads * head_dim * dtype_bytes`。第四层讲正确性：cache 路径和完整重算路径理论上应等价，异常时优先排查是否 decode 还传完整历史、cache 是否更新、attention mask 长度是否等于 cache 长度加新 token、position ids 是否递增、EOS 停止逻辑是否被破坏。最后用 KV Cache Gate 收束：prefill 建 cache、decode 追加 cache、输出等价、重复计算下降、显存估算和 trace 都可验收，才算真正实现了最小 KV Cache。

40ZZZZL12. 如何实现 batched prefill，左 padding 和右 padding 对首 token logits 有什么影响？
       考察点：batched prefill audit、padding side policy、attention mask contract、last real token logits、padding waste ratio、prefill token budget、batched prefill gate。
       回答框架：先定义 batched prefill：把多个 prompt tokenize 后 padding 成 `[batch, max_prompt_len]`，传入 `attention_mask`，一次 forward 得到每个请求的首 token logits 和初始 KV Cache。第二层讲 padding：attention mask 中 1 是真实 token、0 是 PAD；没有 mask，模型可能把 PAD 当成上下文。左 padding 通常让每个请求最后位置都是真实 token，所以 `logits[:, -1, :]` 可用；右 padding 下短请求最后位置可能是 PAD，必须用 `attention_mask.sum(dim=-1)-1` 找最后真实 token 并 gather logits。第三层讲成本：padding waste ratio 可写成 `(B * L_max - sum(L_i)) / (B * L_max)`；长短 prompt 混在一起会提高 TTFT、浪费 prefill compute 和 KV 空间，因此生产系统会做长度分桶、max prefill token budget、chunked prefill 或 packed prefill。第四层讲状态：batched prefill 返回 batch 维度的 `past_key_values`，还要把首 token、prompt length、attention mask、request id 和状态写回 request object。最后用 Batched Prefill Gate 收束：padding / mask 正确、last real logits 正确、padding waste 可量化、token budget 可控、KV cache shape 可解释，才算实现可靠。

40ZZZZL13. 如何实现 batched decode，finished 请求和 KV Cache 对齐怎么处理？
       考察点：batched decode audit、finished mask、active batch compaction、batch cache alignment、decode position tracking、decode row savings、batched decode gate。
       回答框架：先定义 batched decode：prefill 后每个请求已经有首 token 和初始 KV Cache，decode 阶段每轮把未完成请求的最新 token 组成 `[batch,1]`，带上对应 `past_key_values` forward，得到每个请求的新 token。第二层讲状态：每个 request 要保存 output ids、next token、prompt len、max new tokens、finished、finish reason 和 cache slot；停止条件必须逐请求判断，不能全 batch 共用一个计数器。第三层讲对齐：第 i 行 token 必须对应第 i 行 KV cache，compact active batch 时要用同样 active indices 选择 cache，并维护 batch row -> request id -> cache slot 映射；否则会出现输出串台。第四层讲效率：固定 batch 简单但 finished 请求仍参与 forward；compact batch 每轮只处理 active 请求，可用 `fixed_rows - compact_rows` 粗略量化节省。最后用 Batched Decode Gate 收束：finished mask 正确、position / attention mask 长度按请求增长、cache alignment 正确、输出和停止原因可复盘，才算 batched decode 可靠。

40ZZZZL14. 如何实现请求队列和一个最小 LLM scheduler？
       考察点：simple scheduler audit、waiting queue、running set、finished set、scheduler admission、max active sequences、scheduler token budget、queue wait、TTFT steps、scheduler trace、simple scheduler gate。
       回答框架：先说明 LLM serving 里的请求动态到达，不能等固定 batch 全部结束后再处理下一批，因此 scheduler 每轮都要看 waiting queue、running set 和资源预算。第一层讲状态：waiting queue 保存尚未 prefill 的请求，running set 保存已 prefill 且仍在 decode 的请求，finished set 保存结束请求；请求状态至少覆盖 WAITING、PREFILLING、DECODING 和 FINISHED。第二层讲准入：最小 scheduler 不能只按请求数 pop，而要同时检查 max active sequences 和本轮 token budget；如果当前 decode 请求数是 `D`，本轮 prefill prompt token 之和是 `P`，就要满足 `D + P <= max_batched_tokens`。第三层讲指标：每个请求记录 arrival、prefill start、first token 和 finish time，从而计算 queue wait、TTFT、E2E；每轮 trace 记录 arrived、decode、prefill、waiting、running、scheduled tokens 和 deferred reason。第四层讲 trade-off：decode 优先能稳定 TPOT / ITL，但可能增加新请求 TTFT；prefill 优先能降低首响，但长 prompt 会挤占已有流式输出。最后用 Simple Scheduler Gate 收束：所有请求进入终态，max running 不超限，token budget 不超限，deferred 可解释，queue wait / TTFT 和 trace 都可复盘，才算最小 scheduler 可靠。

40ZZZZL15. 如何实现 token streaming，为什么不能只 `print` 每个 token？
       考察点：token streaming audit、streaming event contract、incremental detokenization、first token chunk、finish event contract、stop sequence buffer、client cancellation cleanup、streaming backpressure、TTFT / TPOT、streaming gate。
       回答框架：先说明 token streaming 发生在 prefill 首 token 或 decode 新 token 生成之后、请求结束之前，把增量文本作为事件推给客户端；它不一定缩短总生成时间，但能降低用户感知等待，TTFT 看首个 chunk 何时可见，TPOT / ITL 看后续 chunk 间隔。第一层讲事件契约：不要直接 `print`，而要定义 StreamingEvent，至少包含 request id、event type、delta、finish reason、usage 和 error，并给每个请求一个 stream queue，让 engine 和 HTTP / SSE / WebSocket 输出层解耦。第二层讲 detokenization：单 token decode 不一定等于自然字符或完整词，BPE、前导空格、Unicode 和 special token 都可能让输出边界出错，教学版可用完整 decode 后和已发送文本做差分，生产版用增量 detokenizer。第三层讲结束和异常：finish event 必须存在；stop sequence 可能跨 token，需要小缓冲区避免把 stop sequence 前缀泄露；客户端断开要标记 cancelled、停止 decode、释放 KV 和 stream 状态；慢客户端会造成 backpressure，不能长期阻塞模型执行主循环。最后用 Streaming Gate 收束：首 token chunk、文本重构、finish event、stop sequence 隐藏、取消清理、背压检测和 trace 都过线，才算 streaming 可靠。

40ZZZZL16. 如何实现最小 HTTP API 服务，为什么不能让 handler 直接调用 `model.generate()`？
       考察点：minimal HTTP API audit、generate request contract、request validation gate、queue admission control、sync generate response、SSE frame format、streaming generate response、HTTP cancellation cleanup、OpenAI-compatible boundary、HTTP API gate。
       回答框架：先说明 API 层和 engine 层要解耦：API 层负责协议、JSON request body、参数校验、准入、连接生命周期、错误响应和流式写出；engine 层负责 request state、waiting queue、scheduler、prefill、decode、stream queue 和 KV cleanup。第一层讲请求契约：`prompt` 要是非空字符串，`max_new_tokens`、temperature、top-p、top-k 等 sampling params 要在合法范围内，非法输入返回 400；队列满或资源不足返回 429，不应继续压入 engine。第二层讲同步路径：`/generate` 把请求转成 RequestState 入队，后台 engine loop 执行，完成后返回 request id、text、finish reason 和 usage，但长请求会占用连接并增加超时风险。第三层讲流式路径：SSE 要使用 `text/event-stream`，事件按 `event:`、`data:` 和空行分隔，delta 和 finish event 都要从 engine stream queue 读取；客户端断开时 generator 退出，要把 abort 传回 engine，释放 KV cache / running slot，并留下 `client_cancelled` 或等价 finish reason。第四层讲兼容边界：OpenAI-compatible 不是只改 URL，还要处理 messages、chat template、Responses typed events、Chat Completions `choices.delta`、usage、tool call、错误 body 和 finish reason 语义。最后用 HTTP API Gate 收束：validation、admission、sync、stream、cancel cleanup 和 trace 都可验收，才算最小 HTTP API 可靠。

40ZZZZL17. LLM serving 压测为什么不能只看 QPS，如何分析 TTFT、TPOT、吞吐和显存？
       考察点：serving benchmark audit、TTFT metric、TPOT metric、E2E latency metric、token throughput report、queue length percentile、active request percentile、KV memory peak、allocated vs reserved memory、benchmark bottleneck classification、benchmark gate。
       回答框架：先说明 LLM 请求消耗的是 token 级资源，input length 决定 prefill，output length 决定 decode 轮数，KV cache 随 active tokens 增长；所以 QPS 不能脱离 workload、输入输出长度分布、并发和到达模式讨论。第一层讲指标定义：TTFT 从请求到达到客户端可见首 token，主要受 queue、tokenization、prefill 和首 token flush 影响；TPOT / inter-token latency 看后续输出 token 间隔，主要受 decode batch、KV 读取、显存带宽和 streaming backpressure 影响；E2E latency 是完整请求耗时，tokens/s 要分 input tokens/s、output tokens/s 和 total tokens/s。第二层讲 workload：报告必须写清模型、硬件、请求数、并发、固定并发 / 固定 QPS / 泊松 / burst / 阶梯模式、输入输出长度分布、streaming、sampling 参数和超时策略。第三层讲显存：总显存可拆成模型权重、KV cache、工作区和 runtime 开销；KV cache 粗略正比于层数、KV head、head dim、dtype bytes 和 active tokens；PyTorch 里 allocated 和 reserved 要区分，reserved 高不一定是泄漏。第四层讲归因：TTFT 高但 TPOT 正常优先查 queue、prefill、长 prompt、scheduler 和 prefix cache；TPOT 高但 TTFT 正常优先查 decode batch、KV 读取、显存带宽和 stream queue；显存持续上涨查 cleanup 和引用释放。最后用 Benchmark Gate 收束：trace、latency、throughput、KV peak、cleanup 和 bottleneck classification 都能复盘，才算压测有效。

40ZZZZL18. vLLM 主要解决什么问题，为什么不能只说 PagedAttention？
       考察点：vLLM motivation audit、naive KV reservation、paged KV block allocation、KV waste ratio、logical physical block mapping、block reuse after cleanup、static decode rows、continuous decode rows、scheduler、block manager、vLLM motivation gate。
       回答框架：先说明 vLLM 是 LLM serving engine，不只是调用模型的 HTTP 包装；它主要解决高并发生成中的 KV cache 管理、动态请求调度、吞吐、显存利用和请求生命周期问题。第一层讲 naive 痛点：如果每个请求连续预留 `prompt + max_new_tokens` 的 KV 空间，实际输出短、请求提前结束或长短请求混合时都会造成显存浪费和碎片；fixed batch decode 还会让短请求结束后继续占空行，新请求也难以及时加入。第二层讲 PagedAttention：把 KV cache 切成固定 block，用 block table 把逻辑 token 位置映射到物理 block，减少连续分配要求，降低浪费，finished / cancelled 请求释放 block 后可复用，也为 prefix sharing 打基础。第三层讲 continuous batching：每个 iteration 动态移除完成请求、加入新请求，并结合 token budget 和 cache budget 决定 prefill / decode，从而提升 GPU 有效工作比例，同时控制 TTFT / TPOT。第四层讲系统边界：vLLM 还包括 scheduler、block manager、worker / executor、model runner、sampler、streaming、OpenAI-compatible server、metrics 和配置；生产还需要平台层做路由、限流、灰度、观测和成本治理。最后用 vLLM Motivation Gate 收束：KV waste 降低、block reuse、continuous batching 行数减少、cleanup 和 metrics 都能解释，才算真正理解 vLLM 解决的问题。

40ZZZZL19. PagedAttention 的核心思想是什么，block table 如何把逻辑 token 映射到物理 KV block？
       考察点：paged attention core audit、logical block、physical KV block、block table address translation、block size tradeoff、internal block waste、prefix block ref count、freed block reuse、continuous batching、paged attention gate。
       回答框架：先说明 PagedAttention 的核心不是单纯“attention 更快”，而是把 KV cache 从连续大张量管理改成分页式 block 管理；请求看到的是连续 token 序列，物理 KV blocks 可以不连续。第一层讲地址翻译：给定 block size `S` 和 token position `t`，先算 logical block `t // S` 和 offset `t % S`，再查请求的 block table 得到 physical block，attention kernel 根据 physical block + offset 读取历史 K/V。第二层讲收益：请求不必一开始预留最大长度，增长到新 block 时再分配；请求完成或取消后释放 blocks，空闲 blocks 能被其他请求复用；这降低预留浪费和碎片，也让 continuous batching 中请求动态加入 / 退出更容易。第三层讲 prefix sharing：多个请求共享相同前缀时，可以引用相同 physical blocks，用 ref count 或等价机制避免提前释放，分叉后再单独分配后续 block，接近 copy-on-write 直觉。第四层讲 trade-off：PagedAttention 仍有最后一个 block 的内部浪费；block size 小会减少内部浪费但增加 block table 长度、地址映射和 kernel 访问开销，block size 大则相反。最后用 PagedAttention Gate 收束：地址翻译、block size 取舍、内部浪费、prefix ref count、释放复用和 scheduler 配合都能解释，才算真正理解 PagedAttention。

40ZZZZL20. KV Cache Block Manager 负责什么，为什么 scheduler 必须查询它？
       考察点：KV block manager audit、block free list、block reference count、prefill block admission、decode block extension、block allocation failure、shared prefix cleanup、block manager metrics、block manager gate。
       回答框架：先说明 Block Manager 是 PagedAttention 的资源控制面，负责管理 GPU 上的 physical KV blocks，包括分配、扩展、释放、复用、引用计数、block table 维护和指标。第一层讲 prefill：prompt token 数先换算成所需 blocks，如果 free blocks 不够，scheduler 应让请求继续等待或拒绝，而不是调进去后 OOM；这比只按请求数限流更准确。第二层讲 decode：每生成一个 token，sequence length 增加；如果当前 logical block 满了，Block Manager 分配新 physical block 并追加到 block table；如果没有 free block，要触发准入失败、等待、抢占、swap 或重算策略。第三层讲 cleanup：FINISHED、ABORTED、FAILED 都必须释放 blocks；如果 prefix sharing 存在，要用 ref count，只有引用数归零的 block 才能回 free list，避免提前释放共享 prefix。第四层讲指标：至少看 total / free / used blocks、block utilization、allocation failures、blocks per request、ref count 分布、shared blocks 和 OOM / preemption 次数。最后用 Block Manager Gate 收束：prefill admission、decode extension、ref count、cleanup、allocation failure 和 metrics 都可复盘，scheduler 才能可靠控制 KV 容量。

40ZZZZL21. Continuous Batching 和 iteration-level scheduling 到底在调度什么？
       考察点：continuous batching audit、iteration level scheduling、dynamic request join、dynamic request exit、decode first scheduling、continuous token budget、scheduler KV budget、deferred scheduling reason、static batch hole、continuous batching gate。
       回答框架：先说明 LLM 生成是 multi-iteration workload，请求生命周期持续多轮，但 execution batch 只应该存在一个 iteration；iteration-level scheduling 就是在每轮模型执行前重新选择哪些 running 请求 decode、哪些 waiting 请求 prefill，并移除 finished / cancelled 请求。第一层讲 static batch 的问题：固定 batch 中短请求完成后会留下空洞，新请求必须等长请求结束，TTFT 和 GPU 利用率都会变差；可以用 `|S| * max(output_len) - sum(output_len)` 解释 Static Batch Hole。第二层讲一轮 scheduler：先接收动态到达请求和 cleanup，再 decode-first 给已有 streaming 请求进度，随后用剩余 token budget 接纳 waiting prefill；每轮 `decode_tokens + prefill_tokens <= max_num_batched_tokens`，不能只看 request count。第三层讲 KV budget：prefill 需要 prompt blocks，decode 在 block 边界可能追加 block；scheduler 必须查询 Block Manager 的 free blocks 和 append 能力，否则会出现调度后 OOM 或状态半更新。第四层讲取舍：decode-first 稳定 TPOT，但可能增加新请求 TTFT；prefill-first 降低首响但可能让流式输出卡顿；长 prompt 通常要靠 chunked prefill、优先级和隔离策略缓解。最后用 Continuous Batching Gate 收束：dynamic join / exit、decode-first、token budget、KV budget、cleanup、deferred reason 和 metrics trace 都可复盘，才算理解 continuous batching。

40ZZZZL22. 一个请求进入 vLLM-like engine 后，调度链路如何闭环？
       考察点：vLLM scheduler flow audit、engine request contract、request state machine trace、scheduler output metadata、model runner execution metadata、slot mapping metadata、output processor finish gate、abort cleanup path、request flow metrics、request flow gate。
       回答框架：先说明请求调度流程是一条状态和资源链路，不是某个类名：API server 或 Python LLM 接口接收请求，input processor 做参数校验、chat template 和 tokenization，构造成 engine request 后进入 waiting queue；engine core 每轮运行 scheduler，结合 waiting/running、token budget、sequence budget 和 KV block budget 产出本轮执行计划。第一层讲状态机：请求至少经历 WAITING、PREFILL、RUNNING / DECODING、FINISHED / ABORTED；arrival time、prefill start、first token、finish time、finish reason 都要记录，否则 queue wait、TTFT、E2E 和 cleanup 无法复盘。第二层讲 scheduler output：不能只输出 request id，还要给 model runner 准备 input ids、positions、block table、slot mapping、phase、sampling metadata 和本轮要分配/追加/释放的 blocks；metadata 不一致会导致 attention 读写错误位置。第三层讲 output processor：采样后要 detokenize、stream delta、检查 EOS、stop string、max tokens 和 logprobs；stop 命中或客户端断连要反馈给 engine core，让请求退出后续调度。第四层讲 cleanup：FINISHED、ABORTED、FAILED、超时和连接断开都必须释放 KV blocks、streaming buffer、请求状态和 metrics 引用，不能只在 EOS 路径释放。最后用 Request Flow Gate 收束：input、state、scheduler、metadata、output、cleanup 和 metrics 都可验收，才算讲清 vLLM 请求调度链路。

40ZZZZL23. vLLM memory management 如何管理显存、KV Cache、prefix cache 和 preemption？
       考察点：vLLM memory management audit、GPU memory breakdown、KV page size、block pool、free queue、prefix cache hash、cached free block、prefix cache hit rate、LRU cache eviction、preemption recompute path、hybrid KV cache group、memory management gate。
       回答框架：先说明 LLM serving 的 GPU memory 不只有模型权重，还包括 KV Cache、workspace、CUDA graph、sampling / logits buffer、LoRA adapter、多模态输入和 reserved memory；其中 KV Cache 最动态，会随 active tokens、prompt 长度、输出长度和并发增长，是 OOM 和吞吐瓶颈的核心。第一层讲 block pool：vLLM 用 PagedAttention 思路把 KV Cache 切成固定 physical blocks，每个请求维护 block table，logical token 连续但 physical blocks 可以不连续；page size 由 block size、层数、KV heads、head dim 和 dtype bytes 决定。第二层讲 prefix cache：只缓存 full blocks，block hash 要包含 parent hash、block tokens 和 LoRA / 多模态 / cache salt / 租户等 extra hashes；相同 tokens 但上下文或 extra hash 不同不能复用。第三层讲 free queue 和 ref count：finished 请求释放 block 时，如果 block 还有其他请求引用，ref count 只减 1；如果 ref count 到 0，block 可进入 free queue，但它可能仍然是 cached free block，后续命中可复用，内存紧张时再通过 LRU eviction 覆盖。第四层讲内存压力：当 free blocks 不足时，可以等待、拒绝、preemption、swap/offload 或 recompute；preemption 是保护机制，频繁发生通常说明 KV cache budget、`max_num_seqs`、`max_num_batched_tokens` 或上下文长度配置不合理。第五层讲混合模型：full attention、sliding window、state space layer 的 cache 需求不同，需要 Hybrid KV Cache Group 或等价分组管理。最后用 Memory Management Gate 收束：pool、prefix hash、hit rate、ref count、cached free、eviction、preemption、cleanup 和 metrics 都能复盘，才算讲清 vLLM 显存管理。

40ZZZZL24. vLLM 的 engine core、executor、worker 和 model runner 如何分工？
       考察点：vLLM executor architecture audit、engine core dispatch loop、executor dispatch layer、worker rank mapping、model runner metadata gate、tensor parallel merge、CPU process budget、worker failure recovery、executor architecture gate。
       回答框架：先说明这不是“scheduler 直接调用 model”这么简单，而是 API、engine core、executor、worker、model runner 和 model 的职责分层。第一层讲 engine core：它维护 waiting/running request state，运行 scheduler，管理 KV cache，处理 abort / cleanup，把本轮 scheduler output 交给 executor；它决定谁跑，但不应该直接承担 GPU forward。第二层讲 executor：它是派发层，屏蔽单 GPU、多进程、Ray、multiprocessing、TP/PP/DP 等执行拓扑，把一个 execution plan 转成 worker inputs，并收集 / merge worker outputs；executor 不应该做业务调度策略。第三层讲 worker：通常一进程一 GPU，负责设置 device、rank/local rank、通信组、加载模型、持有本地 KV cache 和 model runner；rank、local rank、device 映射错会导致 shard 错位或 collective hang。第四层讲 model runner：它把 request-level scheduler output 变成 tensor-level metadata，包括 input ids、positions、block tables、slot mapping、attention metadata、sampling metadata，再调用模型 forward；metadata 缺失或不一致会造成 KV 读写错位。第五层讲并行和资源：TP workers 共同处理同一 batch 的不同权重 shard，需要 all-reduce / merge；PP 按层传 hidden states；DP 复制 engine/worker group 处理不同 batch；CPU 也要覆盖 API server、engine core 和每个 worker 的控制路径。最后用 Executor Architecture Gate 收束：layer boundary、dispatch、runner metadata、rank mapping、merge、CPU budget、worker failure 和 trace 都可验收，才算讲清执行架构。

40ZZZZL25. Prefix caching 和 prompt cache 有什么区别，vLLM-like runtime 如何避免错误复用？
       考察点：prefix caching audit、prompt cache boundary、full block prefix reuse、parent hash chain、extra hash isolation、cache salt isolation、cached free touch、prefix cache eviction、prefix saved prefill tokens、prefix cache gate。
       回答框架：先区分层次：prompt cache 是平台层复用 prompt 前缀处理结果的宽泛概念，可以缓存 tokenized prompt、中间结果、KV blocks 或业务结果；prefix caching 是 runtime 内部更具体的机制，复用已经计算过的 KV cache full blocks。第一层讲收益：它主要减少重复前缀的 prefill token 数，所以改善 TTFT 和 prefill 吞吐，对 TPOT 主要是间接影响。第二层讲 full block：vLLM-like runtime 只缓存 full blocks，命中长度要按 block size 向下取整，partial suffix 仍要重新 prefill。第三层讲 hash：block hash 不能只看当前 block tokens，还要包含 parent hash，把整条 prefix 链编码进去；extra hashes 还要包含 model / tokenizer、LoRA、多模态媒体 hash、cache salt 和其他会改变 forward 结果或隔离边界的因素。第四层讲生命周期：命中 cached-free block 后要 touch，从 free queue 移除并增加 ref count；请求结束后 block 可变成 cached-free，显存紧张时再按 LRU eviction 移除 hash entry 并复用物理 block。第五层讲安全和指标：多租户默认用 cache salt 隔离，避免 TTFT 侧信道；线上要看 prefix cache hit rate、saved prefill tokens、cached-free blocks、eviction count、TTFT by hit length 和 cache miss 原因。最后用 Prefix Cache Gate 收束：hash、full block、touch、ref count、salt / LoRA / media isolation、eviction 和 metrics 都可复盘，才算实现可信 prefix caching。

40ZZZZL26. vLLM serving 中 TP、PP、DP、EP 怎么选？
       考察点：serving parallelism audit、tensor parallel topology、pipeline parallel bubble、data parallel replica、expert parallel placement、worker group size、per rank memory fit、cross node TP risk、prefix cache locality、parallel serving gate。
       回答框架：先说明并行选择要从瓶颈出发，而不是 GPU 越多越好。第一层讲 TP：Tensor Parallel 切单层矩阵、attention heads 或 MLP projection，主要解决单卡权重显存和单层计算分布；TP 高频 collective 会直接影响 decode TPOT，所以最好放在同机 NVLink / NVSwitch 范围内，跨节点 TP 通常是高风险配置。第二层讲 PP：Pipeline Parallel 按层切模型，适合单节点放不下或 TP 跨节点通信太贵的情况；跨节点常见做法是 TP 等于每节点 GPU 数、PP 等于节点数，但要解释 pipeline bubble、stage imbalance 和单请求延迟变长。第三层讲 DP：Data Parallel 在 serving 中通常是复制 engine / worker group / KV cache 处理不同请求，解决吞吐扩展，不解决单副本放不下；每个 DP rank 有独立 KV cache 和 prefix cache，所以路由策略会影响 cache locality 和 p99。第四层讲 EP：Expert Parallel 面向 MoE，把 experts 分布到不同 GPU，降低单卡 expert 权重，但会引入 token routing、all2all 和 expert 负载不均。第五层讲验收指标：总 GPU 数 `DP*TP*PP`、worker group size、per-rank weight / KV / buffer 显存、TPOT、PP bubble、DP throughput、prefix cache hit rate by rank、expert imbalance、CPU 进程预算和网络拓扑都要可观测。最后用 Parallel Serving Gate 收束：fit、topology、TP、PP、DP、EP 和 metrics 都过线，才算配置合理。

40ZZZZL27. vLLM 性能调优应该怎么系统排查，而不是盲目调 batch？
       考察点：vLLM performance tuning audit、TTFT bottleneck diagnosis、TPOT bottleneck diagnosis、KV pressure tuning、preemption rate、prefix cache effectiveness、config trade-off plan、rollback guard、vLLM tuning gate。
       回答框架：先说明调优顺序是定义指标、拆请求生命周期、定位瓶颈、改一个主要变量、压测对比、灰度上线和持续观测，而不是直接调大 batch 或加 GPU。第一层讲指标：TTFT 拆 frontend、tokenize、queue、prefill、first flush；TPOT 拆 decode、sampling、detokenize、streaming flush；吞吐看 input / output tokens/s；显存看 KV blocks、free blocks、allocation failures、preemption 和 GPU memory；缓存看 prefix hit rate、saved prefill tokens、cached-free blocks、eviction 和 DP route locality。第二层讲现象排查：TTFT 高先看 queue、input length、prefill tokens/s、prefix cache hit、tokenizer / RAG 前置链路；TPOT 高看 decode step、running batch、长 prefill 是否混跑、KV 读取、TP 通信和 streaming；OOM / preemption 看 `max_model_len`、`max_num_seqs`、`max_num_batched_tokens`、KV pressure、prefix cache 占用和 cleanup。第三层讲参数 trade-off：`max_num_batched_tokens` 调大可能改善 prefill / TTFT 但伤害 TPOT；`max_num_seqs` 调大提高并发但增加 KV 压力；`gpu_memory_utilization` 调大增加 KV 空间但减少安全余量；`max_model_len` 要按业务分层；TP/PP/DP 要结合拓扑和 cache locality。第四层讲实验和回滚：真实流量 replay 覆盖短 prompt、长 RAG、多轮、长输出和峰值并发；每次只改一个主要变量，同时看 p50/p95/p99、tokens/s、preemption、error / timeout、质量和成本；灰度要有 p99、OOM、error rate 和质量回滚阈值。最后用 vLLM Tuning Gate 收束：metrics、TTFT、TPOT、KV、cache、config trade-off 和 rollback 都可验收，才算完成调优方案。

40ZZZZL28. SGLang 主要解决什么问题，为什么不能只说成另一个 vLLM？
       考察点：SGLang motivation audit、complex LLM program、frontend language、RadixAttention motivation、radix prefix reuse、structured decoding retry saving、OpenAI-compatible API boundary、SGLang motivation gate。
       回答框架：先说明 vLLM 和 SGLang 的问题意识不同：vLLM 更强调高效服务大量独立或半独立请求，核心是 PagedAttention、continuous batching、KV block manager、scheduler 和 serving throughput；SGLang 更强调复杂 LLM programs 的表达和执行效率，这类程序包含多次 generation、变量引用、fork / parallel branch、choices、multi-turn、tool / RAG / agent 状态和 structured output。第一层讲表达：普通 OpenAI-compatible API 适合单次 chat / completion，但 runtime 只看到一批独立请求，很难知道哪些调用共享同一个长文档、few-shot examples、对话历史或中间 reasoning path；SGLang frontend language 用 `gen`、`fork`、choices、Python control flow 和约束输出让程序结构对 runtime 更可见。第二层讲执行：复杂程序中的共享前缀常呈树状，RadixAttention 用 radix tree 做最长前缀匹配和 KV cache reuse，能把 naive prefill 的重复计算变成只 prefill 新增 suffix；这对 self-consistency、Tree-of-Thought、agent、RAG pipeline、多轮对话和 structured extraction 特别有价值。第三层讲结构化输出：JSON / regex / EBNF constrained decoding 是 runtime 能力，不是输出后 parse；它减少解析失败、修复和重试带来的 token 成本和尾延迟。第四层讲边界：SGLang 也支持 OpenAI-compatible API，但 API 兼容是接入形态，frontend language 和 runtime 协同才是核心；短 prompt 单次生成、无共享前缀、无结构化输出的场景收益可能有限。最后用 SGLang Motivation Gate 收束：program structure、radix reuse、branch visibility、structured decoding、API boundary 和 metrics 都说清，才算答对。

40ZZZZL29. SGLang Runtime 的核心链路怎么画，哪些模块必须说清？
       考察点：SGLang runtime audit、runtime entrypoint unification、request state contract、Radix prefix lookup、KV memory pool budget、prefill decode scheduler、model runner boundary、grammar mask step、streaming event trace、SGLang runtime gate。
       回答框架：先把入口画清楚：OpenAI-compatible API、native `/generate`、offline engine 和 SGLang frontend program 入口不同，但最终都要落到 runtime 的 request state 和执行后端。第一层讲 request state：它不是字符串，而要包含 token ids、prompt length、generated tokens、sampling params、stop 条件、streaming 标记、structured output / grammar state、phase、KV slot、prefix hit、finish reason 和 error / cancel 状态。第二层讲调度链路：API server 解析请求和 chat template，tokenizer 产出 token ids；scheduler 从 waiting queue 和 running batch 里按 prefill / decode token budget、KV budget、输出长度和 structured output 开销选择请求；prefill 前 RadixAttention 做 longest prefix lookup，只对未命中 suffix 做 prefill；memory pool 分配 KV cache slot，并和 radix cache 的保留 / eviction 协同。第三层讲执行链路：model runner 把 scheduler 结果转成 input ids、position ids、attention metadata 和 KV read/write 位置，attention backend 负责高效执行 prefill / decode，sampler 处理 temperature、top-p、top-k、penalty 和 stop；如果有 JSON schema、regex 或 EBNF，grammar backend 在 sampler 前生成合法 token mask。第四层讲输出和指标：streaming 是 decode loop 里的增量事件，要处理 detokenize、finish reason、usage、客户端断开和 backpressure；指标要按 entry、queue、prefill、decode、KV、cache、grammar、stream 拆 TTFT / TPOT / E2E。最后用 SGLang Runtime Gate 收束：entrypoint、state、radix、memory、schedule、grammar、stream 和 metrics 都能验收，才算讲清 runtime。

40ZZZZL30. RadixAttention 是什么，为什么它不是 attention kernel？
       考察点：RadixAttention prefix sharing audit、compressed prefix tree、longest prefix match、radix tree split、page aligned prefix hit、radix KV ref count、leaf LRU eviction、cache aware scheduling cost、RadixAttention gate。
       回答框架：先纠正概念：RadixAttention 不是新的 attention 数学公式，也不是 FlashAttention 替代品，而是 SGLang Runtime 中自动复用 KV cache 的 runtime 机制；真正执行 attention 的仍是 attention backend / kernel。第一层讲数据结构：RadixAttention 用 compressed prefix tree / radix tree 管理 token prefix 到 KV cache slot 的映射，边上是 token segment，节点关联 KV refs、children、last access、ref count 和占用 token 数。第二层讲核心操作：新请求 tokenize 后先做 longest prefix match，找到已缓存的最长 token prefix，只对未命中的 suffix 做 prefill；插入新路径时，如果新旧路径在边中间分叉，就 split 出共享中间节点，再挂新 suffix；这比完整 prompt key 更适合 self-consistency、Tree-of-Thought、多轮 chat、agent 和共享长文档 RAG 的树状共享。第三层讲 page / block 边界：底层 KV cache 常以 page 或 block 为粒度，只有完整 page / full block 才能稳定复用，page size 越大越可能牺牲细粒度 prefix hit，但可能有 kernel 性能收益。第四层讲正确性：复用不仅要求 token prefix 一致，还要求模型、tokenizer、chat template、position 规则、LoRA、多模态输入、prompt adapter、KV layout 和租户隔离条件一致；active request 引用的节点要用 ref count pin 住，eviction 只能选 ref count 为 0 的叶子或低价值节点。第五层讲调度和指标：scheduler 要按 `prompt_len - hit_len` 估算 prefill 成本，而不是只看原始 prompt 长度；指标看 hit length、saved prefill tokens、reuse ratio、cached tokens、eviction、TTFT 和 prefill tokens/s。最后用 RadixAttention Gate 收束：match、split、page、ref、evict、schedule 和 metrics 都可验收，才算讲清。

40ZZZZL31. SGLang scheduler 怎么设计，为什么不能只说 decode-first？
       考察点：SGLang scheduler audit、decode first scheduling、cache aware admission、scheduler token budget、scheduler sequence budget、scheduler KV admission、suffix cost scheduling、scheduler aging fairness、chunked prefill scheduling、scheduler abort cleanup、SGLang scheduler gate。
       回答框架：先说明 scheduler 是每个 engine step 的资源分配中枢，不是简单排序器。第一层讲输入输出：输入包括 waiting queue、running requests、RadixAttention 命中、KV memory pool、token budget、sequence budget、structured output / grammar state、streaming / abort 状态和调度策略；输出不是 request ids，而是 prefill work、decode work、KV allocation、cached prefix refs、eviction / deferred reason 和 execution metadata。第二层讲 decode-first：running 请求已经占用 KV，decode 影响 TPOT 和 streaming 平滑度，所以通常先推进 decode，但必须保留 prefill 机会，否则 waiting 请求 TTFT 会恶化。第三层讲 cache-aware admission：waiting 请求要先做 prefix match，把 `prompt_len` 转成未命中 suffix cost；一个长 prompt 如果命中大部分 prefix，可能比短 cache miss 请求更便宜。第四层讲三类预算：token budget 约束本轮 prefill + decode token 数，sequence budget 约束 batch 中请求数，KV budget 约束新增 slots 和可安全 eviction；KV 不够时只能淘汰 ref count 为 0 且未被本轮 pin 住的 cached nodes。第五层讲公平性和复杂请求：cache hit bonus 要配合 aging，避免 cache miss 饥饿；长 suffix 用 chunked prefill 让 decode 穿插；JSON schema / regex / EBNF 等 structured output 会带来 grammar mask 开销和不同结束条件；abort / finish 必须释放 active KV refs。最后用 SGLang Scheduler Gate 收束：decode、token、seq、suffix、KV、fairness、chunk、grammar、cleanup 和 metrics 都能验收，才算讲清。

40ZZZZL32. Constrained decoding 如何保证结构化输出，为什么它不保证事实正确？
       考察点：Structured generation audit、toy constrained decoder、grammar state、valid token mask、JSON schema constraint、regex choice constraint、EBNF grammar constraint、structural tag constraint、empty valid token set、format not fact gate、structured streaming partial、structured generation gate。
       回答框架：先说明 structured generation 不是在 prompt 里写“请输出 JSON”，而是 runtime 在每步 decode 前把格式约束转成合法 token 集合。第一层讲核心流程：请求里传入 JSON schema、regex、EBNF、structural tag 或 choices；grammar backend 根据 tokenizer 编译约束并为每个请求维护 grammar state；model runner 输出 logits 后，sampler 查询当前 state 的 valid token mask，把非法 token logits 置为负无穷，再做 temperature、top-p、top-k 或 greedy；采样出 token 后 grammar state 接受 token 并推进。第二层讲约束选择：固定分类用 choices / regex；标准对象用 JSON schema；自定义 DSL 用 EBNF；工具调用或 XML-like 片段用 structural tag。第三层讲工程边界：空合法 token 集合要显式失败或明确 fallback，不能静默继续；custom logit processor 不能把所有合法 token 屏蔽掉；streaming 中间 chunk 通常不是完整 JSON，程序消费最好等 finish 或使用增量 parser。第四层讲正确性边界：constrained decoding 能保证格式、字段、类型或枚举边界，但不能保证事实正确、业务规则正确、工具权限安全或参数语义合理，仍需要 grounding、validator、权限检查和人工复核。最后用 Structured Generation Gate 收束：schema、mask、regex、EBNF、tag、empty valid、format-not-fact、stream 和 metrics 都可验收，才算讲清。

40ZZZZL33. Speculative decoding 在 SGLang 中为什么能降 TPOT，什么时候会变慢？
       考察点：Speculative decoding audit、draft source、target verify call、accept length、draft acceptance rate、target call reduction、speculative fallback token、speculative KV cleanup、adaptive speculative steps、structured output interaction、speculative decoding gate。
       回答框架：先说明 speculative decoding 是 decode 阶段加速手段，不是用小模型替代大模型。普通自回归 decode 每轮 target model 通常只提交一个 token；speculative decoding 先用 EAGLE / EAGLE3、MTP、Standalone draft model、NGRAM 等更便宜的 draft source 提出多个候选 token，再由 target model 一次 verify 一段候选。如果 target 接受多个 token，就能用一次 target verify 前进多个 output tokens，从而降低 target calls per output token，改善 TPOT 和 output tokens/s。第一层讲正确性：draft 只是 proposal，标准 speculative sampling 通过接受-拒绝和 residual correction 目标是保持 target 分布；教学版 greedy demo 只能证明执行链路，不等价于完整采样算法。第二层讲收益公式：看 acceptance rate、accept length、tokens per target verify、target call reduction、draft latency、verify latency、scheduler overhead 和显存峰值；低 temperature、代码、模板化和重复上下文更可能高接受率，高 temperature 开放写作或 draft 很弱时收益差。第三层讲 SGLang 交互：scheduler 要估算 draft / verify token 成本，临时 speculative KV 不能和 committed KV 混淆，rejected tokens 不能进入 RadixAttention prefix cache；structured output 还要让 grammar state 随 accepted tokens 推进，并在拒绝或 grammar block 时回滚。第四层讲变慢原因：draft 模型太慢、accept length 低、verify 路径没有并行收益、额外显存降低 batch、输出太短或 p99 受低接受率请求拖累。最后用 Speculative Decoding Gate 收束：draft、verify、accept、fallback、KV、grammar 和 metrics 都可观测，才算能上线调优。

40ZZZZL34. SGLang 如何支持 multi-turn、tool use 和 agent serving，runtime 要验收什么？
       考察点：Agent serving audit、multi-turn runtime state、session aware routing、agent trajectory prefix sharing、tool parser fit、tool schema validation、tool result backfill、tool wait GPU release、agent round budget、Agent Serving Gate。
       回答框架：先说明 multi-turn、tool use 和 agent serving 不是一次普通 chat completion，而是一个 task / session 内的多次 generation、工具调用、工具结果回灌和动态停止。第一层讲 multi-turn：OpenAI-compatible messages 或 SGLang frontend program 最终都会被 chat template 渲染成 token prefix；只要 system prompt、工具说明、消息顺序和裁剪策略稳定，后续轮次就能用 RadixAttention / prefix cache 复用历史 KV，session-aware routing 要尽量把同一 session 路由到同一 runtime replica。第二层讲 tool use：tools schema 告诉模型可用工具，structured output / EBNF / JSON schema 可约束格式，tool parser 把模型原始文本解析成 name / arguments，validator 做 schema、权限、确认和业务规则校验，真正工具执行通常在应用层，结果作为 tool message 回灌给模型。第三层讲 agent loop：模型生成 action，应用执行工具并返回 observation，再继续下一次 generation；trajectory 会增长，也可能有并行工具调用或分支搜索，因此 scheduler 要处理多次 generation、工具等待、cache locality、公平性和 abort cleanup。第四层讲指标：不能只看 QPS，要看 task success、task E2E latency、model calls per task、tool rounds、tool latency / error / timeout、total tokens、prefix saved tokens、KV pressure、stream events 和 blocked tool rate。最后用 Agent Serving Gate 收束：session、cache、parser、tool、slot、round 和 metrics 都可验收，才算真的支持 agent serving。

40ZZZZL35. SGLang 和 vLLM 怎么对比，为什么不能说谁完全替代谁？
       考察点：Serving architecture comparison audit、common serving core、Paged prefix cache evidence、Radix program reuse evidence、workload fit score、runtime workload router、vLLM not replaced gate、SGLang program fit gate、architecture comparison gate。
       回答框架：先说明两者共同点：都是现代 LLM serving / runtime 系统，都要处理 API、tokenization、prefill / decode、KV cache、scheduler、sampler、streaming、metrics、并行和稳定性。第一层讲问题出发点：vLLM 更偏通用高并发 serving，主线是 PagedAttention、KV block manager、continuous batching、scheduler、worker / executor、prefix cache 和调优；SGLang 更偏复杂 LLM program runtime，主线是 frontend / runtime 协同、RadixAttention、structured output、speculative decoding、multi-turn、tool use 和 agent serving。第二层讲 PagedAttention vs RadixAttention：PagedAttention 关注 KV cache 物理块、block table、碎片和显存利用；RadixAttention 关注 token prefix / program trajectory 的 longest prefix match、split、pin / evict 和多分支复用，它最终仍要落到底层 KV memory pool 或 page layout。第三层讲 workload：独立 chat、普通 RAG、高并发 OpenAI-compatible API 很自然适合 vLLM-like 后端；结构化抽取、多步 generation、self-consistency、Tree-of-Thought、agent trajectory 和工具调用更能体现 SGLang-like runtime 优势。第四层讲平台选型：不要只看 benchmark，要看 prompt / output 长度分布、共享 prefix、branch factor、structured output、tool latency、KV pressure、p95 / p99 和团队运维能力；平台层可以做 workload router，而不是强行统一。最后用 Architecture Comparison Gate 收束：common core、paged、radix、vLLM 不被替代、SGLang program fit、router 和 metrics 都讲清，才算答对。

40ZZZZL36. 你会怎么读 mini-sglang 源码，怎么证明不是只看过文件名？
       考察点：mini-sglang source path audit、runtime module map、request lifecycle order、source resource coverage、source experiment coverage、source observable signal、radix split experiment、abort cleanup experiment、source path gate。
       回答框架：先说明不从文件列表顺序读，而是先跑通最小 generate 请求，再沿 request lifecycle 读入口、tokenizer、request state、scheduler、KV cache、RadixAttention、engine / model runner、attention backend、sampler、detokenizer、streaming 和 cleanup。第一层讲模块地图：每个模块都要记录输入、输出、修改的状态和占用的资源，例如 scheduler 输入 waiting / running / token budget / KV state，输出 schedule plan 和 execution metadata；KV cache 要看 allocate、write、read、append、release；Radix cache 要看 prefix hit、split、insert、pin / evict。第二层讲阅读顺序：外部 JSON 或 offline request 如何变成 input ids 和 request state，请求如何从 WAITING 到 PREFILLING、DECODING、FINISHED / ABORTED，model runner 如何区分 prefill 和 decode，sampler 如何把 logits 变成 token。第三层讲实验验证：最小生成证明主链路，两个共享 system prompt 看 prefix hit，三个分支 prompt 看 radix split，长短 prompt 混排看 scheduler，max tokens 看 finish，streaming 看增量输出，structured output 看 grammar mask，abort 看 KV release。第四层讲面试证据：用 module coverage、resource coverage、experiment coverage、observable signals 和 missing item 清单证明阅读完整；同时说明 mini-sglang 是教学骨架，完整 SGLang 还有更多模型、后端、并行、grammar、speculative decoding、tool parser 和生产监控。最后用 Source Path Gate 收束：module、order、resource、experiment、signal 和 note 都可验收，才算真的读懂。

40ZZZZL37. 为什么 Prefill 和 Decode 的资源画像不同，PD 分离动机怎么讲才不是背概念？
       考察点：Prefill decode resource profile audit、request token profile、run prefill tokens、prefill compute profile、prefill KV write、decode KV read、TTFT TPOT separation、long prefill stall risk、decode-first starvation risk、PD KV transfer cost、prefill decode profile gate。
       回答框架：先定义生命周期：prefill 处理 prompt，建立 prompt KV，主要影响 TTFT；decode 逐 token 生成，每步读取历史 KV 并追加新 KV，主要影响 TPOT / ITL 和 streaming。第一层讲 token 画像：请求要拆成 prompt tokens、output tokens 和 prefix hit tokens，真正需要跑的 prefill 是 `max(prompt-hit,0)`，所以长 prompt 命中缓存后不一定同等昂贵。第二层讲资源画像：prefill 一次处理多个 token，矩阵乘法和 attention prefill 更容易形成大 token batch，同时大量写入 KV；decode 每请求每轮通常 1 个 token，计算量小但要对每个 active sequence 反复读历史 KV，上下文越长、输出越长，KV 读取和调度越敏感。第三层讲干扰：同一 GPU worker 混跑时，长 prefill 会抢占执行窗口，让 running decode 的 TPOT 抖动；长期 decode-first 虽然保护 streaming，却可能让 waiting prefill 饥饿，TTFT 变差。第四层讲缓解：chunked prefill 把长 prefill 切块，降低单次 stall，但 P/D 仍在同一资源池争抢；PD 分离把 P/D 放入不同资源池，能分别调度和扩缩容，但要支付 KV transfer、路由、状态同步和故障恢复成本。最后用 Prefill Decode Profile Gate 收束：token 分布、prefix hit、prefill compute、decode KV read、TTFT、TPOT、stall、starvation 和 KV transfer 都可观测，才算讲清。

40ZZZZL38. PD 分离到底解决什么问题，什么时候不值得做？
       考察点：PD disaggregation motivation audit、unified engine interference、prefill interruption evidence、decode-first starvation evidence、independent P/D scaling、KV transfer cost gate、slow transfer counterexample、PD motivation gate。
       回答框架：先说明 PD 分离解决的不是“把代码拆成两个函数”，而是统一 serving engine 中 prefill 和 decode 混合调度带来的资源干扰、尾延迟和扩缩容耦合。第一层讲 unified interference：长 prompt prefill 会占用执行窗口，running decode 不能及时推进，表现为 TPOT / ITL 抖动和 streaming 卡顿；如果 scheduler 长期 decode-first，又会让 waiting prefill 饥饿，新请求 TTFT 上升。第二层讲资源池：prefill 更需要大 token batch、input tokens/s 和 KV 写入吞吐；decode 更需要稳定低 step latency、KV 读取效率和 running batch 管理，统一参数只能折中。第三层讲扩缩容：长输入短输出和短输入长输出对 P/D 资源需求不同，统一 replica 扩容会同时增加两种能力，可能浪费；PD 分离后 prefill pool 和 decode pool 可以按比例独立扩缩容。第四层讲代价：prefill worker 生成的 KV cache 必须迁移或共享给 decode worker，带来 transfer backend、网络带宽、KV layout、路由、状态同步、故障恢复和观测复杂度；如果 slow transfer time 大于干扰节省，PD 分离就不值得。最后用 PD Motivation Gate 收束：只有 unified 干扰、jitter / starvation 改善、独立扩缩容价值、KV transfer 成本和慢传输反例都讲清，才算回答完整。

40ZZZZL39. 请画出 PD 分离系统架构，并说明 router、worker、KV transfer 和失败清理怎么协作？
       考察点：PD architecture audit、PD router、prefill worker pool、decode worker pool、KV transfer backend、bootstrap metadata、decode reservation、cross component state machine、transfer failure cleanup、client abort cleanup、model version compatibility、PD architecture gate。
       回答框架：先画主链路：client / API gateway -> PD router -> prefill worker pool -> KV transfer backend -> decode worker pool -> stream output。第一层讲 router：它不是普通 LB，要维护 P/D worker 列表、健康、模型版本、prefill queued tokens、decode running sequences、KV capacity、transfer backend 状态、request state 和 abort / timeout；它要分别选择 prefill worker 和 decode worker。第二层讲 worker：prefill worker 负责 prompt tokens、prefill forward、prompt KV cache 和 source metadata；decode worker 负责先预留 decode slot / target KV blocks，再接收或拉取 KV，进入 continuous batching decode loop，持续 sampling 和 streaming。第三层讲 KV transfer：bootstrap metadata 要包含 request id、source / target worker、source / target block mapping、sequence length、rank / layout、dtype、session id 和 timeout；transfer backend 可以是同机、RDMA、NIXL、Mooncake、CPU / GPU staging 或远端 KV cache，但本质是把 GPU KV 数据正确交给 D 侧。第四层讲状态机和故障：状态从 RECEIVED、PREFILL_ASSIGNED、DECODE_RESERVED、PREFILL_DONE、KV_TRANSFERRING、DECODE_READY、DECODING 到 FINISHED；transfer 失败要释放 P 侧临时 KV 和 D 侧预留 slot；client abort 要广播到 router、P、transfer 和 D；模型版本、tokenizer、KV dtype、layout、TP/DP rank 不一致必须阻断。最后用 PD Architecture Gate 收束：route、reserve、metadata、transfer cleanup、abort cleanup、version check 和 metrics 都可验收，才算架构合理。

40ZZZZL40. KV Cache 在 PD 分离里怎么迁移、共享和路由，为什么不能只复制 bytes？
       考察点：KV transfer routing audit、toy KV transfer router、KV metadata compatibility、decode capacity reservation、KV routing cost score、tenant cache isolation、recompute fallback path、transfer failure plan、KV transfer routing gate。
       回答框架：先说明 KV cache 是运行时状态，不是普通 RPC payload。它绑定 model version、tokenizer、KV dtype、layout version、paged block table、sequence length、TP size、rank mapping、source worker 和 tenant。第一层讲兼容性：decode worker 必须和 prefill worker 在模型、tokenizer、dtype、layout、TP/DP 配置上兼容，不兼容要直接跳过，不能静默接收 KV。第二层讲容量：目标 decode worker 通常要先根据 prompt tokens 和预计 output tokens 预留 KV blocks，再启动 push / pull / remote read / staging transfer，否则传过去也没有稳定落点。第三层讲路由：KV-aware routing 不能只看空闲 worker，要把 free blocks、running sequences、pending transfers、当前 TPOT、网络 zone、transfer 带宽和 prefix cache locality 放进 score；prefix-aware routing 还要防止热点 worker。第四层讲隔离和兜底：跨租户 prefix hash 命中不能复用；短 prompt 或 transfer 成本高时可以 recompute；transfer 失败要释放目标 blocks、标记 session、保留 source KV 可重试或退回重算。最后用 KV Transfer Routing Gate 收束：metadata compatibility、capacity reservation、cost score、tenant isolation、recompute fallback、failure plan 和 metrics 都可验收，才算 KV transfer 方案完整。

40ZZZZL41. Chunked Prefill 和 Disaggregated Prefill 有什么区别，怎么和 scheduler / KV transfer 一起讲？
       考察点：Chunked disaggregated prefill audit、toy chunked disagg prefill auditor、prefill chunk count、decode interleaving、prefill token budget、position continuity、short request fairness、PD transfer pipeline、backpressure cleanup、chunked disagg prefill gate。
       回答框架：先区分层级：Chunked Prefill 是执行粒度优化，把长 prompt 的 prefill 按 token chunk 拆开，每个 chunk 仍经过完整 Transformer 层；Disaggregated Prefill 是系统架构优化，把 prefill 放到独立 prefill worker / pool，再把 KV 交给 decode pool。第一层讲为什么要 chunk：长 prefill 一次性执行会形成大 stall，打断 running decode、拖高短请求 TTFT 和 TPOT tail；chunk 后 scheduler 能在 chunk 边界插入 decode step 或短请求。第二层讲调度门禁：要计算 run prefill tokens、chunk count、每轮 token budget，decode rows 和 prefill chunk 不能超预算；还要有 waiting time / progress 机制，避免短请求被长请求挡住，也避免长请求永远跑不完。第三层讲正确性：chunked prefill 必须等价于 full prefill，position id、RoPE offset、attention mask、KV block table、partial block 和 prefix cache 都要连续。第四层讲 PD 结合：在 disaggregated prefill pool 中可以按 chunk 生成 KV，并和 transfer 流水化，但 decode 生成首 token 前通常仍要完整 prompt KV；同时要处理 partial KV metadata、destination block 增量预留、transfer ordering、backpressure 和 abort cleanup。最后用 Chunked Disagg Prefill Gate 收束：chunking、decode interleaving、token budget、position continuity、fairness、PD transfer、backpressure cleanup 和 metrics 都可验收，才算回答完整。

40ZZZZL42. 如何设计多级 KV Cache，为什么不能只看 cache hit rate？
       考察点：Multi level KV cache audit、toy multi level KV cache auditor、KV residency level、active GPU block protection、CPU KV promote、remote KV fetch、recompute fallback、KV tenant isolation、KV demotion policy、residency metrics、multi level KV gate。
       回答框架：先说明 KV cache 是运行时状态，不是普通对象缓存。活跃 decode 的 KV 每步都被 attention 读取，必须优先 GPU resident；idle session、被 preempt 的请求、可复用 prefix 可以放到 CPU、SSD 或 remote；如果 fetch 成本大于 recompute，就应该重算。第一层讲层级画像：GPU HBM 延迟最低但容量最紧；CPU DRAM 适合短期恢复和 swap；SSD / remote 容量大但延迟和抖动高；recompute 是最后兜底。第二层讲 scheduler 决策：prefix hit 后不只返回 hit length，还要返回 residency level、promote/fetch cost、GPU block budget、transfer queue、recompute cost 和 tenant 权限。第三层讲保护策略：active decode block 不能被为了缓存命中率而 demote；GPU 满时优先 demote ref count 为 0、低复用、非关键路径的 block；remote fetch 不应该进入 TPOT 关键路径。第四层讲正确性：cache key 要覆盖 model revision、tokenizer、token ids、position、attention config、dtype、parallel config、adapter、多模态输入和 tenant scope，跨租户或版本不一致必须阻断。最后用 Multi Level KV Gate 收束：hot KV protection、CPU promote、remote fetch、recompute fallback、tenant isolation、demotion policy 和 residency metrics 都可验收，才算设计完整。

40ZZZZL43. 跨节点 serving 的网络瓶颈怎么分析，为什么 TP 通常不应该跨慢网络？
       考察点：Cross node network audit、toy cross node network auditor、cross node link profile、TP cross node risk、pipeline activation cost、PD KV transfer cost、remote recompute decision、topology aware routing、transfer backpressure、control data plane separation、cross node network gate。
       回答框架：先区分跨节点通信类型：DP 是请求分流，replica 间基本独立；TP 是模型单层内部的 all-reduce / all-gather / reduce-scatter 等 collective；PP 是 stage boundary 传 hidden states；PD 分离是 prefill 侧把 KV transfer 到 decode 侧；remote KV cache 是跨节点 fetch / push KV block。第一层讲 TP 风险：decode 每个 output token 要过所有层，如果每层都有跨节点 collective，网络延迟和 p99 拥塞会累积到 TPOT，所以 TP 通常应限制在单节点 NVLink / NVSwitch 内，跨节点优先考虑 PP 或 DP。第二层讲成本模型：网络时间要看 latency、bandwidth 和 p99 multiplier；KV transfer 偏大块带宽，TP collective 偏高频延迟 + collective bandwidth，PP activation 是 stage 边界传输。第三层讲 PD / remote cache：跨节点 PD 要把 prompt KV bytes 算进 TTFT；remote hit 不一定好，要比较 remote fetch + promote 与 recompute。第四层讲路由和保护：router 不能只看 worker load，还要看拓扑距离、pending transfer、KV locality、decode TPOT 和 KV capacity；大 KV transfer 要 admission control / backpressure，并和 heartbeat、metadata、streaming control plane 隔离。最后用 Cross Node Network Gate 收束：TP risk、PP cost、PD KV cost、remote-vs-recompute、topology routing、backpressure、control/data plane 和 metrics 都可验收，才算分析完整。

40ZZZZL44. 什么时候应该做 PD 分离，什么时候是反模式？
       考察点：PD tradeoff audit、toy PD tradeoff auditor、PD benefit cost score、PD positive case、short request anti pattern、decode bottleneck anti pattern、slow transfer anti pattern、ops readiness gate、PD alternative path、PD tradeoff gate。
       回答框架：先给判断标准：PD 分离不是默认终态，而是当 P/D 干扰收益大于 KV transfer 成本、分布式状态复杂度和运维成本时才值得做。第一层讲正例：长 prompt / 长上下文比例高，unified engine 中 prefill interruption 明显，decode TPOT p95/p99 被长 prefill 打爆，或者 decode-first 又让 prefill queue 饥饿；同时 P/D 输入输出比例波动大，需要独立扩缩容，多租户隔离收益明显。第二层讲成本：prefill 生成的 KV 必须 transfer / share 给 decode，transfer p99、网络拥塞、decode 等 KV、metadata、router、失败清理、backpressure、观测和调参都会吃掉收益。第三层讲反模式：短请求、小流量、单机已达标、主要瓶颈是长输出 decode、模型 compute、tokenizer、RAG 或网络 streaming 时，不该优先 PD；KV transfer 很慢、无脑跨节点、remote cache 当万能药、只拆服务不拆资源、没有 metrics / cleanup / backpressure 都是反模式。第四层讲替代路径：先评估 chunked prefill、prefix cache、scheduler 调参、DP replicas、speculative decoding、same-node PD，再做 cross-node PD。最后用 PD Tradeoff Gate 收束：正例、短请求反例、decode 瓶颈反例、慢 transfer 反例、ops readiness、成本模型、替代路径和反模式识别都可验收，才算决策完整。

40ZZZZL45. 如何把已有单机 serving engine 升级成 PD 分离架构？
       考察点：Single engine to PD migration audit、toy PD migration auditor、explicit request stage、model runner interface split、KV metadata contract、prefill decode scheduler split、PD router state machine、same node PD prototype、cleanup cancel path、observability fallback readiness、PD migration gate。
       回答框架：先说明这不是把 `prefill()` 和 `decode()` 直接拆成两个远程服务，而是把单进程 generate 状态机逐步改成跨 router、prefill worker、decode worker 和 KV transfer backend 的可清理状态机。第一层讲单机重构：把 request stage 从 `is_finished` 变成 waiting_prefill / prefilling / waiting_decode / decoding / finished / failed；model runner 暴露 prefill / decode 接口；prefill 返回标准 KV metadata，decode 通过 metadata 接管 KV，而不是引用本地 Python 对象。第二层讲逻辑 PD：拆 prefill scheduler 和 decode scheduler，前者关注 prompt tokens、chunk、prefix hit 和 input tokens/s，后者关注 active sequences、KV capacity、TPOT 和 streaming deadline；再引入 router 状态机维护 request state、worker assignment、transfer handle、stream handle、timeout 和 cancel。第三层讲分阶段部署：先同进程逻辑 PD，再 same-node 多进程 / 多 GPU PD，验证 decode reservation、KV transfer、cleanup、client cancel、metrics、backpressure 和 unified fallback；稳定后再跨节点，引入 topology-aware routing 和 transfer admission control。第四层讲上线门禁：按长 prompt、租户或流量比例灰度，观察 request queue wait、prefill time、transfer p99、decode wait、TPOT p99、KV free blocks、cleanup events、cancel rate 和 fallback rate。最后用 PD Migration Gate 收束：接口、KV contract、scheduler、router、same-node 原型、失败清理、观测和回滚都可验收，才扩大到 cross-node PD。

40ZZZZL46. 如何通过 nano-vLLM 学 vLLM-like 推理框架源码？
       考察点：nano-vLLM source learning audit、toy nano-vLLM source auditor、source module map、generate trace path、engine step trace、sequence state audit、scheduler decision trace、KV block lifecycle、model runner batch contract、source experiment signal、nano-vLLM source gate。
       回答框架：先说明 nano-vLLM 的价值不是完整复刻生产级 vLLM，而是用小代码展示 vLLM-like serving engine 的主骨架。第一层讲阅读顺序：沿 request lifecycle 读 example / LLM、sampling params、Sequence、LLMEngine、Scheduler、BlockManager、ModelRunner、Attention、Sampler 和模型结构，不从模型层逐行啃。第二层讲主链路：一次 `generate()` 会创建请求状态，进入 waiting / running 队列，scheduler 选择 prefill / decode，block manager 分配或追加 KV block，model runner 准备 input ids、positions、slot mapping 和 block table，attention 读写 KV，sampler 产出 next token，sequence append token，结束后释放 KV。第三层讲实验验证：跑最小 generate trace、prefill/decode step trace、scheduler token budget、KV block reuse、prefix cache hit、finish cleanup、sampler params，观察 TTFT / TPOT、scheduled tokens、free blocks、block ref count、prefix hash、finish reason 和 next token。第四层讲边界：nano-vLLM 适合学习 sequence、scheduler、block manager、runner、attention 和 sampler，但生产级 vLLM 还有 API server、async streaming、worker/executor、分布式部署、多租户、多模型、完整 metrics 和故障恢复。最后用 nano-vLLM Source Gate 收束：模块、路径、engine step、sequence state、scheduler decision、KV lifecycle、runner batch contract、实验和信号都可验收，才算真的读懂。

40ZZZZL47. 如何通过 tiny-llm 理解 LLM 推理系统是怎么从模型 forward 演进出来的？
       考察点：tiny-LLM learning audit、toy tiny-LLM learning auditor、operator to model path、attention shape trace、RoPE position alignment、GQA KV head audit、generate sampling loop、KV cache equivalence、serving optimization ladder、chunk position continuity、tiny-LLM learning gate。
       回答框架：先说明 tiny-llm 和 nano-vLLM 的区别：nano-vLLM 更像已经成型的轻量 vLLM-like engine，tiny-llm 更像从模型算子到 serving 优化的课程。第一层讲模型基础：从 attention shape、RoPE position、GQA 的 query heads / KV heads、RMSNorm、MLP、模型加载和 decoder layer forward 读起，因为这些决定 KV cache shape、position 对齐和 decode 内存成本。第二层讲最小推理闭环：完成 generate loop 和 sampling，区分 prefill 输入完整 prompt、decode 每步输入一个 token，没有 KV cache 时会重复计算历史上下文。第三层讲 KV cache：证明 KV cache generate 与 naive generate 的输出等价，并用 TTFT / TPOT 看到 prefill / decode 分段；GQA 降低 KV heads 后，KV cache 显存和 decode 读带宽也下降。第四层讲 serving 优化阶梯：quantized matmul 降权重显存和带宽，FlashAttention 优化长 prefill IO，continuous batching 让请求按 iteration 动态进出，chunked prefill 缓解长 prompt 阻塞 decode，paged attention 用 block table 管不连续 KV。最后用 tiny-LLM Learning Gate 收束：算子、模型、generate、sampling、KV cache、serving 优化、指标实验和依赖顺序都可验收，才算真正读懂 tiny-llm。

40ZZZZL48. 如何通过 mini-sglang 学 SGLang-style runtime，而不是只把它当另一个 vLLM clone？
       考察点：mini-sglang learning audit、toy mini-sglang learning auditor、SGLang runtime capability、Radix Cache runtime evidence、chunked prefill ablation、overlap scheduling ablation、online serving trace、structured tool boundary、abort cleanup check、production gap map、mini-sglang learning gate。
       回答框架：先说明 mini-sglang 应该放在 tiny-llm 和 nano-vLLM 之后读：tiny-llm 解释 attention、KV cache、batching、chunked prefill、paged attention 为什么出现；nano-vLLM 解释 vLLM-like engine 的 sequence、scheduler、block manager、model runner 骨架；mini-sglang 则解释 SGLang runtime 特色。第一层讲 Radix Cache：它不是普通 hash prefix cache，而是支持 longest prefix match、tree split、node insert / eviction、KV ownership 和 partial prefix reuse，适合 system prompt、tool schema、few-shot、多轮对话和分支程序。第二层讲 scheduler：chunked prefill 要先命中共享 prefix，再对未命中的 suffix 做 chunk，并验证 position continuity、partial KV cleanup；overlap scheduling 要让 CPU 侧下一轮 schedule / metadata 准备与 GPU 当前 forward 重叠，减少 GPU 等 CPU。第三层讲 online runtime：通过 OpenAI-compatible API、interactive shell、streaming、client abort、online benchmark 观察持续请求到达、TTFT、TPOT、prefix hit、KV 使用和 cleanup；这比 offline generate 更接近服务化。第四层讲 structured/tool 边界：structured generation 要有 grammar state、valid token mask 和 sampler 边界；tool/agent serving 要区分 tool call 生成 / 解析和真实工具执行，后者通常属于应用层。最后用 mini-sglang Learning Gate 收束：Radix Cache、chunked prefill、overlap、online serving、structured/tool 边界、abort cleanup、production gap 和依赖顺序都可验收，才算真正读懂。

40ZZZZL49. 如何从 tiny-llm、nano-vLLM、mini-sglang 抽象出自己的最小推理框架核心模块？
       考察点：Teaching project core module audit、toy core module auditor、core module boundary、request state ownership、scheduler modelrunner boundary、KV manager ownership、BatchBuilder metadata contract、output processor cleanup boundary、metrics observability contract、replaceable upgrade path、core module gate。
       回答框架：先说明三个教学项目提供不同视角：tiny-llm 解释模型算子到 KV cache / batching 的演进，nano-vLLM 解释 vLLM-like engine 的 sequence / scheduler / block manager / runner 骨架，mini-sglang 解释 SGLang runtime 的 prefix sharing / online / structured / tool 边界。抽象出来的最小框架不应该照抄某个项目目录，而要形成稳定模块边界。第一层讲核心模块：Request 保存 prompt tokens、output tokens、sampling params、status、时间戳和 KV 引用；Scheduler 管 waiting/running，按 token budget 和 KV capacity 输出 batch plan；KVCacheManager 负责 allocate / append / free / block table；BatchBuilder 把逻辑 plan 转成 input ids、positions、slot mapping、block table 和 attention metadata；ModelRunner 只 forward 返回 logits；Sampler 按 per-request params 选 token；OutputProcessor append token、检查 EOS / max tokens / stop、触发 cleanup；Metrics 记录 TTFT、TPOT、latency、吞吐和 KV usage。第二层讲边界反模式：scheduler 不直接调用模型，model runner 不修改 request 状态，KV 不挂成 request tensor list，sampler 不假设全 batch 同一参数，metrics 不能缺。第三层讲第一版范围：先做单机、单模型、offline、多请求、prefill/decode、简单 KV 和 greedy sampling，暂不做 streaming、paged attention、prefix cache、TP、structured、tool、分布式。最后用 Core Module Gate 收束：模块覆盖、状态 ownership、边界契约、依赖顺序、metrics 和可替换升级路径都可验收，后续 continuous batching、paged KV、streaming、多模型和分布式才不会推倒重来。

40ZZZZL50. 如何把 naive scheduler 升级成 continuous batching scheduler？
       考察点：Naive scheduler to continuous batching audit、toy scheduler upgrade auditor、request level batch lock、iteration boundary scheduling、dynamic request admission、dynamic request exit、decode first upgrade、bounded prefill budget、KV capacity admission、scheduler upgrade gate。
       回答框架：先讲 naive scheduler 的问题：request-level batch 的生命周期被最长输出请求锁住，短请求完成后 slot 空着，新请求还要等整批结束，在线服务下会造成 TTFT 变差、batch 利用率下降和 GPU 空转。第一层讲接口改造：Scheduler 不直接构造 tensor，而是输出 BatchPlan；每个 ScheduleItem 标明 request、本轮是 prefill 还是 decode、token 起点和 token 数；BatchBuilder 再生成 input ids、positions、slot mapping、block table、query lens 和 logits indices。第二层讲调度策略：在 iteration boundary 重新调度，先 decode running 请求保证 TPOT，再处理 running prefill chunk，最后在 token budget、bounded prefill、max running requests 和 KV capacity 允许时接纳 waiting 请求。第三层讲状态和清理：Request 要记录 computed_prompt_tokens、status、first token time、last decode time 和 KV 引用；finished / cancelled 请求必须移出 running 并释放 KV，否则 continuous batching 会把显存打满。第四层讲验证：对比 static decode rows 和 continuous output rows，记录 finished order、cancelled ids、queue wait、TTFT、max running、max KV blocks、deferred reasons 和 trace tail。最后用 Scheduler Upgrade Gate 收束：request-level lock、iteration boundary、dynamic join / exit、decode-first、bounded prefill、KV admission 和 cleanup 全部可复盘，才算完成升级。

40ZZZZL51. 如何把 list KV cache 升级成 paged KV cache？
       考察点：Paged KV cache upgrade audit、toy paged KV upgrade auditor、list KV private cache、global KV block pool、request block table、allocate until contract、paged slot mapping、decode block extension gate、idempotent KV free、double free guard、paged KV upgrade gate。
       回答框架：先说明 list KV cache 的问题不是 Python list 慢，而是抽象错了：KV 被挂在 request 私有对象上，scheduler 看不到全局 free blocks，finished / aborted 后复用和碎片都不清晰，也很难生成高效 attention metadata。第一层讲数据结构：把 GPU KV 预切成固定大小 physical blocks，用全局 BlockManager 管 free list / allocated set；每个 request 只维护 block table，把 logical block 映射到 physical block。第二层讲分配协议：BlockManager 暴露 required_blocks、can_allocate_until、allocate_until 和 free；prefill chunk 前按 target tokens 补齐 blocks，decode 每生成下一个 token 前检查 `next_pos + 1`，跨 block 边界时追加新 block。第三层讲 BatchBuilder：除了 input ids 和 positions，还要生成 slot mapping 和 block tables；slot mapping 写当前 token 的 KV，block table 读历史 KV，两者不能混。第四层讲清理和验证：finished、abort、timeout、failed 都必须幂等 free，free 后清空 block table 并防 double free；验证 block table 增长、decode extension、释放复用、admission failure、slot mapping、fragmentation、allocation failures 和 free blocks after cleanup。最后用 Paged KV Upgrade Gate 收束：block pool、block table、slot mapping、decode extension、KV admission、reuse 和 cleanup 全部可复盘，才算完成升级。

40ZZZZL52. 如何在 paged KV cache 上实现第一版 prefix cache，并区分 prompt cache？
       考察点：Prefix prompt cache upgrade audit、toy prefix prompt cache auditor、prompt prefix cache boundary、full block cache reuse、parent hash cache chain、extra hash cache isolation、suffix prefill start position、cached ref count lifecycle、full prompt hit fallback、prefix prompt cache upgrade gate。
       回答框架：先划清边界：prompt cache 是平台层的宽泛复用，可以缓存 tokenized prompt、RAG 结果、embedding、KV blocks 或完整响应；本题要实现的是 runtime 内部 KV prefix cache，复用已经算好的 KV full blocks，不等于 result cache。第一层讲数据结构：在 paged KV cache 的 block 对象上增加 ref count、block hash、cached 标记，BlockManager 维护 `cached_blocks: block_hash -> block_id` 和 free queue。第二层讲 hash：只缓存 full blocks，block hash 必须包含 parent hash 和 extra hashes，例如 model、tokenizer、LoRA、tenant salt、多模态 hash；同样 tokens 但前文或 extra hash 不同不能复用。第三层讲命中流程：新请求按 block size 从 prompt 开头切 full blocks，逐块查最长连续命中；命中后把 block ids 接到 request.block_table，把 `num_computed_tokens` 设成 hit tokens，scheduler 只调度未命中 suffix，BatchBuilder 从真实 start position 生成 positions 和 slot mapping。第四层讲生命周期：命中 cached-free block 要 touch 出 free queue 并增加 active ref；请求结束时只释放 active ref，cached ref 保留；显存紧张时只淘汰没有 active request 的 cached-free blocks。第五层讲全命中和指标：完整 prompt KV 命中不代表有 logits，第一版要重算最后 token 或另做 logits cache；验收看 saved prefill tokens、token hit ratio、hit/miss TTFT、cached blocks、free blocks、evictions 和 miss reasons。最后用 Prefix Prompt Cache Upgrade Gate 收束：边界、full block、parent hash、extra hash、suffix prefill、ref count、eviction 和 fallback 都可复盘，才算实现可信。

40ZZZZL53. KV cache 不够时，如何实现 preemption、recompute 和 swap？
       考察点：Preemption recompute swap upgrade audit、toy preemption upgrade auditor、KV pressure trigger、cached block eviction before preemption、victim selection policy、recompute context preservation、recompute resume path、swap failure rollback、swap in state restore、preemption upgrade gate。
       回答框架：先说明 preemption 是防 OOM 的鲁棒性机制，不是性能优化。第一层讲触发顺序：scheduler 在构建 batch plan 前根据本轮 prefill / decode 需要的 KV blocks 和 free blocks 判断压力；如果不够，先降低本轮调度压力或暂缓 admission，再淘汰没有 active request 的 cached-free blocks，仍然不够才 preempt running request。第二层讲 victim 选择：不能总抢同一个请求，要看优先级、preempt 次数、上下文长度、是否接近完成、是否 latency-sensitive；教学版可以优先抢低优先级、上下文短、preempt 次数少的请求。第三层讲 recompute：preempt 时释放 GPU KV blocks，但必须保留 prompt tokens、已生成 output tokens、sampling params 和状态；恢复时重算的上下文是 prompt + output，不是只重算 prompt。第四层讲 swap：swap out 把 GPU blocks 拷到 CPU blocks 并释放 GPU；swap in 分配新的 GPU blocks 再拷回，物理 id 可以变但逻辑顺序不能变；swap in 失败时不能清空 CPU block table。第五层讲指标：记录 preemption_total、recompute tokens、freed blocks、swap in/out blocks or bytes、free blocks before/after、repeated preemption 和被抢占请求的 E2E/TPOT gap。最后用 Preemption Upgrade Gate 收束：pressure、eviction、victim、recompute、swap 和 metrics 都可复盘，才算机制可信。

40ZZZZL54. 如何把 scheduler、KV cache、prefix cache 和 preemption 串成一个完整 engine step？
       考察点：Unified scheduler loop audit、toy unified scheduler loop auditor、engine step order、prefix lookup once、decode first commit、suffix prefill plan、KV budget commit、memory pressure order、batch metadata invariant、output state update、unified scheduler loop gate。
       回答框架：先说明完整 engine step 是一个 iteration-level 调度事务，不是 scheduler 单独返回请求列表。第一层讲顺序：接收新请求后进入 waiting；waiting 请求第一次参与调度前做 prefix cache lookup，命中后更新 block table、active ref、prefix hit tokens 和 `num_computed_tokens`；running 请求如果上下文 KV 已完整，优先进入 decode；剩余 token budget 再给 waiting 请求做 suffix prefill 或 chunked prefill。第二层讲预算：每轮同时算 token budget 和 KV block budget，decode 可能跨 block 需要新 block，prefix hit 会减少 prefill tokens 但仍引用 cached blocks，所以不能只看 token 数。第三层讲压力处理：commit 前估算 required new blocks；不够时先缩小或推迟 prefill，再 evict cached-free blocks，仍不够才 preempt active request。第四层讲 commit 和 metadata：commit plan 统一通过 BlockManager 分配 blocks、移动 waiting/running、记录调度范围；BatchBuilder 只能读取 commit 后的 block table，并按真实 start position 构造 positions、slot mapping 和 block tables。第五层讲输出和不变量：OutputProcessor 追加 sampled token、推进 `num_computed_tokens`、注册可缓存 full blocks、释放 finished refs；每轮检查 request 只在一个队列、free list 不含 active block、computed 不超过 context、prefix hit block 对齐、metadata 行数一致。最后用 Unified Scheduler Loop Gate 收束：prefix、decode、prefill、budget、pressure、commit、metadata、output 和 invariants 都可复盘，才算主循环可信。

40ZZZZL55. 如何设计一个可信的 serving benchmark 和调参实验框架？
       考察点：Serving benchmark framework audit、toy serving benchmark framework、benchmark workload coverage、benchmark experiment fingerprint、request trace summary、engine step trace summary、SLO regression gate、throughput regression check、KV cleanup check、prefix effect check、preemption risk check、benchmark decision gate、serving benchmark framework gate。
       回答框架：先说明 benchmark 不是跑一个 QPS，而是可复现的实验闭环。第一层讲 workload：至少覆盖 short baseline、long prompt、shared prefix、long/short mixed 和 KV pressure；不能只测理想共享 prefix 或单请求。第二层讲实验指纹：每次记录 model、hardware、commit、seed、arrival mode、concurrency/QPS、`max_num_seqs`、`max_num_batched_tokens`、block size、chunked prefill、prefix cache、preemption mode 和 workload 版本，否则 baseline/candidate 不可比。第三层讲 trace 和指标：request trace 推导 TTFT、TPOT、E2E、success rate、prefix hit tokens、preemption count；engine step trace 推导 waiting/running、scheduled prefill/decode tokens、KV peak、KV after cleanup、eviction 和 allocation failure。第四层讲回归门禁：candidate 吞吐提升必须同时满足 TTFT/TPOT/E2E SLO、失败率不过线、KV cleanup 正常、prefix cache 确实改善 shared-prefix TTFT、preemption 没有增加到不可接受。第五层讲决策：输出 bottleneck classification 和 tuning decision，比如适合交互式 serving 还是离线批处理。最后用 Serving Benchmark Framework Gate 收束：workload、trace、SLO、throughput、KV、cache、preemption、reproducibility 和 decision 都可复盘，才算实验可信。

40ZZZZL56. 如何把同步 engine loop 升级成异步 serving 架构？
       考察点：Async serving architecture audit、toy async serving runtime、async boundary design、API admission boundary、tokenizer worker queue、engine input queue、bounded engine drain、engine output queue、per-client stream queue、stream queue backpressure、cancel signal queue、engine state ownership、idempotent async cleanup、async serving architecture gate。
       回答框架：先说明同步 engine loop 适合教学，但生产 serving 里 HTTP 接入、tokenization、detokenization、网络 streaming、日志和 metrics 都可能阻塞 GPU 主循环。第一层讲异步边界：API handler 只做参数校验、鉴权/限流、admission 和消息投递；tokenizer worker 从 tokenizer queue 取 raw request，输出 tokenized request；engine input queue 把 tokenized request 交给 engine core；engine core 独占 scheduler、KV cache、request state 和 output processor；output queue 再交给 detokenizer / stream worker。第二层讲 backpressure：所有队列都要有容量上限，API 队列满就拒绝或降级，engine 每轮 drain 新请求要有上限，per-client stream queue 满说明慢 client，只取消该请求，不能拖慢全局 TPOT。第三层讲 cancel 和 cleanup：client disconnect、timeout、slow stream 和重复 cancel 都进入 cancel signal queue，由 engine core 幂等释放 KV blocks、prefix refs 和 request state。第四层讲状态所有权：API、tokenizer 和 stream worker 不能直接改 engine 内部状态，只能发消息；OutputProcessor 不直接写网络。第五层讲验证指标：TTFT、TPOT、E2E、tokenizer queue wait、engine input backlog、engine step duration、output queue wait、stream backpressure、cancel cleanup latency、队列积压和 cleanup 后 KV。最后用 Async Serving Architecture Gate 收束：admission、tokenizer 解耦、bounded drain、output queue、stream backpressure、cancel signal、engine state ownership、idempotent cleanup、slow client isolation 和 queues drained 都可复盘，才算异步化可信。

40ZZZZL57. 多 worker、多 GPU serving 的 request router 应该怎么设计？
       考察点：Multi worker router audit、toy multi worker router、router worker capability filter、request cost block estimate、load aware worker score、sticky route fallback、global router admission、worker heartbeat timeout、safe retry boundary、request worker map、worker selection imbalance、multi worker router gate。
       回答框架：先说明多 worker serving 的核心变化是多了 router 控制面，router 做 worker 级路由，不直接改 worker 内部 scheduler、KV block table 或 prefix cache。第一层讲能力过滤：根据 model、adapter、GPU group、health 和 worker 状态筛候选；一个 tensor parallel group 从 router 视角通常是一个 worker，多个 replica 才是请求级负载均衡目标。第二层讲成本估算：用 prompt tokens、max output tokens 和 block size 估算 KV blocks，同时看 waiting/running queue、engine input queue、TTFT/TPOT、preemption rate 和 KV free blocks。第三层讲 routing score：round-robin 只能做 baseline，生产策略至少要 load-aware，不能把长 prompt 或 KV 压力请求发给明显过载 worker。第四层讲 sticky locality：共享 system prompt、tenant、conversation、prefix hash 或 adapter 的请求可以 sticky 提高 prefix cache 命中，但首选 worker 过载时必须 fallback，避免热门 tenant 形成热点。第五层讲全局 admission 和故障：所有候选 worker 都不健康或总 KV 容量不足时要 reject；heartbeat timeout 后停止路由；未输出 token 的请求可以重试，已经 streaming 的请求通常只能显式失败。最后用 Multi Worker Router Gate 收束：capability、admission、load-aware、sticky fallback、health check、safe retry、state map 和 per-worker metrics 都可复盘，才算 router 可信。

40ZZZZL58. Tensor parallel、pipeline parallel 和分布式 KV cache 在 serving 里有哪些工程难点？
       考察点：Distributed parallel KV audit、toy distributed parallel KV auditor、parallel group boundary、TP head shard assignment、TP block table consistency、PP layer ownership、distributed KV byte accounting、collective communication cost、pipeline bubble ratio、KV migration decision、cross rank cleanup、distributed parallel KV gate。
       回答框架：先区分并行粒度：replica parallel 是多份完整模型副本，router 在 worker 间做请求级负载均衡；tensor parallel 是一个模型切到多个 ranks，一个请求需要整个 TP group 协同；pipeline parallel 是按层切 stage，一个请求依次经过多个 stage。从 router 视角看，能独立完成 forward 的 parallel group 才是 worker。第一层讲 TP：权重和 attention heads 分片后，column / row parallel 会引入 all-gather、all-reduce 或 reduce-scatter；TP size 增大能降低单卡显存和计算压力，但 decode 阶段 token 少，通信 latency 更容易支配 TPOT。第二层讲 PP：按层切分能降低单卡显存，但在线 decode 有逐 token 依赖，并发不足或 micro-batch 少时 pipeline bubble 和 stage imbalance 明显。第三层讲 KV：TP 下每个 rank 保存自己的 head shard KV，但逻辑 block table、position 和 request state 必须一致；PP 下每个 stage 保存自己负责层的 KV，finish / cancel / preemption 要跨 stage 清理。第四层讲迁移：正在 decode 的请求不容易跨 worker 迁移，因为 KV 与 layer、head shard、block table、dtype、layout、position 和 sampling state 绑定，远程拉取要和 recompute 比成本。最后用 Distributed Parallel KV Gate 收束：group boundary、TP shard、PP ownership、block table、communication、bubble、migration 和 cleanup 都可复盘，才算多 GPU inference 解释可信。

40ZZZZL59. 如何把 LLM serving engine 包装成 OpenAI-compatible API？
       考察点：OpenAI compatible API audit、toy OpenAI compatible API、API compatibility contract、chat request schema validation、SSE stream frame contract、OpenAI style error shape、bearer auth gate、model permission gate、token rate limit gate、concurrency limit gate、usage accounting check、API privacy log check、OpenAI compatible API gate。
       回答框架：先说明 OpenAI-compatible API 的价值是降低 SDK、agent framework、评测脚本和业务接入成本，但兼容必须诚实声明支持、忽略和不支持字段，不能看起来兼容、实际语义不一致。第一层讲请求流水线：生成 request_id / trace_id，做 Bearer auth、tenant / user 解析、模型权限检查、JSON schema 校验、chat messages role/content 校验、chat template 渲染、tokenize、context length 检查、RPM / TPM / concurrency 限流，然后构造内部 EngineRequest 交给 router / engine。第二层讲响应：非流式 response 要有 choices、assistant message、finish_reason、usage 和 request_id；流式 response 用 SSE，每个 chunk 是 `data: JSON` 加空行，第一个 delta 可以发 role，后续发 delta content，结束发 finish_reason 和 `data: [DONE]`。第三层讲错误：错误响应要有稳定 message、type、param、code、request_id；400 表示请求字段/上下文问题，401/403 表示认证和权限，429 表示租户限流，503 更偏系统容量或 admission overload。第四层讲限流和隐私：LLM 不能只按 QPS 限流，还要看 prompt tokens、max_tokens、并发、模型和租户额度；日志不能泄露 API key、完整 prompt 或敏感内容。最后用 OpenAI Compatible API Gate 收束：auth、schema、model permission、context、rate limit、non-stream、stream、usage、error shape 和 privacy 都可复盘，才算 API 层可信。

40ZZZZL60. 如何把 LLM serving 系统部署到生产环境？
       考察点：Production deployment audit、toy production deployment auditor、runtime compatibility matrix、model artifact manifest、startup readiness gate、warmup probe check、worker registration gate、drain completion gate、rolling update capacity gate、canary traffic split、canary regression gate、rollback readiness gate、production deployment gate。
       回答框架：先说明 LLM serving 的生产 ready 不等于进程启动，必须把镜像、GPU runtime、模型 artifact、启动阶段、健康检查、router 注册、滚动升级、灰度和回滚放进同一个部署闭环。第一层讲交付物：镜像固定 CUDA runtime、PyTorch、NCCL、自定义 kernel 和 serving 代码；模型权重、tokenizer、config、chat template 用 manifest 和 hash 整体版本化；driver、GPU arch、CUDA runtime 和健康端点用 Runtime Compatibility Matrix 校验。第二层讲启动和健康检查：worker 依次检查配置、GPU、runtime、模型、tokenizer、parallel group、KV cache 和 warmup；startup probe 给冷启动时间，liveness 看进程/event loop，readiness 只有全部完成并注册 router 后才成功。第三层讲发布：滚动升级要显式计算 ready new、ready old、draining 和有效容量，满足 min ready capacity 后才能 drain 老 worker；drain 停止接新请求但处理已有 stream。第四层讲灰度和回滚：router 按 revision、tenant 或 user hash 稳定分流，观察 error rate、TTFT、TPOT、OOM、KV pressure、NCCL error、输出长度和质量指标；旧 worker 和旧模型缓存要保留，异常时直接切回旧 revision。最后用 Production Deployment Gate 收束：artifact、readiness、rolling capacity、drain、canary、rollback 和 observability 都可复盘，才算生产发布可信。

40ZZZZL61. 如何做 LLM serving 的线上容量规划、成本估算、SLO 和故障演练？
       考察点：Capacity SLO fault drill audit、toy capacity SLO fault drill auditor、workload token profile、benchmark stable capacity、GPU count estimate、KV concurrency limit、cost attribution model、SLO error budget、admission overload policy、fault drill scenario、runbook coverage check、capacity SLO fault drill gate。
       回答框架：先说明不能只用 QPS 做 LLM serving 容量规划，因为请求成本由 input tokens、output tokens、上下文长度、并发和 KV cache 决定。第一层讲 workload profile：按短问答、RAG、代码生成、长文档等拆分，统计峰值 RPS、input token/s、output token/s、P95/P99 长度和 active tokens。第二层讲 benchmark stable capacity：用接近真实分布的压测得到单 worker 在 TTFT/TPOT SLO 下的稳定容量，不用极限吞吐；worker 数从 request、prefill、decode 和 KV 四个轴取最大，再乘规划利用率、安全余量、N+1、发布 surge 和可用区冗余，最后乘每个 worker 的 GPU 数。第三层讲成本：按 GPU 小时、利用率、存储、网络、日志/metrics 和 token 粗分摊到模型、租户和 workload。第四层讲 SLO：按 workload 定义 availability、5xx、timeout、TTFT、TPOT、E2E 和 error budget，告警看 burn rate、queue time、KV pressure、preemption、OOM、NCCL error 和 worker health。第五层讲过载和演练：容量不足时 admission control 做限流、拒绝、max_tokens / context 限制、小模型降级或异步处理；故障演练覆盖 worker 丢失、GPU OOM、模型加载失败、依赖不可用、长上下文突增和新版本 TPOT 退化，并要求 runbook 有 detect、mitigate、verify。最后用 Capacity SLO Fault Drill Gate 收束：profile、benchmark、KV、cost、SLO、admission、fault drill 和 runbook 都可复盘，才算容量设计可信。

40ZZZZL62. 推理框架面试如何从背名词升级成系统化表达？
       考察点：Inference engine interview readiness audit、toy inference engine interview auditor、interview question rubric、concept coverage score、request lifecycle answer、KV cache answer evidence、scheduler answer evidence、serving metrics coverage、ordered debug path、production governance answer、project evidence portfolio、tradeoff coverage、inference engine interview gate。
       回答框架：先说明推理框架面试不是背 vLLM、PagedAttention 或 SGLang 名词，而是证明自己能从系统边界、请求生命周期、数据结构、调度策略、指标、debug、生产治理和项目证据形成闭环。第一层讲回答骨架：目标是 TTFT、TPOT、吞吐、显存、稳定性和成本；生命周期从 API、tokenizer、router、scheduler、prefill、decode、sampler、streamer 到 cleanup；核心数据结构包括 waiting queue、running batch、KV block table、physical block、ref count、prefix cache 和 request state。第二层讲机制：continuous batching 是 iteration-level scheduler，PagedAttention 是 KV block 管理和访存优化，chunked prefill 平衡长 prompt 和 streaming decode，prefix cache 复用稳定前缀但要绑定模型、tokenizer、chat template 和 tenant。第三层讲指标和排查：TTFT 高看 queue、prefill、prompt length、prefix hit；TPOT 高看 decode batch、KV bandwidth、kernel、sampling、网络和 client backpressure；OOM 看 KV pressure、block leak、max model len、batch tokens 和 preemption。第四层讲生产：API 兼容、鉴权、限流、health check、rolling update、canary、capacity plan、SLO 和 fault drill。第五层讲证据：用项目 demo、benchmark report、metrics dashboard、runbook、事故复盘和 trade-off 说明自己做过或能落地。最后用 Inference Engine Interview Gate 收束：lifecycle、KV、scheduler、metrics、debug、production、project evidence 和 trade-off 都可复盘，才算回答可信。
