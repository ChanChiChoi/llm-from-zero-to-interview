...
其中 `g_i` 表示目标是否清楚，`m_i` 表示指标树是否完整，`b_i` 表示 baseline 是否冻结，`e_i` 表示实验记录是否可复现，`v_i` 表示版本 trace 是否完整，`r_i` 表示 RACI 或 DRI 责任边界是否明确，`c_i` 表示变更控制是否执行，`s_i` 表示风险是否升级，`o_i` 表示灰度上线是否就绪，`k_i` 表示回滚是否就绪，`d_i` 表示 bad case 是否进入回归测试，`a_i` 表示复盘 action item 是否关闭，`\ell_i` 表示线上 P95 延迟，`u_i` 表示成本相对预算的比例。
...
```math
R_{\mathrm{rollout}}=\frac{1}{N}\sum_{i=1}^{N}\mathbf{1}[o_i=\mathrm{true}],
\qquad
R_{\mathrm{rollback}}=\frac{1}{N}\sum_{i=1}^{N}\mathbf{1}[k_i=\mathrm{true}]
```
...
```math
R_{\mathrm{post}}=\frac{1}{N}\sum_{i=1}^{N}\mathbf{1}[\mathrm{postmortem}_i=\mathrm{complete}],
\qquad
...
G_{\mathrm{proj}}=
\mathbf{1}[
R_{\mathrm{goal}}\ge \tau_g
\land R_{\mathrm{metric}}\ge \tau_m
\land R_{\mathrm{base}}\ge \tau_b
\land R_{\mathrm{exp}}\ge \tau_e
\land R_{\mathrm{trace}}\ge \tau_v
\land R_{\mathrm{raci}}\ge \tau_r
\land R_{\mathrm{change}}\ge \tau_c
\land R_{\mathrm{risk}}\ge \tau_s
\land R_{\mathrm{rollout}}\ge \tau_o
\land R_{\mathrm{rollback}}\ge \tau_k
\land R_{\mathrm{badcase}}\ge \tau_d
\land R_{\mathrm{post}}\ge \tau_p
\land R_{\mathrm{action}}\ge \tau_a
\land \ell_{95}\le L_{\max}
\land \bar{u}\le U_{\max}
]
```
...
```text
metrics= {'objective_clarity_rate': 0.6, 'metric_tree_coverage': 0.6, 'baseline_coverage': 0.6, 'experiment_reproducibility_rate': 0.6, 'version_trace_coverage': 0.6, 'raci_coverage': 0.4, 'change_control_coverage': 0.6, 'risk_escalation_coverage': 0.2, 'rollout_readiness': 0.4, 'rollback_readiness': 0.6, 'bad_case_regression_coverage': 0.2, 'postmortem_completeness': 0.4, 'action_item_closure': 0.2, 'p95_latency_ms': 6500, 'avg_cost_ratio': 1.186}
root_causes= {'rag_mvp_ok': 'pass', 'metric_drift_no_guardrail': 'metric_misalignment', 'prompt_hotfix_untracked': 'unreproducible_experiment', 'agent_scope_creep': 'unclear_goal', 'incident_no_owner': 'unclear_goal'}
failed_gates= ['objective_clarity_rate', 'metric_tree_coverage', 'baseline_coverage', 'experiment_reproducibility_rate', 'version_trace_coverage', 'raci_coverage', 'change_control_coverage', 'risk_escalation_coverage', 'rollout_readiness', 'rollback_readiness', 'bad_case_regression_coverage', 'postmortem_completeness', 'action_item_closure', 'p95_latency_ms']
