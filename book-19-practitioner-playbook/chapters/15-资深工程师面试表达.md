...
直觉：如果项目回答缺 baseline、指标、失败样例、上线证据或安全边界，即使讲得流畅，也很难支撑资深判断。

项目证据分可以写成：

$$
E_i=0.20C_{\mathrm{goal}}+0.15C_{\mathrm{base}}+0.20C_{\mathrm{metric}}+0.15C_{\mathrm{bad}}+0.15C_{\mathrm{debug}}+0.15C_{\mathrm{prod}}
$$

其中，`C_goal` 看目标和约束是否清楚，`C_base` 看 baseline 是否可比较，`C_metric` 看指标、评估集和分桶是否清楚，`C_bad` 看 bad cases 是否具体，`C_debug` 看排查链路是否分层，`C_prod` 看灰度、监控、回滚、成本和安全是否进入上线闭环。
...
$$
R_{\mathrm{star},i}=\frac{S_i+T_i+A_i+O_i+\rho_i}{5}
$$

其中，`S_i`、`T_i`、`A_i`、`O_i` 分别表示 Situation、Task、Action、Outcome 是否清楚，`\rho_i` 表示 Reflection 是否具体。这里用 Outcome 代替 Result 记号，是为了避免和上文复盘变量混淆。对大模型项目，Reflection 不能只说“以后沟通更好”，而要落到评估集、门禁、版本、监控、权限或行动项关闭。
...
$$
G_{\mathrm{interview}}=I(E_i\ge 0.75)I(D_i\ge 0.70)I(R_{\mathrm{star},i}\ge 0.80)I(F_i\ge 0.70)I(H_i=0)
$$

其中，`H_i` 是红旗数量，例如编造指标、把所有问题归因于模型不够大、回避失败实验、没有权限安全边界、无法解释自己的具体贡献。`G_interview=0` 不代表不能去面试，而是说明这个项目故事还没有达到“可以稳定承受深挖”的状态。
