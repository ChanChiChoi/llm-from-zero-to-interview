...
按照 `WRITING_PLAN.md` 的要求，本讲精修前核对了 CLIP、LAION-5B、Whisper、WebVid / Frozen in Time、VQA、TextVQA 和 DocVQA 等公开论文资料。

本讲聚焦图文、OCR、语音、视频和多模态指令数据的防御性数据治理：合法来源、许可证、图文 / 音文 / 视频字幕对齐、OCR 空间结构、ASR 时间戳、视频切片、隐私脱敏、评测污染、质量评分、配比和版本审计。

```text
媒体来源 -> 模态解析 -> 质量检测 -> 跨模态对齐 -> 隐私 / 版权 -> 污染隔离 -> 训练样本 -> 评估闭环 -> 版本治理
```

本讲不提供未授权媒体抓取、规避平台访问控制、身份识别、声纹追踪、车牌 / 人脸追踪或敏感个人信息复原方法。多模态数据只讨论训练数据治理和安全边界。
...
其中 `D_it` 是 image-text 数据，`D_ocr` 是 OCR / 文档图像数据，`D_aud` 是 audio-transcript 数据，`D_vid` 是 video-caption / video-QA 数据，`D_inst` 是多模态指令数据。

一条样本可以表示为：

```math
m_i=(u_i,t_i,q_i,a_i,r_i,z_i)
```

其中 `u_i` 是媒体对象或媒体片段元数据，`t_i` 是文本、caption、transcript、subtitle 或 answer，`q_i` 是媒体质量分，`a_i` 是跨模态对齐分，`r_i` 是隐私、版权、安全和污染风险，`z_i` 是来源、license、时间戳、hash、OCR / ASR 版本和标注版本。

图文对齐可以用归一化 embedding 的余弦相似度近似：

```math
A_{\mathrm{it},i}=\frac{f_I(I_i)^\top f_T(T_i)}{\|f_I(I_i)\|_2\|f_T(T_i)\|_2}
```

这个分数只能作为过滤信号，不能替代人工抽检。图表、文档、新闻、商品图和截图都可能出现语义相关但描述不充分的问题。

OCR / 文档样本需要保留空间结构：

```math
O_i=\{(s_{ij},b_{ij},p_{ij})\}_{j=1}^{n_i}
```

其中 `s_ij` 是识别文本，`b_ij=(x_ij,y_ij,h_ij,omega_ij)` 是位置框，`omega_ij` 表示框宽度，`p_ij` 是 OCR 置信度。只保存纯文本会丢失阅读顺序、表格结构和版面关系。
...
其中 `S` 是替换数，`D` 是删除数，`I` 是插入数，`N` 是参考 transcript 的词数。中文或字符级任务也可以看 CER。

音文时间对齐误差可以写成：

```math
E_{\mathrm{aud},i}=\frac{1}{K_i}\sum_{k=1}^{K_i}|\hat{t}_{ik}-t_{ik}|
```

如果字幕或 transcript 和音频错位，模型会学到错误对齐。

视频切片后帧数可以粗略写成：

```math
F_i=\lfloor \mathrm{fps}_i\cdot \Delta t_i\rfloor
```

其中 `Delta t_i` 是片段时长。训练时还要记录关键帧、字幕时间、ASR 时间、OCR 时间和事件标签。

视频字幕或事件对齐误差可以写成：

```math
E_{\mathrm{vid},i}=|\hat{t}_{\mathrm{caption},i}-t_{\mathrm{event},i}|
```

视频数据比图像多一个时间维度，`E_vid` 过大时，即使字幕文本质量高，也不应直接进入训练。

隐私风险率可以按样本数或 token / 时长统计：

```math
R_{\mathrm{privacy}}=\frac{\sum_i I(P_i=1)}{n}
```

其中 `P_i` 表示人脸、车牌、证件、声纹、位置、医疗信息、私人聊天或其他敏感信息命中。多模态隐私不能只看文本转录，也要看原始媒体。

多模态样本门禁可以写成：

```math
G_i=I(q_i\ge \tau_q)I(a_i\ge \tau_a)I_{\mathrm{license},i}I_{\mathrm{privacy},i}I_{\mathrm{time},i}I_{\mathrm{contam},i}I_{\mathrm{ground},i}
```

其中 `I_ground` 表示多模态问答或指令回答确实基于输入模态，而不是凭语言先验猜测。

最终上线门禁可以写成：

```math
G_{\mathrm{mm}}=I(C_{\mathrm{modality}}\ge c_{\min})I(R_{\mathrm{privacy}}\le \rho_{\max})I(R_{\mathrm{contam}}\le \kappa_{\max})I(E_{\mathrm{aud}}\le e_{\mathrm{aud}})I(E_{\mathrm{vid}}\le e_{\mathrm{vid}})
```

面试里要强调：多模态数据工程的核心是对齐、证据和治理，不是把图片、音频、视频文件和文本简单拼接。
...
下面这个 demo 不依赖外部库，也不读写文件。输入是一组 toy 图文、OCR、语音、视频和多模态指令样本；输出包括保留样本、拒绝原因、分模态保留率、最终 mixture、平均对齐分和门禁结果。

它演示的是元数据级治理机制，不是真实 CLIP scorer、OCR、ASR、视频事件检测器、版权系统或隐私识别器。真实系统需要接入媒体解析、perceptual hash、embedding 检索、OCR / ASR / VAD / forced alignment、隐私脱敏、许可证审查、人工抽样和训练 ablation。
...
def score(item):
    return round(0.38 * item["media_quality"] + 0.42 * item["alignment"] + 0.20 * item["annotation"], 3)


def reject_reason(item, q):
    if not item["license_ok"]:
        return "license_or_source"
    if item["privacy"]:
        return "privacy_or_sensitive"
    if item["contam"]:
        return "eval_contamination"
    if item["time_error"] > MAX_TIME_ERROR[item["kind"]]:
        return "time_misalignment"
    if not item["grounded"]:
        return "ungrounded_answer"
    if item["media_quality"] < 0.60:
        return "low_media_quality"
...
align_sum = defaultdict(float)

for item in samples:
    kind_raw[item["kind"]] += item["tokens"]
for item in kept:
    kind_kept[item["kind"]] += item["tokens"]
    align_sum[item["kind"]] += item["alignment"]
...
report = {
    ...
    "kind_retention": {k: round(kind_kept[k] / kind_raw[k], 3) for k in sorted(kind_raw)},
    "mixture": {k: round(kind_kept[k] / kept_tokens, 3) for k in sorted(kind_kept)},
    "avg_alignment": {k: round(align_sum[k] / sum(1 for item in kept if item["kind"] == k), 3) for k in sorted(kind_kept)},
    "score_preview": {row["id"]: row["score"] for row in rows},
}

gates = {
    "license_blocked": reason_counts.get("license_or_source", 0) == 1,
    "privacy_blocked": reason_counts.get("privacy_or_sensitive", 0) == 1,
    "contamination_blocked": reason_counts.get("eval_contamination", 0) == 1,
    "time_alignment": reason_counts.get("time_misalignment", 0) == 1,
    "grounding": reason_counts.get("ungrounded_answer", 0) == 1,
    "modality_coverage": set(kind_kept) == {"audio_transcript", "image_text", "mm_instruction", "ocr_doc", "video_caption"},
    "alignment_floor": all(v >= 0.80 for v in report["avg_alignment"].values()),
}
report["gates"] = gates
...
assert report["retention"] == 0.425
assert report["kind_retention"] == {"audio_transcript": 0.517, "image_text": 0.547, "mm_instruction": 0.511, "ocr_doc": 0.524, "video_caption": 0.285}
...
rejected= {'audio_noisy_bad_asr': 'low_media_quality', 'img_decor_misaligned': 'low_alignment', 'mm_hallucinated_answer': 'ungrounded_answer', 'ocr_private_id': 'privacy_or_sensitive', 'video_eval_leak': 'eval_contamination', 'video_subtitle_shift': 'time_misalignment', 'video_unlicensed_clip': 'license_or_source'}
reason_counts= {'eval_contamination': 1, 'license_or_source': 1, 'low_alignment': 1, 'low_media_quality': 1, 'privacy_or_sensitive': 1, 'time_misalignment': 1, 'ungrounded_answer': 1}
retention= 0.425
kind_retention= {'audio_transcript': 0.517, 'image_text': 0.547, 'mm_instruction': 0.511, 'ocr_doc': 0.524, 'video_caption': 0.285}
...
gates= {'license_blocked': True, 'privacy_blocked': True, 'contamination_blocked': True, 'time_alignment': True, 'grounding': True, 'modality_coverage': True, 'alignment_floor': True}
gate_pass= True
```
