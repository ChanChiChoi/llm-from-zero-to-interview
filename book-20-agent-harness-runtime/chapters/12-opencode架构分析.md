...
1. OpenCode 是开源项目，公开文档和仓库能支持我们分析它的配置面、工具面、权限面和 server / SDK 接口。
...
```math
o_i=(n_i,g_i,d_i,p_i,c_i,s_i,r_i)
```
...
```math
C_{\mathrm{doc}}=\frac{1}{N}\sum_{i=1}^{N} d_i
```

开放 runtime 模块覆盖率：

```math
C_{\mathrm{rt}}=\frac{\sum_{i\in \mathcal{R}} p_i d_i}{|\mathcal{R}|}
```

这里 `\mathcal{R}` 包含 multi-client、server、session、agents、providers、tools、permissions、files、shell、search、LSP、MCP、custom tools、plugins、snapshots、diff、compaction、SDK 和 events。

配置治理覆盖率：

```math
C_{\mathrm{cfg}}=\frac{\sum_{i\in \mathcal{C}} c_i}{|\mathcal{C}|}
```

配置治理要覆盖 global config、project config、remote config、managed config、agent config、tool config 和 permission config。配置合并越灵活，越要审计冲突覆盖和组织级约束。

Agent 隔离覆盖率：

```math
C_{\mathrm{agent}}=\frac{\sum_{i\in \mathcal{A}} c_i}{|\mathcal{A}|}
```

这里 `\mathcal{A}` 包含 primary agent、subagent、plan / build mode、hidden system agent、自定义 agent、agent-level permission 和 step budget。

工具权限绑定覆盖率：

```math
C_{\mathrm{tool}}=\frac{\sum_{i\in \mathcal{T}} p_i c_i}{|\mathcal{T}|}
```

扩展面治理覆盖率：

```math
C_{\mathrm{ext}}=\frac{\sum_{i\in \mathcal{E}} p_i c_i}{|\mathcal{E}|}
```

这里 `\mathcal{E}` 包含 MCP servers、custom tools、plugins、skills、commands 和 providers。扩展能力不是越多越好，必须同时有 schema、namespace、鉴权、权限绑定、上下文成本控制和 trace。

Server / SDK 治理覆盖率：

```math
C_{\mathrm{api}}=\frac{\sum_{i\in \mathcal{B}} c_i}{|\mathcal{B}|}
```

这里 `\mathcal{B}` 包含 HTTP server、OpenAPI、SDK、event stream、permission response、session API、file API 和 share API。server 一旦可远程访问，就必须讨论认证、网络边界、最小权限和审计。

状态恢复覆盖率：

```math
C_{\mathrm{recover}}=\frac{\sum_{i\in \mathcal{S}} p_i s_i}{|\mathcal{S}|}
```

这里 `\mathcal{S}` 包含 snapshot、diff、revert、session fork、abort、summary、compaction、todo 和 message parts。
...
```math
G_{\mathrm{opencode}}=
\mathbb{1}[C_{\mathrm{doc}}\ge \tau_{\mathrm{doc}}]
\mathbb{1}[C_{\mathrm{rt}}\ge \tau_{\mathrm{rt}}]
\mathbb{1}[C_{\mathrm{tool}}\ge \tau_{\mathrm{tool}}]
\mathbb{1}[C_{\mathrm{recover}}\ge \tau_{\mathrm{recover}}]
\mathbb{1}[C_{\mathrm{risk}}\ge \tau_{\mathrm{risk}}]
```

面试中可以强调：OpenCode 的开放性降低了资料不透明问题，但也扩大了审计面。你要能同时讲清楚“能扩展什么”和“如何防止扩展能力变成风险面”。
...
capabilities = [
    {"name": "tui_client", "group": "runtime", "documented": True, "covered": True, "governed": True, "stateful": True, "required": True, "risk": False},
    {"name": "ide_client", "group": "runtime", "documented": True, "covered": True, "governed": True, "stateful": True, "required": True, "risk": False},
    {"name": "server_api", "group": "runtime", "documented": True, "covered": True, "governed": False, "stateful": True, "required": True, "risk": True},
    {"name": "sdk_client", "group": "runtime", "documented": True, "covered": True, "governed": False, "stateful": True, "required": True, "risk": True},
    {"name": "session_manager", "group": "runtime", "documented": True, "covered": True, "governed": True, "stateful": True, "required": True, "risk": False},
    {"name": "global_config", "group": "config", "documented": True, "covered": True, "governed": True, "stateful": False, "required": True, "risk": False},
    {"name": "project_config", "group": "config", "documented": True, "covered": True, "governed": True, "stateful": False, "required": True, "risk": False},
    {"name": "remote_config", "group": "config", "documented": True, "covered": True, "governed": False, "stateful": False, "required": True, "risk": True},
    {"name": "managed_config", "group": "config", "documented": True, "covered": False, "governed": False, "stateful": False, "required": True, "risk": True},
    {"name": "primary_agents", "group": "agent", "documented": True, "covered": True, "governed": True, "stateful": True, "required": True, "risk": False},
    {"name": "subagents", "group": "agent", "documented": True, "covered": True, "governed": True, "stateful": True, "required": True, "risk": False},
    {"name": "plan_build_modes", "group": "agent", "documented": True, "covered": True, "governed": True, "stateful": True, "required": True, "risk": False},
    {"name": "custom_agent_permissions", "group": "agent", "documented": True, "covered": True, "governed": False, "stateful": True, "required": True, "risk": True},
    {"name": "bash_tool", "group": "tool", "documented": True, "covered": True, "governed": False, "stateful": True, "required": True, "risk": True},
    {"name": "file_edit_tools", "group": "tool", "documented": True, "covered": True, "governed": True, "stateful": True, "required": True, "risk": True},
    {"name": "search_tools", "group": "tool", "documented": True, "covered": True, "governed": True, "stateful": False, "required": True, "risk": False},
    {"name": "lsp_tool", "group": "tool", "documented": True, "covered": True, "governed": False, "stateful": False, "required": True, "risk": False},
    {"name": "permission_patterns", "group": "tool", "documented": True, "covered": True, "governed": True, "stateful": False, "required": True, "risk": False},
    {"name": "mcp_servers", "group": "extension", "documented": True, "covered": True, "governed": False, "stateful": True, "required": True, "risk": True},
    {"name": "custom_tools", "group": "extension", "documented": True, "covered": True, "governed": False, "stateful": False, "required": True, "risk": True},
    {"name": "plugins", "group": "extension", "documented": True, "covered": False, "governed": False, "stateful": False, "required": True, "risk": True},
    {"name": "provider_adapters", "group": "extension", "documented": True, "covered": True, "governed": True, "stateful": False, "required": True, "risk": False},
    {"name": "http_auth_boundary", "group": "api", "documented": True, "covered": True, "governed": False, "stateful": False, "required": True, "risk": True},
    {"name": "event_stream", "group": "api", "documented": True, "covered": True, "governed": True, "stateful": True, "required": True, "risk": False},
    {"name": "permission_response_api", "group": "api", "documented": True, "covered": True, "governed": True, "stateful": True, "required": True, "risk": True},
    {"name": "share_api", "group": "api", "documented": True, "covered": True, "governed": False, "stateful": True, "required": True, "risk": True},
    {"name": "snapshot", "group": "state", "documented": True, "covered": True, "governed": True, "stateful": True, "required": True, "risk": False},
    {"name": "diff_revert", "group": "state", "documented": True, "covered": True, "governed": True, "stateful": True, "required": True, "risk": False},
    {"name": "session_fork_abort", "group": "state", "documented": True, "covered": True, "governed": True, "stateful": True, "required": True, "risk": False},
    {"name": "summary_compaction", "group": "state", "documented": True, "covered": True, "governed": False, "stateful": True, "required": True, "risk": False},
    {"name": "todo_state", "group": "state", "documented": True, "covered": True, "governed": True, "stateful": True, "required": True, "risk": False},
    {"name": "regression_harness", "group": "eval", "documented": True, "covered": False, "governed": False, "stateful": False, "required": True, "risk": False},
    {"name": "safety_eval_cases", "group": "eval", "documented": True, "covered": False, "governed": False, "stateful": False, "required": True, "risk": False},
    {"name": "cost_latency_report", "group": "eval", "documented": True, "covered": False, "governed": False, "stateful": False, "required": True, "risk": False},
]
...
def group_items(group):
    return [item for item in capabilities if item["group"] == group and item["required"]]
...
risky = [item for item in capabilities if item["risk"]]

metrics = {
    "public_docs": ratio(sum(item["documented"] for item in capabilities), len(capabilities)),
    "runtime_module_coverage": ratio(
        sum(item["covered"] and item["documented"] for item in required),
        len(required),
    ),
    "config_governance": governed("config"),
    "agent_isolation": governed("agent"),
    "tool_permission_binding": ratio(
        sum(item["covered"] and item["governed"] for item in group_items("tool")),
        len(group_items("tool")),
    ),
    "extension_governance": ratio(
        sum(item["covered"] and item["governed"] for item in group_items("extension")),
        len(group_items("extension")),
    ),
    "server_api_governance": governed("api"),
    "snapshot_recovery": ratio(
        sum(item["covered"] and item["stateful"] for item in group_items("state")),
        len(group_items("state")),
    ),
    "eval_readiness": ratio(
        sum(item["covered"] and item["governed"] for item in group_items("eval")),
        len(group_items("eval")),
    ),
    "high_risk_governance": ratio(sum(item["governed"] for item in risky), len(risky)),
}
...
for item in capabilities:
    ...
    if item["required"] and not item["covered"]:
        root_causes[name].append("required_open_runtime_capability_not_covered")
    if item["covered"] and not item["documented"]:
        root_causes[name].append("undocumented_capability_claim")
    if item["required"] and item["covered"] and not item["governed"]:
        root_causes[name].append("covered_but_not_governed")
    if item["risk"] and not item["governed"]:
        root_causes[name].append("high_risk_extension_or_api_ungoverned")
    if item["group"] == "state" and item["covered"] and not item["stateful"]:
        root_causes[name].append("state_capability_without_recovery_signal")

thresholds = {
    "public_docs": 1.0,
    "runtime_module_coverage": 0.9,
    "config_governance": 0.8,
    "agent_isolation": 0.8,
    "tool_permission_binding": 0.8,
    "extension_governance": 0.75,
    "server_api_governance": 0.8,
    "snapshot_recovery": 0.9,
    "eval_readiness": 0.8,
...
这个 demo 的失败点很适合面试复盘：OpenCode 的公开文档覆盖很强，但开放 runtime 的高风险面也很多。server、SDK、remote config、custom tools、MCP、plugins、share API、bash 和 agent-level permission 如果没有治理表，就会从“扩展性”变成“风险面”。评估准备度为 0 也提醒我们：架构分析不能停在模块枚举，还要落到 regression harness、安全样本、成本延迟报告和版本捕获。
