...
Model Release Benchmark Catalog：模型发布评测集目录；按知识、数学、代码、Agent、长上下文、事实性、安全和多模态组织模型发布报告中常见 benchmark 的速查表。

MMLU-Pro：更高难度的多学科多选评测集，相比 MMLU 增加更复杂题目和更多干扰选项，用于区分强模型的知识与推理能力。

Humanity's Last Exam / HLE：专家级、多学科、多模态闭集 benchmark，用于在传统考试类评测接近饱和后继续区分 frontier model。

SWE-Lancer：基于真实 freelance 软件工程任务的软件工程 benchmark，覆盖独立实现、方案选择和更接近商业需求的 coding agent 能力。

DeepSWE：面向 long-horizon coding agent 的原创软件工程 benchmark，强调新任务、真实仓库、手写 verifier 和降低训练污染。

BrowseComp：面向 browsing agent 的网页检索 benchmark，要求模型通过浏览网页找到难检索但可验证的短答案。

SimpleQA：短事实问答 benchmark，用于评估模型事实性、校准和“知道自己不知道”的能力。

RULER：长上下文能力 benchmark，覆盖检索、多 needle、多跳、聚合和 QA 等比简单 needle 测试更系统的任务。

FRAMES：面向 RAG factuality 的多跳问答 benchmark，要求模型检索多个来源、整合证据并给出可核验答案。

MMMU：多学科多模态 benchmark，考察模型对图像、专业知识和跨模态推理的综合能力。

MathVista：视觉数学推理 benchmark，覆盖图形、表格、图像和数学推理结合的题型。

OCRBench：OCR 相关多模态评测集，用于检查场景文字、文档文字和关键信息抽取能力。

Video-MME：视频多模态理解 benchmark，用于评估视频时序理解、长视频理解和音视频信息整合。

Frontier Model Release Radar：前沿模型发布雷达；用新模型 system card、technical report 和发布说明反推架构、训练、后训练、推理、Agent、多模态、评估和安全治理趋势的方法。

Programmatic Tool Calling：程序化工具调用；把确定性工具链、批处理、数据转换、结果聚合等步骤下沉到 workflow、script 或 runtime，减少模型上下文和 round trip 压力。

Multi-Agent Test-Time Compute：多 Agent 推理时计算；用多个 Agent 或 worker 并行探索、执行和验证，再聚合结果的高预算推理形态。

Risk-Calibrated Access：风险校准访问；按用户、任务、数据敏感度、工具风险和模型能力分层开放模型或 Agent 能力。

Fallback Routing：回退路由；当风险、置信度、权限、成本或系统状态不满足要求时，将请求切换到更保守模型、只读模式、人工审核、拒答或降级流程。

Skill / MCP Lifecycle：Skill / MCP 生命周期；用于判断 Skill、MCP、Connector、Plugin、Workflow 等生态技术是处于新兴、抢占生态位、标准化、被模型能力吸收，还是过渡/退潮阶段。

MetaP：Meta 在 Llama 4 发布中提到的训练稳定性相关技术信号；在本项目中作为前沿训练 recipe 观察项，不把具体实现泛化为通用标准。

iRoPE：Meta 在 Llama 4 发布中提到的 RoPE 相关位置编码信号；在本项目中作为超长上下文和位置编码观察项处理。
