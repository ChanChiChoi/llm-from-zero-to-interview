...
AI Infra 面试中的公式不需要写得很复杂，但要能把系统对象量化。下面这些公式可以作为高频题的最小工具箱。

训练平台先看有效 batch、训练步数、训练算力和故障损失：

```math
B_{\mathrm{eff}}=B_{\mathrm{micro}}A_{\mathrm{grad}}W
```

其中 `B_micro` 是单卡 micro batch，`A_grad` 是梯度累积步数，`W` 是 data parallel world size。

```math
S=\left\lceil \frac{D_{\mathrm{tok}}}{B_{\mathrm{eff}}} \right\rceil,\qquad
C_{\mathrm{train}}\approx 6ND_{\mathrm{tok}}
```

其中 `D_tok` 是训练 token 数，`N` 是 dense Transformer 参数量。`6ND` 是粗估，适合面试中做量级判断，不适合替代真实 profiler。

```math
L_{\mathrm{lost}}=G\frac{t_{\mathrm{fail}}-t_{\mathrm{ckpt}}}{3600}
```

其中 `G` 是占用 GPU 数，`t_fail - t_ckpt` 是距离最近 checkpoint 的秒数。它回答“checkpoint 间隔太长会浪费多少 GPU 小时”。

推理平台先拆 TTFT、TPOT 和 KV cache：

```math
\mathrm{TTFT}=T_{\mathrm{gateway}}+T_{\mathrm{router}}+T_{\mathrm{queue}}+T_{\mathrm{tokenize}}+T_{\mathrm{prefill}}
```
...
其中 `N_out` 是输出 token 数；如果只生成 1 个 token，应单独报告 TTFT，不强行计算 TPOT。

```math
M_{\mathrm{KV}}=2LBTH_{\mathrm{kv}}D_h b
```

其中 `L` 是层数，`B` 是并发序列数，`T` 是上下文长度，`H_kv` 是 KV head 数，`D_h` 是 head dim，`b` 是每个元素字节数。前面的 2 来自 Key 和 Value 两份缓存。

SLO 和成本题要能落到错误预算和单位经济账：

```math
A=1-\frac{N_{\mathrm{bad}}}{N_{\mathrm{total}}}
```

```math
B_{\mathrm{remain}}=(1-\mathrm{SLO})N_{\mathrm{total}}-N_{\mathrm{bad}}
```

```math
K_{1k}=1000\frac{K_{\mathrm{gpu}}+K_{\mathrm{runtime}}+K_{\mathrm{storage}}+K_{\mathrm{network}}}{N_{\mathrm{input}}+N_{\mathrm{output}}}
```

多租户调度题要能解释配额、公平性和资源碎片：

```math
D_q=\max_r \frac{u_{q,r}}{c_{q,r}},\qquad
\Delta_{\mathrm{fair}}=\max_q D_q-\min_q D_q
```

其中 `u_{q,r}` 是队列 `q` 在资源 `r` 上的使用量，`c_{q,r}` 是该队列的配额或权重归一化容量。它不是唯一公平性定义，但能帮助面试中把“公平”从口号变成可度量对象。
...
class MiniAIInfraInterviewReadinessAudit:
    def __init__(self):
        ...
        self.remediation_actions = {
            "scope_boundary_clarity": "用 30 秒区分 AI Infra、MLOps 和 LLMOps，避免只报工具名。",
            "training_platform_lifecycle": "补 TrainingJob 状态机、launcher、checkpoint、artifact 和实验追踪链路。",
            "gpu_scheduler_quota_fairness": "补 queue、quota、dominant share、gang scheduling 和拓扑放置口径。",
            "training_debug_evidence": "补 rank log、loss 曲线、NaN 检查、NCCL / data wait 证据和最小复现。",
            "inference_platform_token_slo": "把 QPS 改成 input tokens/s、output tokens/s、TTFT、TPOT 和 KV 水位。",
            ...
            "security_multitenancy": "补身份、RBAC / ABAC、tenant cache key、RAG 权限和日志隔离。",
            "incident_postmortem": "补事故时间线、影响量化、根因、回滚、action item 和回归验证。",
            "tradeoff_boundary_reasoning": "补 latency vs cost、fairness vs utilization、self-service vs governance。",
            "interview_revision_gate": "把每道弱题绑定到一个缺失公式、一个 toy demo 和一个修复动作。",
        }
    ...
    def run(self):
        ...
        revision_plan = {
            case_id: [
                {
                    "missing_gate": gate,
                    "action": self.remediation_actions[gate],
                }
                for gate in gates
            ]
            for case_id, gates in hard_blockers
        }
        ...
        return {
            ...
            "metrics": metrics,
            "weak_questions": failed_cases,
            "revision_plan_sample": {
                case_id: revision_plan[case_id]
                for case_id in failed_cases[:3]
            },
...
print("weak_questions=", result["weak_questions"], sep="")
print("revision_plan_sample=", result["revision_plan_sample"], sep="")
